        ...
        public function setMode($mode)
        {
            if ($mode !== 0 && $mode !== 1) {
                throw new RedException(
                    "Invalid mode selected for logger, use 1 or 0."
                );
            }
            $this->mode = $mode;
            return $this;
    ...
    class RPDO implements Driver
    {
        ...
        protected function runQuery($sql, $bindings, $options = [])
        {
            ...
            try {
                ...
                if ($statement->columnCount()) {
                    $fetchStyle = isset($options["fetchStyle"])
                        ? $options["fetchStyle"]
                        : null;
        ...
        public function connect()
        {
            ...
            try {
                ...
                $dbname = preg_match("/dbname=(\w+)/", $this->dsn, $matches)
                    ? $matches[1]
                    : "?";
    ...
    class OODBBean implements \IteratorAggregate, \ArrayAccess, \Countable
    {
            ...
            if ($this->getID() > 0) {
                ...
                if (!is_numeric($firstKey) || $firstKey === null) {
                    ...
                    $beans = $redbean->find(
                        $type,
                        [],
                        " $myFieldLink = :slot0 " . $this->withSql,
                        $bindings
                    );
                } else {
        ...
        public function import($array, $selection = false, $notrim = false)
        {
            ...
            foreach ($array as $key => $value) {
                if ($key != "__info") {
                    if (
                        !$selection ||
                        ($selection && in_array($key, $selection))
                    ) {
                        if (is_array($value)) {
                            if (isset($value["_type"])) {
                                $bean = $this->beanHelper
                                    ->getToolbox()
                                    ->getRedBean()
        ...
        public function __isset($property)
        {
            ...
            if (
                ...
                ctype_upper(substr($property, 4, 1))
            ) {
        ...
        public function beau($property)
        {
            ...
            if (!isset($beautifulColumns[$property])) {
                $beautifulColumns[$property] = AQueryWriter::camelsSnake(
                    $property
                );
            }
        ...
        public function isListInExclusiveMode($listName)
        {
            ...
            if (
                strpos($listName, "xown") === 0 &&
                ctype_upper(substr($listName, 4, 1))
            ) {
            ...
            return isset($this->__info["sys.exclusive-" . $listName]) &&
        ...
        public function &__get($property)
        {
            ...
            $differentAlias =
                $hasAlias &&
                $isOwn &&
                isset($this->__info["sys.alias." . $listName])
            ...
            [$redbean, , , $toolbox] = $this->beanHelper->getExtractedToolbox();
        ...
        public function __set($property, $value)
        {
            ...
            if (
                ...
                if (!$this->noLoad) {
                    [
                        ...
                        ,
                        $toolbox,
                    ] = $this->beanHelper->getExtractedToolbox();
                    if ($isOwn) {
                        $beans = $this->getOwnList($listName, $redbean);
                    } else {
                        $beans = $this->getSharedList(
            ...
            if (
                array_key_exists($fieldLink, $this->properties) &&
        ...
        public function hasChanged($property)
        {
            return array_key_exists($property, $this->properties)
                ? $this->old($property) != $this->properties[$property]
                : false;
        }
        ...
        {
            return (bool) ((string) $this->properties["id"] ===
                (string) $bean->properties["id"] &&
                (string) $this->__info["type"] ===
                    (string) $bean->__info["type"]);
        }
    }
}
...
namespace RedBeanPHP {
    ...
    interface QueryWriter
    {
         ...
         * @return array
         */
        public function queryRecordRelated(
            $sourceType,
            $destType,
            $linkID,
            $addSql = "",
         ...
         * @return array|null
         */
        public function queryRecordLink(
            $sourceType,
            $destType,
            $sourceID,
            $destID
         ...
         * @return integer
         */
        public function queryRecordCountRelated(
            $sourceType,
            $targetType,
            $linkID,
            $addSQL = "",
...
namespace RedBeanPHP\QueryWriter {
    ...
    abstract class AQueryWriter //bracket must be here - otherwise coverage software does not understand.
    {
         ...
         * @return string
         */
        protected function getParametersForInClause(
            &$valueList,
            $otherBindings,
            $offset = 0
        ) {
         ...
         * @return string
         */
        private function makeSQLFromConditions(
            $conditions,
            &$bindings,
            $addSql = ""
        ) {
         ...
         * @return array
         */
        private function getRelationalTablesAndColumns(
            $sourceType,
            $destType,
            $noQuote = false
        ) {
            $linkTable = $this->esc(
                $this->getAssocTable([$sourceType, $destType]),
                $noQuote
            );
        ...
        protected function insertRecord($type, $insertcolumns, $insertvalues)
        {
            ...
            if (
                count($insertvalues) > 0 &&
                is_array($insertvalues[0]) &&
                count($insertvalues[0]) > 0
            ) {
        ...
        public static function getAssocTableFormat($types)
        {
            ...
            return isset(self::$renames[$assoc])
                ? self::$renames[$assoc]
                : $assoc;
        }
        ...
        public function glueSQLCondition($sql, $glue = null)
        {
            ...
            if (
                preg_match(
                    "/^(INNER|LEFT|RIGHT|JOIN|AND|OR|WHERE|ORDER|GROUP|HAVING|LIMIT|OFFSET)\s+/i",
                    $lsql
                )
            ) {
                if (
                    $glue === QueryWriter::C_GLUE_WHERE &&
                    stripos($lsql, "AND") === 0
                ) {
                ...
                $snippetCache[$key] =
                    ($glue === QueryWriter::C_GLUE_AND ? " AND " : " WHERE ") .
                    $sql;
            }
        ...
        public function esc($dbStructure, $dontQuote = false)
        {
            ...
            return $dontQuote
                ? $dbStructure
                : $this->quoteCharacter . $dbStructure . $this->quoteCharacter;
        }
        ...
        public function addColumn($type, $column, $field)
        {
            ...
            $type = isset($this->typeno_sqltype[$type])
        ...
        public function updateRecord($type, $updatevalues, $id = null)
        {
            ...
            if (!$id) {
                ...
                return (string) $this->insertRecord($table, $insertcolumns, [
                    $insertvalues,
                ]);
            }
        ...
        public function queryRecord(
            ...
            $addSql = $this->glueSQLCondition(
                $addSql,
                count($conditions) > 0 ? QueryWriter::C_GLUE_AND : null
            );
        ...
        public function queryRecordRelated(
            ...
            if (
                $this->flagUseCache &&
                ($cached = $this->getCached($destType, $key))
            ) {
                return $cached;
            }
        ...
        public function queryRecordLink(
            ...
            if (
                $this->flagUseCache &&
                ($cached = $this->getCached($linkTable, $key))
            ) {
                return $cached;
            }
        ...
        public function widenColumn($type, $column, $datatype)
        {
            ...
            $this->adapter->exec(
                "ALTER TABLE $table CHANGE $column $column $newtype "
            );
        }
        ...
        public function addConstraintForTypes($sourceType, $destType)
        {
            [
                ...
                $sourceCol,
                $destCol,
            ] = $this->getRelationalTablesAndColumns(
                $sourceType,
                $destType,
                true
            );
    ...
    class MySQL extends AQueryWriter implements QueryWriter
    {
        ...
        protected function constrain(
            ...
            try {
                ...
                if (
                    $this->code($columns[$property1]) !==
                    MySQL::C_DATATYPE_UINT32
                ) {
                    $this->widenColumn(
                        $table,
                        $property1,
        ...
        public function addIndex($type, $name, $column)
        {
            ...
            try {
                foreach (
                    $this->adapter->get("SHOW INDEX FROM $table ")
                    as $ind
                ) {
		...
		',
            ...
            try {
				...
				FOREIGN KEY $fkName ( {$this->esc($field)} ) REFERENCES {$this->esc(
                        $targetType
                    )} (
				{$this->esc($targetField)}) ON DELETE " .
                        ($isDependent ? "CASCADE" : "SET NULL") .
                        " ON UPDATE " .
                        ($isDependent ? "CASCADE" : "SET NULL") .
                        ";"
                );
    ...
    class SQLiteT extends AQueryWriter implements QueryWriter
    {
        ...
        protected function getIndexes($type)
        {
            ...
            foreach ($indexes as $i) {
                $indexInfoList[$i["name"]] = $this->adapter->getRow(
        ...
        public function scanType($value, $flagSpecial = false)
        {
            ...
            if (
                (is_numeric($value) && $value < 2147483648) ||
        ...
        public function code($typedescription, $includeSpecials = false)
        {
            $r = isset($this->sqltype_typeno[$typedescription])
        ...
        public function addIndex($type, $name, $column)
        {
            ...
            try {
                foreach (
                    $this->adapter->get("PRAGMA INDEX_LIST($table) ")
                    as $ind
                ) {
                    if ($ind["name"] === $name) {
                        return;
    ...
    class PostgreSQL extends AQueryWriter implements QueryWriter
    {
        ...
        public function __construct(Adapter $adapter)
        {
            $this->typeno_sqltype = [
                ...
                self::C_DATATYPE_SPECIAL_DATETIME =>
		...
		',
            ...
            try {
                if (!$cfks) {
					...
					{$this->esc(
                        $targetField
                    )}) ON DELETE $delRule ON UPDATE $delRule DEFERRABLE ;");
                }
            } catch (\Exception $e) {
                return false;
            }
...
namespace RedBeanPHP {
    ...
    class OODB extends Observable
    {
        ...
        private function handleException(\Exception $exception)
        {
            if (
                $this->isFrozen ||
                !$this->writer->sqlStateIn($exception->getSQLState(), [
        ...
        private function createTableIfNotExists(OODBBean $bean, $table)
        {
            ...
            if (
                !$this->isFrozen &&
                !$this->tableExists($this->writer->esc($table, true))
            ) {
        ...
        private function storeBean(OODBBean $bean)
        {
            if ($bean->getMeta("tainted")) {
                if (!$this->isFrozen) {
                    ...
                    $bean->id = $this->writer->updateRecord(
                        $table,
                        $updateValues,
                        $bean->id
                    );
                    ...
                    [$properties, $table] = $bean->getPropertiesAndType();
                    $id = $properties["id"];
                    unset($properties["id"]);
                    $updateValues = [];
                    $k1 = "property";
            ...
            if (!in_array($bean->getMeta("type"), $this->chillList)) {
                ...
                if (isset($columns[$this->writer->esc($property, true)])) {
                    ...
                    $sqlt = $this->writer->code(
                        $columns[$this->writer->esc($property, true)]
                    );
                    if ($typeno > $sqlt) {
        ...
        private function processAdditions($bean, $ownAdditions)
        {
            ...
            foreach ($ownAdditions as $addition) {
                if ($addition instanceof OODBBean) {
                    ...
                    $alias = $bean->getMeta(
                        "sys.alias." . $addition->getMeta("type")
                    );
                    if ($alias) {
                        $myFieldLink = $alias . "_id";
                    }
         ...
         */
        private function processEmbeddedBean(
            &$embeddedBeans,
            $bean,
            $property,
            OODBBean $value
        ...
        private function processLists(OODBBean $bean)
        {
            ...
            foreach ($bean as $property => $value) {
                $value =
                    $value instanceof SimpleModel ? $value->unbox() : $value;
        ...
        public function dispense($type, $number = 1, $alwaysReturnArray = false)
        {
            ...
            return count($beans) === 1 && !$alwaysReturnArray
                ? array_pop($beans)
                : $beans;
        }
        ...
        public function check(OODBBean $bean)
        {
            ...
            foreach ($bean as $prop => $value) {
                ...
                } elseif (strlen($prop) < 1 || preg_match($pattern, $prop)) {
                    throw new RedException(
        ...
        public function store($bean)
        {
            ...
            return (string) $bean->id === (string) (int) $bean->id
                ? (int) $bean->id
                : (string) $bean->id;
        }
        ...
        public function count($type, $addSQL = "", $bindings = [])
        {
            ...
            try {
                return (int) $this->writer->queryRecordCount(
                    $type,
                    [],
                    $addSQL,
                    $bindings
...
namespace RedBeanPHP {
    ...
    class AssociationManager extends Observable
    {
         ...
         * @return mixed
         */
        protected function associateBeans(
            OODBBean $bean1,
            OODBBean $bean2,
            OODBBean $bean
        ) {
            ...
            $indexName1 =
                "index_for_" . $bean->getMeta("type") . "_" . $property1;
            $indexName2 =
                "index_for_" . $bean->getMeta("type") . "_" . $property2;

            $bean->setMeta("buildcommand.indexes", [
                $property1 => $indexName1,
                $property2 => $indexName2,
            ]);
            ...
            try {
                ...
                if (
                    ...
                    if (!$this->oodb->isFrozen()) {
                        $this->writer->addConstraintForTypes(
                            $bean1->getMeta("type"),
                            $bean2->getMeta("type")
                        );
                    }
            ...
            foreach ($beans1 as $bean1) {
                foreach ($beans2 as $bean2) {
                    try {
                        ...
                        if ($fast) {
                            $this->writer->deleteRecord($linkType, [
                                "id" => $row["id"],
                            ]);
...
namespace RedBeanPHP {
    ...
    class Facade
    {
        ...
        private static function query($method, $sql, $bindings)
        {
            if (!self::$redbean->isFrozen()) {
                try {
                    ...
                    if (
                        self::$writer->sqlStateIn($exception->getSQLState(), [
                            QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN,
                            QueryWriter::C_SQLSTATE_NO_SUCH_TABLE,
                        ])
                    ) {
         ...
         * @return void
         */
        public static function addDatabase(
            $key,
            $dsn,
            $user = null,
            $pass = null,
            ...
            if (isset(self::$toolboxes[$key])) {
                throw new RedException(
                    "A database has already be specified for this key."
                );
            }
        ...
        {
            return $type === null
                ? self::$writer->getTables()
                : self::$writer->getColumns($type);
        }
        ...
        public static function dispense(
            ...
            $beanOrBeans = self::$redbean->dispense(
                $type,
                $num,
                $alwaysReturnArray
            );
         ...
         * @return array
         */
        public static function findOrDispense(
            $type,
            $sql = null,
            $bindings = []
        ) {
        ...
        public static function exportAll(
            ...
            return self::$duplicationManager->exportAll(
                $beans,
                $parents,
                $filters
            );
        ...
        public static function ext($pluginName, $callable)
        {
            if (!ctype_alnum($pluginName)) {
                throw new RedException(
                    "Plugin name may only contain alphanumeric characters."
                );
            }
        ...
        public static function __callStatic($pluginName, $params)
        {
            ...
            if (!isset(self::$plugins[$pluginName])) {
                throw new RedException(
                    'Plugin \'' .
                        $pluginName .
                        '\' does not exist, add this plugin using: R::ext(\'' .
                        $pluginName .
                        '\')'
                );
            }
    ...
    class DuplicationManager
    {
        ...
        private function copyOwnBeans(
            ...
            foreach ($beans as $subBean) {
                array_push(
                    $copy->$owned,
                    $this->duplicate($subBean, $trail, $preserveIDs)
                );
            }
        }
        ...
        {
            return in_array(
                AQueryWriter::getAssocTableFormat([$type, $target]),
                $this->tables
            );
        }
        ...
        protected function duplicate(
            ...
            foreach ($this->tables as $table) {
                ...
                [$owned, $shared] = $this->getListNames($table);
        ...
        public function dup(OODBBean $bean, $trail = [], $preserveIDs = false)
        {
            ...
            if (!count($this->columns)) {
                foreach ($this->tables as $table) {
                    $this->columns[$table] = $this->toolbox
                        ->getWriter()
                        ->getColumns($table);
                }
            }
         ...
         * @return array
         */
        public function exportAll($beans, $parents = false, $filters = [])
        {
            $array = [];
