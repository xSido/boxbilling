...
function handler_error(int $number, string $message, string $file, int $line)
{
    if (E_RECOVERABLE_ERROR === $number) {
        if (isCLI) {
            ...
            handler_exception(
                new ErrorException($message, $number, 0, $file, $line)
            );
        }
    } else {
...
function handler_exception($e)
{
    ...
    <div id=\"error\">
        ...
        if (defined("BB_DEBUG") && BB_DEBUG) {
            print sprintf(
                "<hr><center><small><p><b>%s</b></p></small></center>",
                "Set BB_DEBUG to FALSE, to hide the message below"
            );
        ...
        print sprintf(
            '<center><p><a href="http://docs.boxbilling.com/en/latest/search.html?q=%s&check_keywords=yes&area=default" target="_blank">Look for detailed error explanation</a></p></center>',
...
if (!file_exists($vendorPath)) {
    throw new Exception(
        "It seems like Composer packages are missing. You have to run \"<code>composer install</code>\" in order to install them. For detailed instruction, you can see <a href=\"https://getcomposer.org/doc/01-basic-usage.md#installing-dependencies\">Composer's getting started guide</a>.<br /><br />If you have downloaded BoxBilling from <a href=\"https://github.com/boxbilling/boxbilling/releases\">GitHub releases</a>, this shouldn't happen.",
        103
    );
}
...
if (
    ...
    0 !== filesize($configPath) &&
    file_exists(BB_PATH_ROOT . "/install/index.php")
) {
    throw new Exception(
        "For safety reasons, you have to delete the <b><em>/install</em></b> directory to start using BoxBilling.</p><p>Please delete the <b><em>/install</em></b> directory from your web server.",
        102
    );
}
...
include_once BB_PATH_LIBRARY .
    "/Security/CSRF-Protector-PHP/libs/csrf/csrfprotector.php";
