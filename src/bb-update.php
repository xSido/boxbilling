...
class BBPatch_24 extends BBPatchAbstract
{
    public function patch()
    {
        ...
        $q =
            "DELETE FROM `pay_gateway` WHERE `gateway` = 'AlertPay' OR `gateway` = 'WebToPay' OR `gateway` = 'Payza' OR `gateway` = 'SinglePay'";
        $this->execSql($q);

        // Setting up the new custom pages extension
        $q =
            "CREATE TABLE IF NOT EXISTS `custom_pages` (`id` int(11) NOT NULL AUTO_INCREMENT, `title` varchar(255) NOT NULL, `description` varchar(555) NOT NULL, `keywords` varchar(555) NOT NULL, `content` text NOT NULL, `slug` varchar(255) NOT NULL, `created_at` timestamp NOT NULL DEFAULT current_timestamp(), PRIMARY KEY (`id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8";
        $this->execSql($q);
    }
}
...
class BBPatch_22 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `client_balance` CHANGE  `rel_id`  `rel_id` VARCHAR(20) NULL DEFAULT NULL;";
        $this->execSql($q);
    }
}
        ...
        foreach ($tables as $table => $fields) {
            foreach ($fields as $field) {
                try {
                    ...
                    error_log(
                        sprintf(
                            "Error changing table %s field %s type: %s ",
                            $table,
                            $field,
                            $e->getMessage()
                        )
...
class BBPatch_18 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE `promo` ADD  `client_groups` TEXT NULL AFTER  `products`;";
        $this->execSql($q);
    }
}
...
class BBPatch_16 extends BBPatchAbstract
{
    public function patch()
    {
        ...
        $q =
            "ALTER TABLE `product` ADD  `form_id` INT NULL AFTER  `product_payment_id`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE `client_order` ADD  `form_id` INT NULL AFTER  `product_id`;";
        $this->execSql($q);
    }
}
/**
...
class BBPatch_15 extends BBPatchAbstract
{
    public function patch()
    {
        ...
        $q =
            "ALTER TABLE `invoice` ADD  `gateway_id` INT NULL AFTER  `buyer_email`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `post` ADD  `image` VARCHAR( 255 ) NULL AFTER  `status`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `post` ADD  `section` VARCHAR( 255 ) NULL AFTER  `image`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `post` ADD  `publish_at` VARCHAR( 255 ) NULL AFTER `section`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `post` ADD  `published_at` VARCHAR( 255 ) NULL AFTER `publish_at`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `post` ADD  `expires_at` VARCHAR( 255 ) NULL AFTER `published_at`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `client` ADD  `email_approved` BOOLEAN NULL AFTER  `status`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `client_order` ADD  `form_id` INTEGER NULL AFTER  `product_id`;";
        $this->execSql($q);
    }
}
...
class BBPatch_14 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `service_license` ADD  `checked_at` VARCHAR( 35 ) DEFAULT NULL AFTER  `plugin`;";
        $this->execSql($q);
    }
}
...
class BBPatch_13 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `forum` ADD  `category` VARCHAR( 255 ) NOT NULL AFTER  `id`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `forum_topic_message` ADD  `points` VARCHAR( 255 ) NULL DEFAULT NULL AFTER  `ip`;";
        $this->execSql($q);

        $q =
            "UPDATE forum SET category = 'General purpose' WHERE category IS NULL OR category = '';";
        $this->execSql($q);
    }
}
...
class BBPatch_12 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `admin` ADD  `permissions` TEXT NULL DEFAULT NULL AFTER  `api_token`;";
        $this->execSql($q);
    }
}
...
class BBPatch_11 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `client_order` ADD  `referred_by` VARCHAR( 255 ) NULL AFTER  `config`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `client` ADD  `company_vat` VARCHAR( 255 ) NULL AFTER  `company`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `client` ADD  `company_number` VARCHAR( 255 ) NULL AFTER  `company_vat`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `client` ADD  `type` VARCHAR( 255 ) NULL AFTER  `tax_exempt`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice` ADD  `seller_company_vat` VARCHAR( 255 ) NULL AFTER  `seller_company`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice` ADD  `seller_company_number` VARCHAR( 255 ) NULL AFTER  `seller_company_vat`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice` ADD  `buyer_phone_cc` VARCHAR( 255 ) NULL AFTER  `buyer_phone`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice` ADD  `buyer_company_vat` VARCHAR( 255 ) NULL AFTER  `buyer_company`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice` ADD  `buyer_company_number` VARCHAR( 255 ) NULL AFTER  `buyer_company_vat`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice` ADD  `text_1` TEXT NULL DEFAULT NULL AFTER  `notes`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice` ADD  `text_2` TEXT NULL DEFAULT NULL AFTER  `text_1`;";
        $this->execSql($q);

        try {
            $this->di["api_admin"]->extension_activate([
                "id" => "queue",
                "type" => "mod",
            ]);
        } catch (Exception $e) {
...
class BBPatch_10 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `client` ADD  `auth_type` VARCHAR( 255 ) NULL AFTER  `role`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice_item` CHANGE  `rel_id`  `rel_id` TEXT NULL DEFAULT NULL;";
        $this->execSql($q);
    }
}
/**
...
class BBPatch_9 extends BBPatchAbstract
{
    ...
    {
        $q =
            "UPDATE currency SET format = REPLACE(format, '%price%' , '{{price}}' ) WHERE 1;";
        $this->execSql($q);
    }
}
...
class BBPatch_8 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `client` ADD  `salt` VARCHAR(255) NULL AFTER  `pass`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `admin` ADD  `salt` VARCHAR(255) NULL AFTER  `pass`;";
        $this->execSql($q);
    }
}
...
class BBPatch_7 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `email_template` ADD  `description` TEXT NULL AFTER  `content`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `service_domain` ADD  `synced_at` VARCHAR( 255 ) NULL AFTER  `details`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `service_license` ADD  `pinged_at` VARCHAR( 255 ) NULL AFTER  `plugin`;";
        $this->execSql($q);

        $q =
            "UPDATE email_template SET category = 'DEPRECATED - NOT USED ANY MORE' WHERE 1;";
        $this->execSql($q);
        ...
        try {
            ...
            $config = [
                ...
                "smtp_username" => $api->system_param([
                    "key" => "smtp_username",
                ]),
                "smtp_password" => $api->system_param([
                    "key" => "smtp_password",
                ]),
                "smtp_security" => $api->system_param([
                    "key" => "smtp_security",
                ]),
                "sendgrid_username" => $api->system_param([
                    "key" => "sendgrid_username",
                ]),
                "sendgrid_password" => $api->system_param([
...
class BBPatch_6 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `transaction` CHANGE  `amount`  `amount` VARCHAR( 255 ) NULL DEFAULT NULL;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `currency` ADD  `price_format` VARCHAR( 50 ) NOT NULL DEFAULT  '1' AFTER  `format`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `email_template` ADD  `vars` TEXT NOT NULL AFTER  `content`;";
        $this->execSql($q);
        ...
        try {
            ...
            $pubkey = $api->system_param([
                "key" => "captcha_recaptcha_publickey",
            ]);
            $privkey = $api->system_param([
                "key" => "captcha_recaptcha_privatekey",
            ]);
            $config = [
                "ext" => "mod_spamchecker",
...
class BBPatch_5 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `extension_meta` ADD  `client_id` INT NULL DEFAULT NULL AFTER  `id`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `product` ADD  `plugin_config` TEXT NULL DEFAULT NULL AFTER  `plugin`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `service_custom` ADD  `plugin_config` TEXT NULL DEFAULT NULL AFTER  `plugin`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `invoice` ADD  `currency_rate` DECIMAL( 13, 6 ) NULL DEFAULT NULL AFTER  `currency`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `product_payment` ADD `w_price` decimal(18,2) DEFAULT '0.00' AFTER  `once_setup_price`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `product_payment` ADD `w_setup_price` decimal(18,2) DEFAULT '0.00' AFTER  `tria_price`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `product_payment` ADD `w_enabled` tinyint(1) DEFAULT '1' AFTER  `tria_setup_price`;";
        $this->execSql($q);
        ...
        $q =
            "ALTER TABLE  `service_custom` ADD  `f1` TEXT NULL AFTER  `config`;";
        $this->execSql($q);
        ...
        $q =
            "ALTER TABLE  `extension_meta` CHANGE  `rel_id`  `rel_id` VARCHAR( 255 ) NULL DEFAULT NULL;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `client_order` ADD  `invoice_option` VARCHAR( 255 ) NULL AFTER  `group_master`;";
        $this->execSql($q);

        $q =
            "UPDATE `client_order` SET `invoice_option` = 'issue-invoice' WHERE 1;";
        $this->execSql($q);
    }
}
...
class BBPatch_4 extends BBPatchAbstract
{
    ...
    {
        $q =
            "INSERT INTO `email_template` (`action_code`, `category`, `enabled`, `subject`, `content`) VALUES ('support_ticket_admin_close', 'support', 1, 'Support Ticket Closed', '<p>Ticket ID: #{{ticket.id}}<br/> closed</p>\n<hr/>\n<p>{{company.signature}}</p>');";
        $this->execSql($q);

        $q =
            "INSERT INTO `email_template` (`action_code`, `category`, `enabled`, `subject`, `content`) VALUES ('staff_ticket_client_close', 'staff', 1, 'Support Ticket Closed', '<p>Ticket ID: #{{ticket.id}}<br/> closed</p>\n<hr/>\n<p>{{company.signature}}</p>');";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `client_order` ADD  `promo_recurring` tinyint(1) NULL AFTER  `promo_id`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `client_order` ADD  `promo_used` int NULL AFTER  `promo_recurring`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `promo` ADD  `once_per_client` tinyint(1) NULL AFTER  `freesetup`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `promo` ADD  `recurring` tinyint(1) AFTER  `once_per_client`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `service_license` ADD  `config` TEXT NULL AFTER  `versions`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `extension` ADD  `manifest` TEXT NULL AFTER  `version`;";
        $this->execSql($q);
...
class BBPatch_2 extends BBPatchAbstract
{
    ...
    {
        $q =
            "ALTER TABLE  `client_order` CHANGE  `group_id`  `group_id` VARCHAR( 255 ) NULL DEFAULT NULL;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `transaction` ADD  `output` TEXT NOT NULL AFTER  `ipn`";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `extension` CHANGE  `version`  `version` VARCHAR( 100 ) NULL DEFAULT NULL;";
        $this->execSql($q);
        ...
        $q =
            "ALTER TABLE  `setting` ADD `category` VARCHAR( 255 ) NULL AFTER `public`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `service_hosting_server` ADD  `port` VARCHAR( 20 ) NOT NULL AFTER  `accesshash`;";
        $this->execSql($q);

        $q =
            "ALTER TABLE  `service_hosting_server` ADD  `config` TEXT NOT NULL AFTER  `port`;";
        $this->execSql($q);
    }
}
...
abstract class BBPatchAbstract
{
    ...
    private function setParamValue($param, $value)
    {
        if (is_null($this->getParamValue($param))) {
            $query =
                "INSERT INTO setting (param, value, public, updated_at, created_at) VALUES (:param, :value, 1, :u, :c)";
            ...
            $query =
                "UPDATE setting SET value = :value, updated_at = :u WHERE param = :param";
            $stmt = $this->pdo->prepare($query);
            $stmt->execute([
                "param" => $param,
                "value" => $value,
...
print "Update completed. You are using BoxBilling " .
    Box_Version::VERSION .
    PHP_EOL;
