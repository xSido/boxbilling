    ...
    if (hasOptions) {
      ...
      if (typeof options.ranges == "object") {
        for (var range in options.ranges) {
          ...
          // range option.
          if (
            (this.minDate && end < this.minDate) ||
            (this.maxDate && start > this.maxDate)
          ) {
            continue;
          }
  ...
  DateRangePicker.prototype = {
    ...
    notify: function () {
      ...
      if (this.element.is("input")) {
        this.element.val(
          this.cleared
            ? ""
            : this.startDate.toString(this.format) +
                this.separator +
    ...
    updateCalendars: function () {
      this.leftCalendar.calendar = this.buildCalendar(
        this.leftCalendar.month.getMonth(),
        this.leftCalendar.month.getFullYear()
      );
      this.rightCalendar.calendar = this.buildCalendar(
        this.rightCalendar.month.getMonth(),
        this.rightCalendar.month.getFullYear()
      );
      this.container
        .find(".calendar.left")
        .html(
      ...
      for (var range in this.ranges) {
        if (
          this.startDate.equals(this.ranges[range][0]) &&
          this.endDate.equals(this.ranges[range][1])
        ) {
          customRange = false;
    ...
    buildCalendar: function (month, year) {
      ...
      for (
        var i = 0, col = 0, row = 0;
        i < 42;
        i++, col++, curDate = curDate.clone().add(1).day()
      ) {
        if (i > 0 && col % 7 == 0) {
          col = 0;
          row++;
    ...
    renderDropdowns: function (selected, minDate, maxDate) {
      ...
      for (var m = 0; m < 12; m++) {
        if (
          (!inMinYear || m >= minDate.getMonth()) &&
          (!inMaxYear || m <= maxDate.getMonth())
        ) {
          monthHtml +=
            "<option value='" +
            m +
    ...
    renderCalendar: function (calendar, selected, minDate, maxDate) {
      ...
      var dateHtml =
        this.locale.monthNames[calendar[1][1].getMonth()] +
        calendar[1][1].toString(" yyyy");
      ...
      for (var row = 0; row < 6; row++) {
        ...
        for (var col = 0; col < 7; col++) {
          ...
          if (
            (minDate && calendar[row][col] < minDate) ||
            (maxDate && calendar[row][col] > maxDate)
          ) {
            cname = " off disabled ";
