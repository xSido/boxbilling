...
if (
  ...
  elFinder.prototype.i18.jp = {
    ...
    messages: {
      ...
      errJqui:
        "無効なjQuery UI コンフィグレーションです。セレクタブルコンポーネント、ドラッガブルコンポーネント、ドロッパブルコンポーネントがあるかを確認して下さい",
      ...
      errPerm: "権限がありません",
      errLocked:
        '"$1" はロックされているので名前の変更、移動、削除ができません',
      ...
      errPopup:
        "ポップアップウィンドウが開けません。ファイルを開くにはブラウザの設定を変更してください",
      ...
      errNotReplace:
        'アイテム "$1" は、すでにこの場所にありますがアイテムのタイプが違うので置き換えることはできません', // new
      ...
      errReplByChild:
        'ホルダ "$1" に含まれてるアイテムを置き換えることはできません',
      errArcSymlinks:
        "シンボリックリンクを含むアーカイブはセキュリティ上、解凍できません",
      errArcMaxSize: "アーカイブが許容されたサイズを超えています",
      errResize: '"$1"をリサイズできません',
      errUsupportType: "このファイルタイプはサポートされません",
      errNotUTF8Content:
        'ファイル "$1" には UTF-8 以外の文字が含まれているので編集できません', // added 9.11.2011
