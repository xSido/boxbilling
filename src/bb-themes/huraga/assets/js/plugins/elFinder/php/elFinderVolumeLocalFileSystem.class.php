...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver
{
        ...
        if ($this->options["quarantine"]) {
            $this->attributes[] = [
                "pattern" =>
                    "~^" .
                    preg_quote(
                        DIRECTORY_SEPARATOR . $this->options["quarantine"]
                    ) .
                    '$~',
                "read" => false,
                "write" => false,
            ...
            if (strpos($this->tmbPath, $this->root) === 0) {
                $this->tmbURL =
                    ...
                    str_replace(
                        DIRECTORY_SEPARATOR,
                        "/",
                        substr($this->tmbPath, strlen($this->root) + 1)
                    );
    ...
    protected function _path($path)
    {
        return $this->rootName .
            ($path == $this->root
                ? ""
                : $this->separator . $this->_relpath($path));
    }
    ...
    {
        return $path == $parent ||
            strpos($path, $parent . DIRECTORY_SEPARATOR) === 0;
    }
    ...
    protected function readlink($path)
    {
        ...
        if ($this->_inpath($atarget, $this->aroot)) {
            return $this->_normpath(
                $this->root .
                    DIRECTORY_SEPARATOR .
                    substr($atarget, strlen($this->aroot) + 1)
            );
        }
    ...
    protected function _checkArchivers()
    {
        ...
        if ($ctar == 0) {
            $arcs["create"]["application/x-tar"] = [
                "cmd" => "tar",
                "argc" => "-cf",
                "ext" => "tar",
            ];
            $arcs["extract"]["application/x-tar"] = [
                "cmd" => "tar",
                "argc" => "-xf",
                "ext" => "tar",
            ];
            ...
            if ($c == 0) {
                $arcs["create"]["application/x-gzip"] = [
                    "cmd" => "tar",
                    "argc" => "-czf",
                    "ext" => "tgz",
                ];
            ...
            if ($c == 0) {
                $arcs["create"]["application/x-bzip2"] = [
                    "cmd" => "tar",
                    "argc" => "-cjf",
                    "ext" => "tbz",
                ];
                $arcs["extract"]["application/x-bzip2"] = [
                    "cmd" => "tar",
                    "argc" => "-xjf",
                    "ext" => "tbz",
                ];
        ...
        if ($c == 0 || $c == 7) {
            $arcs["create"]["application/x-rar"] = [
                "cmd" => "rar",
                "argc" => "a -inul",
                "ext" => "rar",
            ];
            $arcs["extract"]["application/x-rar"] = [
                "cmd" => "rar",
                "argc" => "x -y",
                "ext" => "rar",
            ];
        ...
        if ($c == 0) {
            $arcs["create"]["application/x-7z-compressed"] = [
                "cmd" => "7za",
                "argc" => "a",
                "ext" => "7z",
            ];
            $arcs["extract"]["application/x-7z-compressed"] = [
                "cmd" => "7za",
                "argc" => "e -y",
                "ext" => "7z",
            ];
    ...
    protected function _unpack($path, $arc)
    {
        ...
        $cmd =
            ...
            $arc["argc"] .
            " " .
            escapeshellarg($this->_basename($path));
    ...
    protected function _extract($path, $arc)
    {
        if ($this->quarantine) {
            ...
            if (
                ...
                $this->options["maxArcFilesSize"] < $this->archiveSize
            ) {
