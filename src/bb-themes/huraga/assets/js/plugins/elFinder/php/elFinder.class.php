...
class elFinder
{
    ...
    public function __construct($opts)
    {
        ...
        setlocale(
            LC_ALL,
            !empty($opts["locale"]) ? $opts["locale"] : "en_US.UTF-8"
        );
            ...
            $result["debug"] = [
                ...
                "memory" =>
                    (function_exists("memory_get_peak_usage")
                        ? ceil(memory_get_peak_usage() / 1024) . "Kb / "
                        : "") .
    ...
    protected function file($args)
    {
        ...
        if ($download) {
            ...
            $disp =
                preg_match("/^(image|text)/i", $file["mime"]) ||
    ...
    protected function upload($args)
    {
        ...
        $volume = $this->volume($target);
        $files =
            isset($args["FILES"]["upload"]) &&
            is_array($args["FILES"]["upload"])
                ? $args["FILES"]["upload"]
                : [];
        $result = [
            "added" => [],
            "header" => empty($args["html"])
                ? false
            ...
            if (($error = $files["error"][$i]) > 0) {
                $result["warning"] = $this->error(
                    ...
                    $error == UPLOAD_ERR_FORM_SIZE
                        ? self::ERROR_UPLOAD_FILE_SIZE
                        : self::ERROR_UPLOAD_TRANSFER
                );
    ...
    protected function extract($args)
    {
        $target = $args["target"];
        $mimes =
            !empty($args["mimes"]) && is_array($args["mimes"])
                ? $args["mimes"]
                : [];
    ...
    protected function archive($args)
    {
        $type = $args["type"];
        $targets =
            isset($args["targets"]) && is_array($args["targets"])
                ? $args["targets"]
                : [];
