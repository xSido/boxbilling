...
// define('ELFINDER_DISABLE_ONLINE_CONVERT', false); // set `true` to disable Online converter
// ===============================================
        ...
        if (in_array($key, ["error", "warning"])) {
            ...
            if (is_array($value)) {
                ...
                foreach ($value as $file) {
                    $filepath = isset($file["realpath"])
                        ? $file["realpath"]
                        : $elfinder->realpath($file["hash"]);
                    array_push($data, $filepath);
                }
            } else {
                // other value (ex. header)
    ...
    public function log($cmd, $result, $args, $elfinder)
    {
        ...
        if (!empty($result["changed"])) {
            foreach ($result["changed"] as $file) {
                $log .=
                    "\tCHANGED: " . $elfinder->realpath($file["hash"]) . "\n";
            }
        }
...
$opts = [
    ...
    "roots" => [
        ...
        [
            ...
            "path" => "../files/.trash/",
            "tmbURL" =>
                dirname($_SERVER["PHP_SELF"]) . "/../files/.trash/.tmb/",
            "winHashFix" => DIRECTORY_SEPARATOR !== "/", // to make hash same to Linux one on windows too
