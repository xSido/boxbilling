...
class elFinderVolumeMySQL extends elFinderVolumeDriver
{
    ...
    {
        if (
            !($this->options["host"] || $this->options["socket"]) ||
            !$this->options["user"] ||
            !$this->options["pass"] ||
            !$this->options["db"] ||
            !$this->options["path"] ||
    ...
    {
        $sql =
            'INSERT INTO %s (`parent_id`, `name`, `size`, `mtime`, `mime`, `content`, `read`, `write`) VALUES ("%s", "%s", 0, %d, "%s", "", "%d", "%d")';
        $sql = sprintf(
            $sql,
            $this->tbf,
            $path,
    ...
    public function resize(
        ...
        if ($result) {
            $sql = sprintf(
                'UPDATE %s SET content=LOAD_FILE("%s"), mtime=UNIX_TIMESTAMP() WHERE id=%d',
                $this->tbf,
                $this->loadFilePath($img),
                $path
            );
            ...
            if (!$this->query($sql)) {
                ...
                $sql = sprintf(
                    'UPDATE %s SET content="%s", mtime=UNIX_TIMESTAMP() WHERE id=%d',
                    $this->tbf,
    ...
    protected function cacheDir($path)
    {
        ...
        $sql =
            ...
            ' AS f 
				LEFT JOIN ' .
            $this->tbf .
            ' AS ch ON ch.parent_id=f.id AND ch.mime="directory"
				WHERE f.parent_id="' .
            $path .
            '"
				GROUP BY f.id';
    ...
    protected function getParents($path)
    {
        ...
        while ($path) {
            if ($file = $this->stat($path)) {
                ...
                $path = isset($file["phash"])
                    ? $this->decode($file["phash"])
                    : false;
            }
        }
    ...
    {
        return ($stat = $this->stat($path)) && isset($stat["dirs"])
            ? $stat["dirs"]
            : false;
    }
    ...
    protected function _copy($source, $targetDir, $name)
    {
        ...
        $sql =
            $id > 0
                ? sprintf(
                    "REPLACE INTO %s (id, parent_id, name, content, size, mtime, mime, width, height, `read`, `write`, `locked`, `hidden`) (SELECT %d, %d, name, content, size, mtime, mime, width, height, `read`, `write`, `locked`, `hidden` FROM %s WHERE id=%d)",
                    $this->tbf,
                    $id,
                    $this->_dirname($id),
                    $this->tbf,
                ...
                : sprintf(
                    'INSERT INTO %s (parent_id, name, content, size, mtime, mime, width, height, `read`, `write`, `locked`, `hidden`) SELECT %d, "%s", content, size, %d, mime, width, height, `read`, `write`, `locked`, `hidden` FROM %s WHERE id=%d',
                    $this->tbf,
                    $targetDir,
                    $this->db->real_escape_string($name),
                    time(),
    ...
    protected function _unlink($path)
    {
        return $this->query(
            sprintf(
                'DELETE FROM %s WHERE id=%d AND mime!="directory" LIMIT 1',
                $this->tbf,
                $path
            )
    ...
    protected function _save($fp, $dir, $name, $mime, $w, $h)
    {
        ...
        if ($tmpfile = tempnam($this->tmpPath, $this->id)) {
            if (($trgfp = fopen($tmpfile, "wb")) == false) {
                ...
                $sql =
                    $id > 0
                        ? "REPLACE INTO %s (id, parent_id, name, content, size, mtime, mime, width, height) VALUES (" .
                            $id .
                            ', %d, "%s", LOAD_FILE("%s"), %d, %d, "%s", %d, %d)'
    ...
    protected function _getContents($path)
    {
        return ($res = $this->query(
            sprintf("SELECT content FROM %s WHERE id=%d", $this->tbf, $path)
        )) && ($r = $res->fetch_assoc())
            ? $r["content"]
            : false;
    }
    ...
    protected function _filePutContents($path, $content)
    {
        return $this->query(
            sprintf(
                'UPDATE %s SET content="%s", size=%d, mtime=%d WHERE id=%d LIMIT 1',
                $this->tbf,
                $this->db->real_escape_string($content),
                strlen($content),
                time(),
