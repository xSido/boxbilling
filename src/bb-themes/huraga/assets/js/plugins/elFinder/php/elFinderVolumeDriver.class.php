...
abstract class elFinderVolumeDriver
{
    ...
    public function debug()
    {
        return [
            "id" => $this->id(),
            "name" => strtolower(
                substr(get_class($this), strlen("elfinderdriver"))
            ),
    ...
    public function mount(array $opts)
    {
        ...
        $this->id =
            $this->driverId .
            (!empty($this->options["id"])
                ? $this->options["id"]
                : elFinder::$volumesCnt++) .
            "_";
        $this->root = $this->_normpath($this->options["path"]);
        $this->separator = isset($this->options["separator"])
            ? $this->options["separator"]
            : DIRECTORY_SEPARATOR;

        // default file attribute
        $this->defaults = [
            "read" => isset($this->options["defaults"]["read"])
                ? !!$this->options["defaults"]["read"]
                : true,
            "write" => isset($this->options["defaults"]["write"])
                ? !!$this->options["defaults"]["write"]
                : true,
            "locked" => false,
            "hidden" => false,
        ];
        ...
        if (
            !empty($this->options["attributes"]) &&
            is_array($this->options["attributes"])
        ) {
        ...
        if (!empty($this->options["accessControl"])) {
            if (
                is_string($this->options["accessControl"]) &&
                function_exists($this->options["accessControl"])
            ) {
                $this->access = $this->options["accessControl"];
            } elseif (
                is_array($this->options["accessControl"]) &&
        ...
        $this->uploadAllow =
            isset($this->options["uploadAllow"]) &&
            is_array($this->options["uploadAllow"])
                ? $this->options["uploadAllow"]
                : [];

        $this->uploadDeny =
            isset($this->options["uploadDeny"]) &&
            is_array($this->options["uploadDeny"])
        ...
        if (is_string($this->options["uploadOrder"])) {
            ...
            $parts = explode(
                ",",
                isset($this->options["uploadOrder"])
                    ? $this->options["uploadOrder"]
                    : "deny,allow"
            );
        ...
        $this->disabled =
            isset($this->options["disabled"]) &&
            is_array($this->options["disabled"])
        ...
        $type = preg_match(
            '/^(finfo|mime_content_type|internal|auto)$/i',
            $type
        )
            ? $type
            : "auto";
        ...
        if (
            ...
            preg_match(
                ...
                array_shift(
                    explode(
                        ";",
                        @finfo_file(finfo_open(FILEINFO_MIME), __FILE__)
                    )
                )
            )
        ) {
            ...
            ($type == "mime_content_type" || $type == "auto") &&
            function_exists("mime_content_type") &&
            preg_match(
                $regexp,
                array_shift(explode(";", mime_content_type(__FILE__)))
            )
        ) {
        ...
        if ($this->mimeDetect == "internal" && !self::$mimetypesLoaded) {
            ...
            $file = false;
            if (
                !empty($this->options["mimefile"]) &&
                file_exists($this->options["mimefile"])
            ) {
                $file = $this->options["mimefile"];
            } elseif (
                file_exists(
                    dirname(__FILE__) . DIRECTORY_SEPARATOR . "mime.types"
                )
            ) {
        ...
        $this->rootName = empty($this->options["alias"])
        ...
        $this->treeDeep =
            $this->options["treeDeep"] > 0
                ? (int) $this->options["treeDeep"]
                : 1;
        $this->tmbSize =
            $this->options["tmbSize"] > 0
                ? (int) $this->options["tmbSize"]
                : 48;
        ...
        $this->tmbURL = !empty($this->options["tmbURL"])
            ? $this->options["tmbURL"]
            : "";
        ...
        $this->nameValidator =
            is_string($this->options["acceptedName"]) &&
            !empty($this->options["acceptedName"])
        ...
        if (
            !empty($this->options["archiveMimes"]) &&
            is_array($this->options["archiveMimes"])
        ) {
        ...
        if (
            !empty($this->options["archivers"]["create"]) &&
            is_array($this->options["archivers"]["create"])
        ) {
        ...
        if (
            !empty($this->options["archivers"]["extract"]) &&
            is_array($this->options["archivers"]["extract"])
        ) {
            foreach ($this->options["archivers"]["extract"] as $mime => $conf) {
                if (
                    substr($mime, "application/") === 0 &&
                    !empty($cons["cmd"]) &&
                    isset($conf["argc"]) &&
                    !empty($conf["ext"]) &&
     ...
     * @author Troex Nevelin
     **/
    public function mimeAccepted($mime, $mimes = [], $empty = true)
    {
    ...
    {
        return ($path = $this->closestByAttr($this->decode($hash), $attr, $val))
            ? $this->encode($path)
            : false;
    }
    ...
    public function mkdir($dst, $name)
    {
        ...
        if (($dir = $this->dir($dst)) == false) {
            return $this->setError(
                elFinder::ERROR_TRGDIR_NOT_FOUND,
                "#" . $dst
            );
        }
        ...
        $this->clearcache();
        return ($path = $this->_mkdir($path, $name))
            ? $this->stat($path)
            : false;
    }
    ...
    public function mkfile($dst, $name)
    {
        ...
        $this->clearcache();
        return ($path = $this->_mkfile($path, $name))
            ? $this->stat($path)
            : false;
    }
    ...
    public function upload($fp, $dst, $name, $tmpname)
    {
        ...
        $mime = $this->mimetype(
            $this->mimeDetect == "internal" ? $name : $tmpname
        );
        ...
        if (
            ...
            filesize($tmpname) > $this->uploadMaxSize
        ) {
    ...
    public function paste($volume, $src, $dst, $rmSrc = false)
    {
        ...
        if (
            $test = $volume->closest($src, $rmSrc ? "locked" : "read", $rmSrc)
        ) {
            return $rmSrc
                ? $this->setError(
                    $err,
                    $errpath,
                    elFinder::ERROR_LOCKED,
                    $volume->path($test)
                )
        ...
        if ($volume == $this) {
            ...
            return ($path = $this->$method($source, $destination, $name))
                ? $this->stat($path)
                : false;
        }
        ...
        if (
            ($path = $this->copyFrom($volume, $src, $destination, $name)) ==
            false
        ) {
            return false;
        }
    ...
    public function extract($hash)
    {
        ...
        $this->clearcache();
        return ($path = $this->_extract($path, $archiver))
            ? $this->stat($path)
            : false;
    }
    ...
    public function archive($hashes, $mime)
    {
        ...
        $name =
            (count($files) == 1 ? $files[0] : "Archive") .
            "." .
            $archiver["ext"];
        $name = $this->uniqueName($dir, $name, "");
        $this->clearcache();
        return ($path = $this->_archive($dir, $files, $name, $archiver))
            ? $this->stat($path)
            : false;
    }
    ...
    public function resize(
        ...
        switch ($mode) {
            ...
            case "fitsquare":
                $result = $this->imgSquareFit(
                    $path,
                    $width,
                    $height,
                    "center",
                    "middle",
                    $bg ? $bg : $this->options["tmbBgColor"]
                );
                break;

            case "rotate":
                $result = $this->imgRotate(
                    $path,
                    $degree,
                    $bg ? $bg : $this->options["tmbBgColor"]
                );
    ...
    public function uniqueName($dir, $name, $suffix = " copy", $checkNum = true)
    {
        ...
        if (
            preg_match(
                '/\.((tar\.(gz|bz|bz2|z|lzo))|cpio\.gz|ps\.gz|xcf\.(gz|bz2)|[a-z0-9]{1,4})$/i',
                $name,
                $m
            )
        ) {
        ...
        if ($this->access) {
            if (is_array($this->access)) {
                ...
                $perm = $obj->{$method}(
                    $name,
                    $path,
                    $this->options["accessControlData"],
                    $this
                );
            } else {
                $func = $this->access;
    ...
    protected function updateCache($path, $stat)
    {
        ...
        if (
            $this->options["utf8fix"] &&
            $this->options["utf8patterns"] &&
            $this->options["utf8replace"]
        ) {
            $stat["name"] = json_decode(
                str_replace(
                    $this->options["utf8patterns"],
                    $this->options["utf8replace"],
                    json_encode($stat["name"])
                )
            );
        }
        ...
        $stat["read"] = intval(
            $this->attr(
                $path,
                "read",
                isset($stat["read"]) ? !!$stat["read"] : false
            )
        );
        $stat["write"] = intval(
            $this->attr(
                $path,
                "write",
                isset($stat["write"]) ? !!$stat["write"] : false
            )
        );
        if ($root) {
            $stat["locked"] = 1;
        ...
        if ($root) {
            ...
            $this->attr($path, "hidden", !empty($stat["hidden"])) ||
                ...
                if ($this->options["checkSubfolders"]) {
                    if (isset($stat["dirs"])) {
                        ...
                        $stat["dirs"] = isset($this->cache[$stat["target"]])
                            ? intval(
                                isset($this->cache[$stat["target"]]["dirs"])
                            )
    ...
    {
        $pinfo = pathinfo($path);
        $ext = isset($pinfo["extension"])
            ? strtolower($pinfo["extension"])
            : "";
        return isset(elFinderVolumeDriver::$mimetypes[$ext])
            ? elFinderVolumeDriver::$mimetypes[$ext]
            : "unknown";
    }
    ...
    {
        return ($mime1 == "directory" && $mime1 == $mime2) ||
        ...
        if ($srcStat["mime"] == "directory") {
            ...
            foreach ($this->getScandir($src) as $stat) {
                if (empty($stat["hidden"])) {
                    $name = $stat["name"];
                    if (
                        !$this->copy($this->_joinPath($src, $name), $dst, $name)
                    ) {
                        return false;
                    }
                }
    ...
    protected function copyFrom($volume, $src, $destination, $name)
    {
        ...
        if ($source["mime"] == "directory") {
            ...
            $this->clearcache();
            if (
                (!$stat || $stat["mime"] != "directory") &&
                !$this->_mkdir($destination, $name)
            ) {
     ...
     * @author Alexey Sukhotin
     **/
    protected function imgResize(
        $path,
        $width,
        $height,
        $keepProportions = false,
        $resizeByBiggerSide = true,
        $destformat = null
    ) {
        ...
        switch ($this->imgLib) {
            ...
            case "gd":
                ...
                if (
                    $img &&
                    false != ($tmp = imagecreatetruecolor($size_w, $size_h))
                ) {
                    if (
                        !imagecopyresampled(
                            $tmp,
    ...
    protected function imgCrop(
        ...
        switch ($this->imgLib) {
            ...
            case "gd":
                ...
                if (
                    $img &&
                    false != ($tmp = imagecreatetruecolor($width, $height))
                ) {
     ...
     * @author Alexey Sukhotin
     **/
    protected function imgSquareFit(
        $path,
        $width,
        $height,
        $align = "center",
        ...
        switch ($this->imgLib) {
            case "imagick":
                ...
                $img1->setImageFormat(
                    $destformat != null ? $destformat : $img->getFormat()
                );
     ...
     * @author Alexey Sukhotin
     **/
    protected function procExec(
        $command,
        array &$output = null,
        &$return_var = -1,
        array &$error_output = null
    ) {
        $descriptorspec = [
            0 => ["pipe", "r"], // stdin
            1 => ["pipe", "w"], // stdout
