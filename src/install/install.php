...
set_include_path(
    implode(PATH_SEPARATOR, [BB_PATH_LIBRARY, get_include_path()])
);
...
final class Box_Installer
{
        ...
        switch ($action) {
            ...
            case "install":
                try {
                    ...
                    $name = $_POST["db_name"];
                    if (
                        !$this->canConnectToDatabase($host, $name, $user, $pass)
                    ) {
                        throw new Exception(
                            "Could not connect to the database, or the database does not exist"
                        );
                    } else {
                        $this->session->set("db_host", $host);
                        $this->session->set("db_name", $name);
                    ...
                    if (
                        ...
                        throw new Exception(
                            'Administrator\'s account is not valid'
                        );
                    } else {
    ...
    private function getLicense()
    {
        ...
        if (!file_exists($path)) {
            return "BoxBilling is licensed under the Apache License, Version 2.0." .
                PHP_EOL .
                "Please visit https://github.com/boxbilling/boxbilling/blob/master/LICENSE for full license text.";
        }
    ...
    private function makeInstall($ns)
    {
        ...
        $stmt = $pdo->prepare(
            "INSERT INTO admin (role, name, email, pass, protected, created_at, updated_at) VALUES('admin', :admin_name, :admin_email, :admin_pass, 1, NOW(), NOW());"
        );
        $stmt->execute([
    ...
    private function _sendMail($ns)
    {
        ...
        $content .=
            "Read BoxBilling documentation to get started https://docs.boxbilling.com/" .
            PHP_EOL;
    ...
    private function _createConfigurationFile($data)
    {
        ...
        if (!@file_put_contents(BB_PATH_CONFIG, $output)) {
            throw new Exception(
                "Configuration file is not writable or does not exist. Please create the file at " .
                    BB_PATH_CONFIG .
