...
$di["logger"] = function () use ($di) {
    ...
    $log_to_db =
        isset($di["config"]["log_to_db"]) && $di["config"]["log_to_db"];
    if ($log_to_db) {
...
$di["pdo"] = function () use ($di) {
    ...
    $pdo = new PDO(
        $c["type"] . ":host=" . $c["host"] . ";dbname=" . $c["name"],
        $c["user"],
        $c["password"],
        [
...
$di["db"] = function () use ($di) {
    ...
    $freeze = isset($di["config"]["db"]["freeze"])
        ? (bool) $di["config"]["db"]["freeze"]
        : true;
...
$di["geoip"] = function () use ($di) {
    return new \GeoIp2\Database\Reader(
        BB_PATH_LIBRARY . "/GeoLite2-Country.mmdb"
    );
};
