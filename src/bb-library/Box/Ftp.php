...
class Box_Ftp
{
    ...
    public function __construct($opt = [])
    {
        ...
        if (!extension_loaded("ftp")) {
            throw new \Box_Exception(
                "FTP extension is not enabled in PHP. You will not be able to use automatic updates and other automation."
            );
        }
    ...
    public function connect()
    {
        ...
        if (!$this->link) {
            throw new \Box_Exception(
                "Failed to connect to FTP Server :host : :port",
                [
        ...
        if (
            ...
            throw new \Box_Exception(
                "FTP Username/Password incorrect for :usernmae",
    ...
    public function delete($file, $recursive = false)
    {
        ...
        if (!empty($filelist)) {
            foreach ($filelist as $delete_file) {
                $this->delete(
                    $this->trailingslashit($file) . $delete_file["name"],
                    $recursive
                );
            };
        }
    ...
    private function parselisting($line)
    {
        ...
        if (is_null($is_windows)) {
            $is_windows =
                strpos(strtolower(ftp_systype($this->link)), "win") !== false;
        }
            ...
            if ("d" == $struc["type"]) {
                ...
                if ($incdot) {
                    ...
                    if ("." != $struc["name"] && ".." != $struc["name"]) {
                        //Ok, It isnt a special folder
                        if ($recursive) {
                            $struc["files"] = $this->dirlist(
                                $path . "/" . $struc["name"],
                                $incdot,
                                $recursive
                            );
