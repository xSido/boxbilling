...
class Payment_Adapter_TwoCheckout implements \Box\InjectionAwareInterface
{
    ...
    public function __construct($config)
    {
        if (!$config["vendor_nr"]) {
            throw new Payment_Exception(
                'Payment gateway "2Checkout" is not configured properly. Please update configuration parameter "Vendor account number" at "Configuration -> Payments".'
            );
        }

        if (!$config["secret"]) {
            throw new Payment_Exception(
                'Payment gateway "2Checkout" is not configured properly. Please update configuration parameter "Secret word" at "Configuration -> Payments".'
            );
        }
    ...
    public static function getConfig()
    {
        return [
            ...
            "description" =>
                'Allows to start accepting payments by 2Checkout. Redirect option must be configured propertly. Login to your 2checkout account and navigate to <i>Account -> Site Management -> Direct Return</i> section. Select <i>Header Redirect (Your URL)</i> and click "Save changes".',
            "form" => [
                "vendor_nr" => [
                    "text",
                    [
                        "label" => "2CO Account #",
                        "description" =>
                            "2Checkout account number is number with which you login to 2CO account",
                    ],
                ],
                "secret" => [
                    "password",
    ...
    public function processTransaction($api_admin, $id, $data, $gateway_id)
    {
        ...
        if (!$invoice_id) {
            throw new Payment_Exception(
                "Invoice id could not be determined for this transaction"
            );
        }
        ...
        if (
            isset($ipn["credit_card_processed"]) &&
            $ipn["credit_card_processed"] == "Y"
        ) {
            $bd = [
                "id" => $client_id,
                "amount" => $ipn["total"],
            ...
            $api_admin->invoice_batch_pay_with_credits([
                "client_id" => $client_id,
            ]);
        }
    ...
    private function _isIpnValid($ipn)
    {
        ...
        if (
            isset($ipn["order_number"]) &&
            isset($ipn["total"]) &&
            isset($ipn["key"])
        ) {
            $md5_hash = $ipn["key"];
            $check_key = strtoupper(
                md5(
                    $secret .
                        $vendorNumber .
                        $ipn["order_number"] .
            ...
            $md5_hash = $ipn["key"];
            $check_key = strtoupper(
                md5($secret . $ipn["order_number"] . $ipn["total"])
            );
        }

        error_log(
            sprintf(
                "Returned MD5 Hash %s should be equal to %s",
                $md5_hash,
                $check_key
            )
        );
    ...
    private function _generateForm($url, $data, $method = "post")
    {
        ...
        $form .=
            '<input class="bb-button bb-button-submit" type="submit" value="Please click here to continue if this page does not redirect automatically in 5 seconds" id="payment_button"/>' .
            PHP_EOL;
        $form .= "</form>" . PHP_EOL . PHP_EOL;

        if (
            isset($this->config["auto_redirect"]) &&
            $this->config["auto_redirect"]
        ) {
            $form .= sprintf("<h2>%s</h2>", __("Redirecting to 2Checkout.com"));
            $form .=
                "<script type='text/javascript'>$(document).ready(function(){    document.getElementById('payment_button').style.display = 'none';    document.forms['payment_form'].submit();});</script>";
        }
