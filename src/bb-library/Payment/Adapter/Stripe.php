...
class Payment_Adapter_Stripe implements \Box\InjectionAwareInterface
{
    ...
    public function __construct($config)
    {
        ...
        if (!isset($this->config["api_key"])) {
            throw new Payment_Exception(
                'Payment gateway "Stripe" is not configured properly. Please update configuration parameter "api_key" at "Configuration -> Payments".'
            );
        }

        if (!isset($this->config["pub_key"])) {
            throw new Payment_Exception(
                'Payment gateway "Stripe" is not configured properly. Please update configuration parameter "pub_key" at "Configuration -> Payments".'
            );
        }
    }
    ...
    public static function getConfig()
    {
        return [
            ...
            "description" =>
                " You authenticate to the Stripe API by providing one of your API keys in the request. You can manage your API keys from your account.",
            "form" => [
                "test_api_key" => [
                    "text",
                    [
    ...
    protected function _generateForm(Model_Invoice $invoice)
    {
        ...
        $bindings = [
            ...
            ":label" => __("Pay now"),
            ":callbackUrl" => $payGatewayService->getCallbackUrl(
                $payGateway,
                $invoice
            ),
            ":redirectUrl" => $this->di["tools"]->url(
    ...
    public function get_test_pub_key()
    {
        if (!isset($this->config["test_pub_key"])) {
            throw new Payment_Exception(
                'Payment gateway "Stripe" is not configured properly. Please update configuration parameter "test_pub_key" at "Configuration -> Payments".'
            );
        }
    ...
    public function get_test_api_key()
    {
        if (!isset($this->config["test_api_key"])) {
            throw new Payment_Exception(
                'Payment gateway "Stripe" is not configured properly. Please update configuration parameter "test_api_key" at "Configuration -> Payments".'
            );
        }
