...
class Payment_Adapter_AuthorizeNet extends Payment_AdapterAbstract
{
    public function init()
    {
        ...
        if (!$this->getParam("apiLoginId")) {
            throw new Payment_Exception(
                'Payment gateway "Authorize.net" is not configured properly. Please update configuration parameter "API Login ID" at "Configuration -> Payments".'
            );
        }

        if (!$this->getParam("transactionKey")) {
            throw new Payment_Exception(
                'Payment gateway "Authorize.net" is not configured properly. Please update configuration parameter "Transaction Key" at "Configuration -> Payments".'
            );
        }

        if (!$this->getParam("md5Hash")) {
            throw new Payment_Exception(
                'Payment gateway "Authorize.net" is not configured properly. Please update configuration parameter "MD5 Hash Value" at "Configuration -> Payments".'
            );
        }
    }
    ...
    public static function getConfig()
    {
        return [
            ...
            "description" =>
                'To setup Authorize.net merchant account in BoxBilling you need to go to <i>Account &gt; Settinggs &gt; MD5-Hash</i> and enter your own secret hash values. To receive instant payment notifications, copy "IPN callback URL" to Authorize.net "Account->Silent Post URL"',
            "form" => [
                "apiLoginId" => [
                    "text",
                    [
    ...
    public function getTransaction($data, Payment_Invoice $invoice)
    {
        ...
        if (isset($ipn["x_response_code"])) {
            switch ($ipn["x_response_code"]) {
                ...
                default:
                    throw new Payment_Exception(
                        "Authorize.net unknown x_response_code"
                    );
            }
        }
    ...
    public function isIpnValid($data, Payment_Invoice $invoice)
    {
        ...
        if (
            $this->testMode &&
            isset($ipn["x_test_request"]) &&
            $ipn["x_test_request"] == "true"
        ) {
            return true;
        }
        $hash = $this->_getHash(
