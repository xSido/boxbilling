...
class Payment_Adapter_Interkassa extends Payment_AdapterAbstract implements
    \Box\InjectionAwareInterface
{
    ...
    public function init()
    {
        if (!$this->getParam("ik_co_id")) {
            throw new Payment_Exception(
                "Shop ID is missing in gateway configuration"
            );
        }
        if (!$this->getParam("ik_secret_key")) {
            throw new Payment_Exception(
                "Secret key is missing in gateway configuration"
            );
        }
    }
    ...
    public static function getConfig()
    {
        return [
            ...
            "description" =>
                "это удобный в использовании сервис, подключение к которому позволит Интернет-магазинам, веб-сайтам и прочим торговым площадкам принимать все возможные формы оплаты в максимально короткие сроки. http://www.interkassa.com/",
            "form" => [
                "ik_co_id" => [
                    "text",
                    [
                        "label" =>
                            'Shop ID which is registered in "INTERKASSA" system. Can be found under area "Настройки магазина". Example: 64C18529-4B94-0B5D-7405-F2752F2B716C',
                        "value" => "",
                    ],
                ],
                "ik_secret_key" => [
                    ...
                    [
                        "label" =>
                            "Secret Key. This is a line with characters, which is added to payment requisites, which is sent to seller with a notification. It is used to check and form data signature.",
                        "value" => "",
                    ],
                ],
                "ik_secret_key_test" => [
    ...
    {
        throw new Payment_Exception(
            "Interkassa payment gateway do not support recurrent payments"
        );
    }
    ...
    public function processTransaction($api_admin, $id, $data, $gateway_id)
    {
        ...
        if ($ipn["ik_inv_st"] == "success") {
            $invoiceModel = $this->di["db"]->load(
                "Invoice",
                $data["get"]["bb_invoice_id"]
            );
            $clientModel = $this->di["db"]->load(
                "Client",
