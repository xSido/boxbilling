...
class Payment_Adapter_WebMoney implements \Box\InjectionAwareInterface
{
    ...
    public function __construct($config)
    {
        $this->config = $config;
        $this->testMode =
            isset($config["test_mode"]) && $config["test_mode"] ? true : false;

        if (!$this->config["purse"]) {
            throw new Payment_Exception(
                'Payment gateway "WebMoney" is not configured properly. Please update configuration parameter "Purse" at "Configuration -> Payment gateways > WebMoney".'
            );
        }
    }
    ...
    public static function getConfig()
    {
        return [
            ...
            "description" =>
                'Configure WebMoney gateway. Do not forget enable option "Allow URLs transmitted in the form"',
            "form" => [
                "purse" => [
                    "text",
                    [
    ...
    private function _getHash($data)
    {
        $string =
            ...
            number_format($data["LMI_PAYMENT_AMOUNT"], 2) .
            $data["LMI_PAYMENT_NO"] .
            $data["INVOICE_ID"];
    ...
    {
        if (
            isset($request["LMI_PREREQUEST"]) &&
            $request["LMI_PREREQUEST"] == 1
        ) {
            return self::REQUEST_TYPE_PREREQUEST;
        } elseif (
            isset($request["act"]) &&
            ($request["act"] == "success" || $request["act"] == "fail")
        ) {
