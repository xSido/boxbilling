...
class Payment_Adapter_PayPalEmail implements \Box\InjectionAwareInterface
{
    ...
    public function __construct($config)
    {
        ...
        if (!function_exists("curl_exec")) {
            throw new Payment_Exception(
                "PHP Curl extension must be enabled in order to use PayPal gateway"
            );
        }

        if (!isset($this->config["email"])) {
            throw new Payment_Exception(
                'Payment gateway "PayPal" is not configured properly. Please update configuration parameter "PayPal Email address" at "Configuration -> Payments".'
            );
        }
    }
    ...
    public static function getConfig()
    {
        return [
            ...
            "description" =>
                "Enter your PayPal email to start accepting payments by PayPal.",
            "form" => [
                "email" => [
                    "text",
                    [
    ...
    public function processTransaction($api_admin, $id, $data, $gateway_id)
    {
        ...
        $invoice = $api_admin->invoice_get([
            "id" => $data["get"]["bb_invoice_id"],
        ]);
        ...
        switch ($ipn["txn_type"]) {
            ...
            case "subscr_payment":
                if ($ipn["payment_status"] == "Completed") {
                    ...
                    if ($tx["invoice_id"]) {
                        $api_admin->invoice_pay_with_credits([
                            "id" => $tx["invoice_id"],
                        ]);
                    }
            ...
            case "subscr_eot":
            case "subscr_cancel":
                $s = $api_admin->invoice_subscription_get([
                    "sid" => $ipn["subscr_id"],
                ]);
    ...
    private function download(
        ...
        $path = $uinf["path"];
        $path .=
            isset($uinf["query"]) && $uinf["query"] ? "?" . $uinf["query"] : "";
    ...
    private function _generateForm($url, $data, $method = "post")
    {
        ...
        $form .=
            '<input class="bb-button bb-button-submit" type="submit" value="Pay with PayPal" id="payment_button"/>' .
            PHP_EOL;
    ...
    public function getOneTimePaymentFields(array $invoice)
    {
        ...
        $data["business"] = $this->config["email"];
        $data["cmd"] = "_xclick";
        $data["amount"] = $this->moneyFormat(
            $invoice["subtotal"],
            $invoice["currency"]
        );
        $data["tax"] = $this->moneyFormat(
            $invoice["tax"],
            $invoice["currency"]
        );
        $data["bn"] = "BoxBilling_SP";
