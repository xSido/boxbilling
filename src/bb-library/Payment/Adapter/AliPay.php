...
class Payment_Adapter_AliPay extends Payment_AdapterAbstract
{
    public function init()
    {
        if (!$this->getParam("partner")) {
            throw new Payment_Exception(
                'Payment gateway "AliPay" is not configured properly. Please update configuration parameter "Partner ID" at "Configuration -> Payment gateways".'
            );
        }

        if (!$this->getParam("security_code")) {
            throw new Payment_Exception(
                'Payment gateway "AliPay" is not configured properly. Please update configuration parameter "Security Code" at "Configuration -> Payment gateways".'
            );
        }

        if (!$this->getParam("seller_email")) {
            throw new Payment_Exception(
                'Payment gateway "AliPay" is not configured properly. Please update configuration parameter "Seller email" at "Configuration -> Payment gateways".'
            );
        }
    ...
    public static function getConfig()
    {
        return [
            ...
            "description" =>
                "Clients will be redirected to Alipay to make payment.",
            "form" => [
                "partner" => [
                    "text",
                    [
                        "label" =>
                            "AliPay Partner ID. After signing contract online successfully, Alipay provides the partner id",
                    ],
                ],
                "security_code" => [
                    "text",
                    [
                        "label" =>
                            "AliPay security code. After signing contract online successfully, Alipay provides the 32bits security code",
                    ],
                ],
                "seller_email" => [
                    "text",
                ...
                "type" => [
                    ...
                    [
                        "multiOptions" => [
                            "1" =>
                                "Type 1 - receive money at once. You need to be registered company in china",
                            "2" =>
                                "Type 2 - receive money after buyer confirm it",
                        ],
                        "label" =>
                            "Contract type. Type 1: is mainly for virtual goods. So seller can receive money at once, but it requires seller is an register company; Type 2 is mainly for real goods, when buyer receive goods, then buyer check the goods is right and confirm to pay! During the process, the money is held by Alipay, if buyer dont claim to get money back, whether confirm or not, the money will transfer to seller after certain days.",
                    ],
                ],
            ],
        ];
    ...
    public function getInvoiceId($data)
    {
        ...
        if (is_null($id)) {
            $id = isset($data["post"]["out_trade_no"])
                ? (int) $data["post"]["out_trade_no"]
                : null;
        }
        return $id;
    }
    ...
    {
        throw new Payment_Exception(
            "AliPay does not support recurrent payments"
        );
    }
