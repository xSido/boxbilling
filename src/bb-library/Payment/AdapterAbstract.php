...
abstract class Payment_AdapterAbstract
{
    ...
    public function __construct($config)
    {
        ...
        if (!$this->getParam("return_url")) {
            throw new Payment_Exception(
                "Return URL for payment gateway was not set",
                [],
                6001
            );
        }
        ...
        if (!$this->getParam("cancel_url")) {
            throw new Payment_Exception(
                "Cancel URL for payment gateway was not set",
                [],
                6002
            );
        }
        ...
        if (!$this->getParam("notify_url")) {
            throw new Payment_Exception(
                "IPN Notification URL for payment gateway was not set",
                [],
                6003
            );
        }
        ...
        if (!$this->getParam("redirect_url")) {
            throw new Payment_Exception(
                "IPN redirect URL for payment gateway was not set",
                [],
                6004
            );
        }
    ...
    {
        throw new Payment_Exception(
            "Payment adapter class did not implement configuration options method",
            [],
            749
        );
    }
    ...
    {
        return isset($data["get"]["bb_invoice_id"])
            ? (int) $data["get"]["bb_invoice_id"]
            : null;
    }
