...
class Server_Manager_MOFH extends Server_Manager
{
    ...
    public function synchronizeAccount(Server_Account $a)
    {
        ...
        if ($response->isFound()) {
            ...
            } elseif ($response->getStatus() == "SUSPENDED") {
                $a->setSuspended(true);
            }
    ...
    public function suspendAccount(Server_Account $a, $suspend = true)
    {
        ...
        if (!$response->isSuccessful()) {
            throw new Server_Exception(
                "MOFH error: " . $response->getMessage()
            );
        }
    ...
    {
        throw new Server_Exception(
            "Unfortunately, MyOwnFreeHost API does not support account termination. You may log into your reseller panel, and execute the action there."
        );
    }
    ...
    {
        throw new Server_Exception(
            "Unfortunately, MyOwnFreeHost API does not support changing account packages. You may log into your reseller panel, and execute the action there."
        );
    }
    ...
    {
        throw new Server_Exception(
            "Server manager does not support DNS templates"
        );
    }
}
