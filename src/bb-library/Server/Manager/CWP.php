...
class Server_Manager_CWP extends Server_Manager
{
    public function init()
    {
        ...
        if (empty($this->_config["ip"])) {
            throw new Server_Exception(
                'Server manager "CWP" is not configured properly. IP address is not set!'
            );
        }

        if (empty($this->_config["host"])) {
            throw new Server_Exception(
                'Server manager "CWP" is not configured properly. Hostname is not set!'
            );
        }

        if (empty($this->_config["accesshash"])) {
            throw new Server_Exception(
                'Server manager "CWP" is not configured properly. API Key / Access Hash is not set!'
            );
        } else {
            $this->_config["accesshash"] = preg_replace(
                "'(\r|\n)'",
                "",
        ...
        if (!$this->_config["secure"]) {
             ...
             */
            throw new Server_Exception(
                'Server manager "CWP" is not configured properly. CWP API will only accept a secure connection!'
            );
        }
        /**
    ...
    {
        $this->getLog()->info(
            "Changing package on account " . $a->getUsername()
        );
    ...
    {
        $this->getLog()->info(
            "Changing password on account " . $a->getUsername()
        );
    ...
    {
        throw new Server_Exception(
            "CWP Does not support changing the primary domain name"
        );
    }
