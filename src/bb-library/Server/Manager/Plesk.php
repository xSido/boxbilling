...
class Server_Manager_Plesk extends Server_Manager
{
    ...
    public function testConnection()
    {
        ...
        if (
            isset($response->server->get->result->status) &&
            $response->server->get->result->status == "error"
        ) {
            throw new Server_Exception(
                "Plesk error: " .
                    $response->client->get->result->errcode .
                    " - " .
    ...
    public function setSubscription(Server_Account $a)
    {
        ...
        $response = $this->_makeRequest($params);
        if (
            isset($response->system->status) &&
            $response->system->status == "error"
        ) {
            throw new Server_Exception(
                "Plesk error: " .
                    $response->system->errcode .
                    " - " .
    ...
    public function suspendAccount(Server_Account $a, $suspend = true)
    {
        ...
        if (
            isset($response->{$type}->set->result->status) &&
            $response->{$type}->set->result->status == "error"
        ) {
            throw new Server_Exception(
                "Plesk error: " .
                    $response->{$type}->set->result->errcode .
                    " - " .
        ...
        if (
            isset($response->{$type}->set->result->status) &&
            $response->{$type}->set->result->status == "ok"
        ) {
            return true;
        } else {
            return false;
    ...
    public function cancelAccount(Server_Account $a)
    {
        ...
        if (
            isset($response->{$type}->del->result->status) &&
            $response->{$type}->del->result->status == "error"
        ) {
            throw new Server_Exception(
                "Plesk error:" .
                    $response->{$type}->del->result->errcode .
                    " - " .
        ...
        if (
            isset($response->{$type}->del->result->status) &&
            $response->{$type}->del->result->status == "ok"
        ) {
            return true;
        } else {
            return false;
    ...
    private function _makeRequest($params)
    {
        ...
        $xml = $this->_arrayToXml(
            $params,
            new SimpleXMLElement("<packet />")
        )->asXML();
        error_log($xml);
        $ch = curl_init();
    ...
    private function _arrayToXml(array $arr, SimpleXMLElement $xml)
    {
        ...
        foreach ($arr as $k => $v) {
            if (is_array($v)) {
                $this->_arrayToXml(
                    $v,
                    $xml->addChild(str_replace($numbers, "", $k))
                );
            } else {
    ...
    private function _parseResponse($result)
    {
        try {
            ...
            throw new Server_Exception(
                "simpleXmlException: " . $e->getMessage()
            );
        }
    ...
    private function _creatClient(Server_Account $a)
    {
        ...
        if (
            isset($response->{$type}->add->result->status) &&
            $response->{$type}->add->result->status == "error"
        ) {
            throw new Server_Exception(
                "Plesk error: " .
                    $response->{$type}->add->result->errcode .
                    " - " .
        ...
        if (
            isset($response->{$type}->add->result->status) &&
            $response->{$type}->add->result->status == "ok"
        ) {
    ...
    private function _getIps()
    {
        ...
        if (
            isset($response->ip->get->result->status) &&
            $response->ip->get->result->status == "error"
        ) {
            throw new Server_Exception(
                "Plesk error: " .
                    $response->ip->get->result->errcode .
                    " - " .
    ...
    private function _setIp(Server_Account $a)
    {
        ...
        if (
            isset($response->{$type}->ippool_add_ip->result->status) &&
            $response->{$type}->ippool_add_ip->result->status == "error"
        ) {
            throw new Server_Exception(
                "Plesk error: " .
                    $response->{$type}->ippool_add_ip->result->errcode .
                    " - " .
        ...
        if (
            isset($response->{$type}->ippool_add_ip->result->status) &&
            $response->{$type}->ippool_add_ip->result->status == "ok"
        ) {
            return true;
        } else {
            return false;
    ...
    private function _changeIpType(Server_Account $a)
    {
        ...
        if (
            isset($response->reseller->{'ippool-set-ip'}->result->status) &&
            $response->reseller->{'ippool-set-ip'}->result->status == "error"
        ) {
            throw new Server_Exception(
                "Plesk error: " .
                    $response->reseller->{'ippool-set-ip'}->result->errcode .
                    " - " .
