...
class Server_Manager_Directadmin extends Server_Manager
{
    public function init()
    {
        if (!extension_loaded("curl")) {
            throw new Server_Exception(
                "Direct admin server manager requires php curl extension to be enabled"
            );
        }

        if (empty($this->_config["host"])) {
            throw new Server_Exception(
                'Server manager "Direct Admin" is not configured properly. Hostname is not set'
            );
        }

        if (empty($this->_config["username"])) {
            throw new Server_Exception(
                'Server manager "Direct Admin" is not configured properly. Username is not set'
            );
        }

        if (empty($this->_config["password"])) {
            throw new Server_Exception(
                'Server manager "Direct Admin" is not configured properly. Password is not set'
            );
        }
    }
    ...
    public function createAccount(Server_Account $a)
    {
        ...
        if (empty($ips)) {
            throw new Server_Exception(
                sprintf(
                    'Server Manager DirectAdmin Error: "%s" ',
                    "There are no IPs on DirectAdmin server"
                )
            );
        }
        ...
        $fields["catchall"] = $package->getCustomValue("catchall")
            ? "ON"
            : "OFF"; //ON or OFF If ON, the User will have the ability to enable and customize a catch-all email (*@domain.com).
        ...
        $fields["dnscontrol"] = $package->getCustomValue("dnscontrol")
            ? "ON"
            : "OFF"; //ON or OFF If ON, the User will be able to modify his/her dns records.
        ...
        if (
            strpos(
                implode("", $results),
                "Unable to assign the Reseller ANY ips"
            ) !== false
        ) {
            throw new Server_Exception(
                "Unable to assign the Reseller ANY ips. Make sure to have free, un-assigned ips."
            );
        }
    ...
    public function unsuspendAccount(Server_Account $a)
    {
        ...
        if ($info["suspended"] == "no") {
            throw new Server_Exception(
                'Server Manager DirectAdmin Error: "Account is not suspended"'
            );
        }
    ...
    {
        throw new Server_Exception(
            "DirectAdmin do not support username changes"
        );
    }
    ...
    {
        $results = $this->_request("CMD_API_SHOW_RESELLER_IPS");
        return isset($results["list"]) ? $results["list"] : [];
    }
    ...
    private function _request($command, $fields = [], $post = true)
    {
        ...
        if ($usessl) {
            ...
            $url =
                "https://" . $host . ":2222/" . $command . "?" . $fieldstring;
        } else {
        ...
        if (strlen(strstr($data, "DirectAdmin Login")) > 0) {
            throw new Server_Exception(
                'Server Manager DirectAdmin Error: "Login failed"'
            );
        }
        ...
        if (
            ...
            throw new Server_Exception(
                'Server Manager DirectAdmin Error: "The request you have made cannot be executed because it does not exist in your authority level"'
            );
        }
