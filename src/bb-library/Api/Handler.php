    ...
    public function __call($method, $arguments)
    {
        if (strpos($method, "_") === false) {
            throw new \Box_Exception(
                "Method :method must contain underscore",
                [":method" => $method],
                710
            );
        }
        ...
        //cache
        $cache_key = md5(
            $this->type . $mod . $method_name . serialize($arguments)
        );
        ...
        if (!$service->isExtensionActive("mod", $mod)) {
            throw new \Box_Exception(
                "BoxBilling module :mod is not installed/activated",
                [":mod" => $mod],
                715
            );
        }
        ...
        if ($this->type == "admin") {
            ...
            if (
                ...
                if ($this->_acl_exception) {
                    throw new \Box_Exception(
                        "You do not have access to :mod module",
                        [":mod" => $mod],
                        725
                    );
                } else {
        ...
        $api_class =
            "\Box\Mod\\" . ucfirst($mod) . "\\Api\\" . ucfirst($this->type);

        $api = new $api_class();

        if (!$api instanceof Api_Abstract) {
            throw new \Box_Exception(
                "Api class must be instance of Api_Abstract",
                null,
                730
            );
        }
        ...
        if (
            !method_exists($api, $method_name) ||
            !is_callable([$api, $method_name])
        ) {
            $reflector = new ReflectionClass($api);
            if (!$reflector->hasMethod("__call")) {
                throw new \Box_Exception(
                    ":type API call :method does not exist in module :module",
                    [
