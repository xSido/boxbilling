...
class Model_ForumTopicTable implements \Box\InjectionAwareInterface
{
    ...
    public function getTopicsByIds(array $ids)
    {
        ...
        return $this->di["db"]->find(
            "ForumTopic",
            "id IN (:ids) AND status IN ('active', 'locked')",
            $bindings
        );
    }
    ...
    public function getAuthorClient(Model_ForumTopic $topic)
    {
        ...
        $forumTopicMessage = $this->di["db"]->findOne(
            "ForumTopicMessage",
            "forum_topic_id = :forum_topic_id ORDER BY id ASC LIMIT 1",
            $bindings
        );
        if ($forumTopicMessage instanceof Model_ForumTopicMessage) {
            return $this->di["db"]->getExistingModelById(
                "Client",
                $forumTopicMessage->client_id,
                "Client not found"
            );
        }
        return null;
    ...
    public function rm(Model_ForumTopic $topic)
    {
        ...
        $forumTopicMessage = $this->di["db"]->find(
            "ForumTopicMessage",
    ...
    public function toApiArray(
        ...
        $first = $this->di["db"]->findOne(
            "ForumTopicMessage",
            "forum_topic_id = :forum_topic_id ORDER BY id ASC LIMIT 1",
            $bindings
        );
        $last = $this->di["db"]->findOne(
            "ForumTopicMessage",
            "forum_topic_id = :forum_topic_id ORDER BY id DESC LIMIT 1",
            $bindings
        );
        ...
        $data["stats"]["posts_count"] = $table->getMessagesCountForTopic(
            $topic
        );
