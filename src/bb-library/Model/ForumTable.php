...
class Model_ForumTable implements \Box\InjectionAwareInterface
{
    ...
    public function findOneActiveById($id)
    {
        ...
        return $this->di["db"]->findOne(
            "Forum",
            "id = :forum_id AND status IN ('active', 'locked')",
            $bindings
        );
    }
    ...
    public function findOneActiveBySlug($slug)
    {
        ...
        return $this->di["db"]->findOne(
            "Forum",
            "slug = :slug AND status IN ('active', 'locked')",
            $bindings
        );
    }
    ...
    public function moveForum(Model_Forum $forum, $dir = "up")
    {
        ...
        $fn = $this->di["db"]->findOne("Forum", "priority = :priority", [
            ":priority" => $near,
        ]);
        $fn->priority = $current;
        $this->di["db"]->store($fn);
