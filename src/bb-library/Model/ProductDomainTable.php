...
class Model_ProductDomainTable extends Model_ProductTable
{
    ...
    private function _hasFreePeriod($addon)
    {
        ...
        if (
            in_array($addon_period, $free_domain_periods) ||
            sizeof($free_domain_periods) > 0
        ) {
            return true;
        } else {
            return false;
    ...
    {
        return isset($item["config"]["domain"]["action"]) &&
            $item["config"]["domain"]["action"] == $actionName;
    }

    private function _isFreeDomainSet($item)
    {
        $free_domain = $this->di["array_get"](
            $item["config"],
            "free_domain",
            false
        );
        $tld = $this->di["array_get"]($item["config"], "tld", null);
        $free_tlds = $this->di["array_get"]($item["config"], "free_tlds", []);

        if (
            $tld != null &&
            !$free_domain &&
            is_array($free_tlds) &&
            in_array($tld, $free_tlds)
        ) {
            return true;
        } else {
            return false;
    ...
    private function registerDomainMatch($item, $config)
    {
        ...
        return $item["config"]["domain"]["register_sld"] ==
            $config["register_sld"] &&
            $item["config"]["domain"]["register_tld"] ==
                $config["register_tld"];
    }
    ...
    private function transferDomainMatch($item, $config)
    {
        ...
        return $item["config"]["domain"]["transfer_sld"] ==
            $config["transfer_sld"] &&
            $item["config"]["domain"]["transfer_tld"] ==
                $config["transfer_tld"];
    }

    private function isFreeTransferSet($item)
    {
        return isset($item["config"]["free_transfer"]) &&
            $item["config"]["free_transfer"];
    }
    ...
    public function getProductPrice(
        ...
        if (!isset($config["action"])) {
            throw new \Box_Exception(
                "Could not determine domain price. Domain action is missing",
                null,
                498
            );
        }
        ...
        if (!$tld instanceof Model_Tld) {
            throw new \Box_Exception(
                "Unknown TLD. Could not determine registration price"
            );
        }
