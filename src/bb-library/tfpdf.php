...
class tFPDF
{
    ...
    function AddFont($family, $style = "", $file = "", $uni = false)
    {
        ...
        if ($file == "") {
            if ($uni) {
                $file =
                    str_replace(" ", "", $family) . strtolower($style) . ".ttf";
            } else {
                $file =
                    str_replace(" ", "", $family) . strtolower($style) . ".php";
            }
        }
        ...
        if ($uni) {
            if (
                defined("_SYSTEM_TTFONTS") &&
                file_exists(_SYSTEM_TTFONTS . $file)
            ) {
            ...
            $unifilename =
                $this->fontpath .
                "unifont/" .
                strtolower(substr($file, 0, strpos($file, ".")));
            $name = "";
            $originalsize = 0;
        ...
        if ($txt !== "") {
            ...
            if ($this->ws && $this->unifontSubset) {
                ...
                $s .= sprintf(
                    "BT 0 Tw %.2F %.2F Td [",
                    ($this->x + $dx) * $k,
                    ($this->h - ($this->y + 0.5 * $h + 0.3 * $this->FontSize)) *
                        $k
                );
                $t = explode(" ", $txt);
                $numt = count($t);
            ...
            if ($this->underline) {
                $s .=
                    ...
                    $this->_dounderline(
                        $this->x + $dx,
                        $this->y + 0.5 * $h + 0.3 * $this->FontSize,
                        $txt
                    );
            }
            if ($this->ColorFlag) {
    ...
    protected function _putfonts()
    {
        ...
        foreach ($this->fonts as $k => $font) {
            ...
            if ($type == "Core") {
                ...
                if (isset($font["uv"])) {
                    $this->_put(
                        "/ToUnicode " . $this->cmaps[$cmapkey] . " 0 R"
                    );
                }
                $this->_put(">>");
                $this->_put("endobj");
    ...
    protected function _putTTfontwidths(&$font, $maxUni)
    {
        ...
        for ($cid = $startcid; $cid < $cwlen; $cid++) {
            ...
            if (
                $cid > 255 &&
                (!isset($font["subset"][$cid]) || !$font["subset"][$cid])
            ) {
                continue;
            }
            if (
                !isset($font["dw"]) ||
                (isset($font["dw"]) && $width != $font["dw"])
            ) {
                if ($cid == $prevcid + 1) {
                    if ($width == $prevwidth) {
