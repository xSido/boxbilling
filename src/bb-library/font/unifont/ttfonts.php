...
class TTFontFile
{
    ...
    {
        return substr($stream, 0, $offset) .
            $value .
            substr($stream, $offset + strlen($value));
    }
    ...
    function extractInfo()
    {
        ...
        $this->skip(4);
        $this->italicAngle =
            $this->read_short() + $this->read_ushort() / 65536.0;
        ...
        foreach ($subsetglyphs as $originalGlyphIdx => $uni) {
            ...
            if ($glyphLen > 2 && $up[1] & (1 << 15)) {
                ...
                while ($flags & GF_MORE) {
                    ...
                    $glyphIdx = $up[1];
                    $this->glyphdata[$originalGlyphIdx][
                        "compGlyphs"
                    ][] = $glyphIdx;
                    $data = $this->_set_ushort(
                        $data,
                        $pos_in_glyph + 2,
                        $glyphSet[$glyphIdx]
                    );
    ...
    function getGlyphData(
        ...
        if (count($this->glyphdata[$originalGlyphIdx]["compGlyphs"])) {
            foreach (
                $this->glyphdata[$originalGlyphIdx]["compGlyphs"]
                as $glyphIdx
            ) {
                $this->getGlyphData(
                    $glyphIdx,
            ...
            $this->glyphdata[$originalGlyphIdx]["nContours"] > 0 &&
            $depth > 0
        ) {
            // simple
        ...
        for ($n = 0; $n < $segCount; $n++) {
            ...
            for ($unichar = $startCount[$n]; $unichar < $endpoint; $unichar++) {
                if ($idRangeOffset[$n] == 0) {
                    ...
                    $offset =
                        ($unichar - $startCount[$n]) * 2 + $idRangeOffset[$n];
