...
class openSRS_base
{
    ...
    private function _pullConfigVariables()
    {
        ...
        if ($actConfig != false) {
            ...
            // Load active Config
            $fileConfig = $xmlHand->xml2array(
                OPENSRSURI . OPENSRSCONFINGS . $actFile
            );
            if ($fileConfig != false) {
                $fileFileArray = $xmlHand->getValueByPath(
                    $fileConfig,
                    "config/osrsRegular"
                );
                ...
                foreach ($fileFileArray as $ffa) {
                    if (
                        $ffa["name"] == "osrs_username" &&
                        isset($ffa["value"])
                    ) {
                        $this->osrs_username = $ffa["value"];
                    }
                    if (
                        $ffa["name"] == "osrs_password" &&
                        isset($ffa["value"])
                    ) {
                    ...
                    if (
                        $ffa["name"] == "osrs_environment" &&
                        isset($ffa["value"])
                    ) {
                        $this->osrs_environment = $ffa["value"];
                    }
                    if (
                        $ffa["name"] == "osrs_protocol" &&
                        isset($ffa["value"])
                    ) {
                    ...
                    if (
                        $ffa["name"] == "osrs_sslPort" &&
                        isset($ffa["value"])
                    ) {
                        $this->osrs_sslPort = $ffa["value"];
                    }
                    if (
                        $ffa["name"] == "osrs_baseClassVersion" &&
                        isset($ffa["value"])
                    ) {
                        $this->osrs_baseClassVersion = $ffa["value"];
                    }
                    if (
                        $ffa["name"] == "osrs_version" &&
                        isset($ffa["value"])
                    ) {
                ...
                if (
                    $this->osrs_username == "" ||
                    $this->osrs_key == "" ||
                    $this->osrs_environment == "" ||
                    $this->osrs_protocol == ""
                ) {
                    if ($this->osrs_username == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_username",
                            E_USER_WARNING
                        );
                    }
                    if ($this->osrs_key == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_key",
                            E_USER_WARNING
                        );
                    }
                    if ($this->osrs_environment == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_environment",
                            E_USER_WARNING
                        );
                    }
                    if ($this->osrs_protocol == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_protocol",
                            E_USER_WARNING
                        );
                    }
                    die();
                ...
                if (
                    ...
                    $this->osrs_port == "" ||
                    $this->osrs_sslPort == "" ||
                    $this->osrs_baseClassVersion == "" ||
                    $this->osrs_version == ""
                ) {
                    if ($this->osrs_host == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_host",
                            E_USER_WARNING
                        );
                    }
                    if ($this->osrs_port == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_port",
                            E_USER_WARNING
                        );
                    }
                    if ($this->osrs_sslPort == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_sslPort",
                            E_USER_WARNING
                        );
                    }
                    if ($this->osrs_baseClassVersion == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_baseClassVersion",
                            E_USER_WARNING
                        );
                    }
                    if ($this->osrs_version == "") {
                        trigger_error(
                            "oSRS Error - Incomplete config file - Missing osrs_version",
                            E_USER_WARNING
                        );
                    }
                    die();
                ...
                if ($this->osrs_environment != "") {
                    $this->osrs_environment = strtoupper(
                        $this->osrs_environment
                    );
                }
                if ($this->osrs_protocol) {
    ...
    private function _verifySystemProperties()
    {
        ...
        if ("SSL" == $this->crypt_type) {
            if (
                ...
                version_compare("4.3", phpversion(), ">=")
            ) {
                $error_message =
                    "PHP version must be v4.3+ (current version is " .
                    phpversion() .
                ...
                $error_message =
                    "PHP must be compiled using --with-openssl to use \"SSL\" encryption";
                trigger_error($error_message, E_USER_WARNING);
                die();
            }
        }
    ...
    public function send_cmd($request)
    {
        ...
        if (!$this->init_socket()) {
            trigger_error(
                "oSRS Error - Unable to establish socket: (" .
        ...
        if (!$auth) {
            ...
            trigger_error(
                "oSRS Error - Authentication Error: " . $auth["error"],
                E_USER_WARNING
            );
            die();
        }
        ...
        if ($num_matches > 0) {
            trigger_error(
                "oSRS Error - Reseller username or osrs_key is incorrect, please check your config file."
            );
        }
    ...
    private function authenticate()
    {
        ...
        if (isset($prompt["response_code"])) {
            ...
            !preg_match("/OpenSRS\sSERVER/", $prompt["attributes"]["sender"]) ||
        ...
        if ($hex_check) {
            $this->_CBC = new openSRS_crypt(
                pack("H*", $this->osrs_key),
                $this->crypt_type
            );
            ...
            trigger_error(
                "oSRS Error - Please check the osrs_key value in the config file, it contains a non hexidecimal character."
            );
        }
    }
    ...
    {
        if ($this->_CBC) {
            $message = $this->_CBC->encrypt($message);
        }
    ...
    private function writeData(&$fh, $msg)
    {
        ...
        switch ($this->crypt_type) {
            case "SSL":
                ...
                $header .=
                    "Content-Length: " . $len . $this->CRLF . $this->CRLF;
                break;
            case "BLOWFISH":
            case "DES":
            default:
        ...
        switch ($this->crypt_type) {
            case "SSL":
                ...
                while ($line != $this->CRLF) {
                    ...
                    if (feof($fh)) {
                        trigger_error(
                            "oSRS Error - UNEXPECTED READ: Error reading HTTP header.",
                            E_USER_WARNING
                        );
                        return false;
                    }
            ...
            default:
                ...
                if (
                    ...
                    trigger_error(
                        "oSRS Error - UNEXPECTED READ: No Content-Length.",
                        E_USER_WARNING
                    );
                    return false;
                }
                ...
                if ($line != $this->CRLF) {
                    trigger_error(
                        "oSRS Error - UNEXPECTED READ: No CRLF.",
                        E_USER_WARNING
                    );
                    return false;
                }
    ...
    private function readData(&$fh, $timeout = 5)
    {
        ...
        if (
            ...
            empty($header["content-length"])
        ) {
            if ($this->crypt_type == "SSL") {
                trigger_error(
                    "oSRS Error - UNEXPECTED ERROR: No Content-Length header provided! Please make sure IP is whitelisted in RWI.",
                    E_USER_WARNING
                );
            } else {
                trigger_error(
