...
class openSRS_crypt
{
    ...
    function openSRS_crypt($key, $cipher = "DES")
    {
        if (!extension_loaded("mcrypt")) {
            trigger_error(
                "oSRS Error - mcrypt module is not compiled into PHP",
                E_USER_ERROR
            );
        }
        if (!function_exists("mcrypt_module_open")) {
            trigger_error(
                "oSRS Error - libmcrypt version insufficient",
                E_USER_ERROR
            );
        }
        ...
        } elseif (function_exists("mcrypt_generic_end")) {
            $this->deinit_function = "mcrypt_generic_end";
        } else {
            trigger_error(
                "oSRS Error - PHP version insufficient",
                E_USER_ERROR
            );
        }
        ...
        if (!$key) {
            trigger_error("oSRS Error - no key specified", E_USER_ERROR);
        }

        $cipher = strtoupper($cipher); // check for cipher
        if (!isset($this->known_ciphers[$cipher])) {
            trigger_error(
                "oSRS Error - unknown cipher - " . $cipher,
                E_USER_ERROR
            );
        }
    ...
    function encrypt($clear)
    {
        ...
        $padsize =
            $this->blocksize -
            (strlen($clear) % $this->blocksize); /* pad the cleartext */
        ...
        while ($block = substr($clear, $start, $this->blocksize)) {
            ...
            if (mcrypt_generic_init($this->TD, $this->key, $iv) < 0) {
                trigger_error(
                    "oSRS Error - mcrypt_generic_init failed",
                    E_USER_ERROR
                );
            }
    ...
    function decrypt($crypt)
    {
        ...
        if ($header != $this->header_spec) {
            trigger_error(
                "oSRS Error - no initialization vector",
                E_USER_ERROR
            );
        }
