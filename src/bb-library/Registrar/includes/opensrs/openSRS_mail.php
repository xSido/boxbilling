...
class openSRS_mail
{
    ...
    private function _pullConfigVariables()
    {
        ...
        if ($fileFileArray == null) {
            trigger_error(
                "oSRS-eMail Error - Could not fine the osrsEmail section in current config, please check your config file",
                E_USER_WARNING
            );
        } else {
    ...
    public function makeCall($sequence)
    {
        ...
        if (!$fp) {
            ...
            for ($i = 0; $i < count($sequence); $i++) {
                ...
                if (!$fwrite) {
                    trigger_error(
                        "oSRS - Mail System Write Error, please check if your network allows connection to the server."
                    );
                }
                ...
                if ($arrayMatch != 0) {
                    for ($j = 0; $j < $arrayMatch; $j++) {
                        if ($arrayMatches[1][$j] == "LIST") {
                            $result["attributes"][
                                strtolower($arrayMatches[1][$j])
                            ] = explode(",", $arrayMatches[2][$j]);
                        } else {
                            $result["attributes"][
                                strtolower($arrayMatches[1][$j])
                            ] = $arrayMatches[2][$j];
                    ...
                    if (count($tableLines) > 1) {
                        ...
                        for ($j = 1; $j < count($tableLines); $j++) {
                            ...
                            foreach ($tableHeaders as $tableHeader) {
                                $result["attributes"]["list"][$j - 1][
                                    strtolower($tableHeader)
                                ] = str_replace('"', "", $values[$k]);
                                $k++;
                            }
                        }
                    }
