...
class bulkChange extends openSRS_base
{
    ...
    private function _validateObject()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change_items is not defined.",
                E_USER_WARNING
            );
            $allPassed = false;
        }
        if (
            ...
            trigger_error(
                "oSRS Error - change_type is not defined.",
                E_USER_WARNING
            );
            $allPassed = false;
        } else {
            switch ($this->_dataObject->data->change_type) {
                ...
                default:
                    trigger_error(
                        "oSRS Error - Unknown change type.",
                        E_USER_WARNING
                    );
                    $allPassed = false;
                    break;
    ...
    private function _allDomainRenewCheck()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is domain_renew but at least one of period, let_expire or auto_renew has to be defined.",
                E_USER_WARNING
            );
    ...
    private function _allPushDomainsCheck()
    {
        ...
        if (
            !isset($this->_dataObject->data->gaining_reseller_username) ||
            $this->_dataObject->data->gaining_reseller_username == ""
        ) {
            trigger_error(
                "oSRS Error - change type is dns_zone but gaining_reseller_username is not defined.",
                E_USER_WARNING
            );
    ...
    private function _allDnsZoneCheck()
    {
        ...
        if (
            !isset($this->_dataObject->data->apply_to_domains) ||
            $this->_dataObject->data->apply_to_domains == ""
        ) {
            trigger_error(
                "oSRS Error - change type is dns_zone but apply_to_domains is not defined.",
                E_USER_WARNING
            );
            $allDnsZonePassed = false;
        }
        if (
            !isset($this->_dataObject->data->dns_action) ||
            $this->_dataObject->data->dns_action == ""
        ) {
            trigger_error(
                "oSRS Error - change type is dns_zone but dns_action is not defined.",
                E_USER_WARNING
            );
            $allDnsZonePassed = false;
        }
    ...
    private function _allDnsZoneRecordCheck()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is dns_zone_record but dns_action is not defined.",
                E_USER_WARNING
            );
            $allDnsZoneRecordPassed = false;
        }
        if (
            !isset($this->_dataObject->data->dns_record_type) ||
            $this->_dataObject->data->dns_record_type == ""
        ) {
            trigger_error(
                "oSRS Error - change type is dns_zone_record but dns_record_type is not defined.",
                E_USER_WARNING
            );
            $allDnsZoneRecordPassed = false;
        }
        if (
            !isset($this->_dataObject->data->dns_record_data) ||
            $this->_dataObject->data->dns_record_data == ""
        ) {
            trigger_error(
                "oSRS Error - change type is dns_zone_record but dns_record_data is not defined.",
                E_USER_WARNING
            );
            $allDnsZoneRecordPassed = false;
        }
    ...
    private function _allDomainContactsCheck()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is domain_contacts but type is not defined.",
                E_USER_WARNING
            );
            $alllDomainContactsPassed = false;
        }
        if (
            !isset($this->_dataObject->personal) ||
            $this->_dataObject->personal == ""
        ) {
            trigger_error(
                "oSRS Error - change type is domain_contacts but personal is not defined.",
                E_USER_WARNING
            );
            $alllDomainContactsPassed = false;
        }
    ...
    private function _allDomainForwardingCheck()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is domain_forwarding but op_type is not defined.",
                E_USER_WARNING
            );
    ...
    private function _allDomainLockCheck()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is domain_lock but op_type is not defined.",
                E_USER_WARNING
            );
    ...
    private function _allDomainNameserversCheck()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is domain_nameservers but op_type is not defined.",
                E_USER_WARNING
            );
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is domain_nameservers but at least one of add_ns, remove_ns or assign_ns has to be defined.",
                E_USER_WARNING
            );
            $allDomainNameserversPassed = false;
        }
    ...
    private function _allDomainParkedPagesCheck()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is domain_parked_pages but op_type is not defined.",
                E_USER_WARNING
            );
    ...
    private function _allWhoisPrivacyCheck()
    {
        ...
        if (
            ...
            trigger_error(
                "oSRS Error - change type is whois_privacy but op_type is not defined.",
                E_USER_WARNING
            );
    ...
    {
        $this->_dataObject->data->change_items = explode(
            ",",
            $this->_dataObject->data->change_items
        );
        ...
        if (
            ...
            $cmd["attributes"]["apply_to_locked_domains"] =
                $this->_dataObject->data->apply_to_locked_domains;
        }
        if (
        ...
        if (
            ...
            $cmd["attributes"]["apply_to_all_reseller_items"] =
                $this->_dataObject->data->apply_to_all_reseller_items;
        }
        ...
        switch ($this->_dataObject->data->change_type) {
            ...
            case "push_domains":
                if (
                    ...
                    $cmd["attributes"]["gaining_reseller_username"] =
                        $this->_dataObject->data->gaining_reseller_username;
                }
                break;
            case "dns_zone":
                if (
                    ...
                    $cmd["attributes"]["apply_to_domains"] =
                        $this->_dataObject->data->apply_to_domains;
                }
                if (
                    isset($this->_dataObject->data->dns_action) &&
                ...
                if (
                    isset($this->_dataObject->data->only_if) &&
                    $this->_dataObject->data->only_if != ""
                ) {
                    $cmd["attributes"]["only_if"] =
                        $this->_dataObject->data->only_if;
                }
                if (
                    isset($this->_dataObject->data->force_dns_nameservers) &&
                    $this->_dataObject->data->force_dns_nameservers != ""
                ) {
                    $cmd["attributes"]["force_dns_nameservers"] =
                        $this->_dataObject->data->force_dns_nameservers;
                }
                break;
            case "dns_zone_record":
                ...
                if (
                    ...
                    $cmd["attributes"]["dns_record_type"] =
                        $this->_dataObject->data->dns_record_type;
                }
                if (
                    isset(
                        $this->_dataObject->data->dns_record_data->ip_address
                    ) &&
                    $this->_dataObject->data->dns_record_data->ip_address != ""
                ) {
                    $cmd["attributes"]["dns_record_data"]["ip_address"] =
                        $this->_dataObject->data->dns_record_data->ip_address;
                }
                if (
                    isset(
                        $this->_dataObject->data->dns_record_data->subdomain
                    ) &&
                    $this->_dataObject->data->dns_record_data->subdomain != ""
                ) {
                    $cmd["attributes"]["dns_record_data"]["subdomain"] =
                        $this->_dataObject->data->dns_record_data->subdomain;
                }
                if (
                    isset(
                        $this->_dataObject->data->dns_record_data->ipv6_address
                    ) &&
                    $this->_dataObject->data->dns_record_data->ipv6_address !=
                        ""
                ) {
                    $cmd["attributes"]["dns_record_data"]["ipv6_address"] =
                        $this->_dataObject->data->dns_record_data->ipv6_address;
                }
                if (
                    isset(
                        $this->_dataObject->data->dns_record_data->hostname
                    ) &&
                    $this->_dataObject->data->dns_record_data->hostname != ""
                ) {
                    $$cmd["attributes"]["dns_record_data"]["hostname"] =
                        $this->_dataObject->data->dns_record_data->hostname;
                }
                if (
                    isset(
                        $this->_dataObject->data->dns_record_data->priority
                    ) &&
                    $this->_dataObject->data->dns_record_data->priority != ""
                ) {
                    $cmd["attributes"]["dns_record_data"]["priority"] =
                        $this->_dataObject->data->dns_record_data->priority;
                }
                if (
                    isset($this->_dataObject->data->dns_record_data->weight) &&
                    $this->_dataObject->data->dns_record_data->weight != ""
                ) {
                    $cmd["attributes"]["dns_record_data"]["weight"] =
                        $this->_dataObject->data->dns_record_data->weight;
                }
                if (
                    isset($this->_dataObject->data->dns_record_data->port) &&
                    $this->_dataObject->data->dns_record_data->port != ""
                ) {
                    $cmd["attributes"]["dns_record_data"]["port"] =
                        $this->_dataObject->data->dns_record_data->port;
                }
                if (
                    isset($this->_dataObject->data->dns_record_data->text) &&
                    $this->_dataObject->data->dns_record_data->text != ""
                ) {
                    $cmd["attributes"]["dns_record_data"]["text"] =
                        $this->_dataObject->data->dns_record_data->text;
                }
                if (
                    isset($this->_dataObject->data->only_if) &&
            ...
            case "domain_nameservers":
                ...
                if (
                    ...
                    $cmd["attributes"]["remove_ns"] = explode(
                        ",",
                        $this->_dataObject->data->remove_ns
                    );
                }
                if (
                    ...
                    $cmd["attributes"]["assign_ns"] = explode(
                        ",",
                        $this->_dataObject->data->assign_ns
                    );
                }
