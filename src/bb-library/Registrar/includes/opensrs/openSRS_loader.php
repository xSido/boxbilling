...
function loadOpenSRS($local, $extension = "php", $subfolders = false)
{
    if (!is_dir($local)) {
        trigger_error(
            "OSRS Error - Unable to find library directory $local, please check the OPENSRSURI path in openSRS_config.php"
        );
    } else {
        $dir = opendir($local);
        while (false != ($file = readdir($dir))) {
            ...
            if (
                ...
                if (
                    substr($fileArr[0], 0, 1) != "_" &&
                    $fileArr[$lastExt] == $extension
                ) {
                    if (!is_file($local . $file)) {
                        trigger_error(
                            "OSRS Error - Unable to find library file $file, please check the paths in openSRS_config.php"
                        );
                    } else {
            ...
            if (
                ...
                if (!is_dir($subPath)) {
                    trigger_error(
                        "OSRS Error - Unable to find library directory $subPath, please check path constants in openSRS_config.php"
                    );
                } else {
                    ...
                    while (false != ($subFile = readdir($subDir))) {
                        ...
                        if (
                            $subFile != "." &&
                            $subFile != ".." &&
                            !is_dir($subPath . $subFile . "/")
                        ) {
                            $subfileArr = explode(".", $subFile);
                            $sublastExt = count($subfileArr) - 1;
                            if (
                                substr($subfileArr[0], 0, 1) != "_" &&
                                $subfileArr[$sublastExt] == $extension
                            ) {
...
function processOpenSRS($type = "", $data = "")
{
    ...
    if (class_exists($dataObject->func)) {
        ...
        $classCall = null;
        trigger_error(
            "OSRS Error - Unable to find the function passed.  Either the function is misspelled or there are incorrect file paths set in openSRS_config.php."
        );
    }
    return $classCall;
}
