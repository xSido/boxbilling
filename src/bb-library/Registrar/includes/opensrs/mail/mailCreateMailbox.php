...
class mailCreateMailbox extends openSRS_mail
{
    ...
    private function _validateObject()
    {
        ...
        if (
            !isset($this->_dataObject->data->workgroup) ||
            $this->_dataObject->data->workgroup == ""
        ) {
            trigger_error(
                "oSRS-eMail Error - workgroup is not defined.",
                E_USER_WARNING
            );
            $allPassed = false;
        } else {
            $compile .=
                " workgroup=\"" . $this->_dataObject->data->workgroup . "\"";
        }

        if (
            !isset($this->_dataObject->data->mailbox) ||
            $this->_dataObject->data->mailbox == ""
        ) {
            trigger_error(
                "oSRS-eMail Error - mailbox is not defined.",
                E_USER_WARNING
            );
            $allPassed = false;
        } else {
            $compile .=
                " mailbox=\"" . $this->_dataObject->data->mailbox . "\"";
        }
        ...
        if (
            isset($this->_dataObject->data->phone) &&
            $this->_dataObject->data->phone != ""
        ) {
            $compile .= " phone=\"" . $this->_dataObject->data->phone . "\"";
        }

        if (
            isset($this->_dataObject->data->fax) &&
            $this->_dataObject->data->fax != ""
        ) {
        ...
        if (
            isset($this->_dataObject->data->filteronly) &&
            $this->_dataObject->data->filteronly != ""
        ) {
            $compile .=
                " filteronly=\"" . $this->_dataObject->data->filteronly . "\"";
        }
    ...
    private function _processRequest($command = "")
    {
        $sequence = [
            ...
            1 =>
                "login user=\"" .
                $this->_dataObject->data->authusername .
                "\" domain=\"" .
                $this->_dataObject->data->authdomain .
                "\" password=\"" .
                $this->_dataObject->data->authpassword .
                "\"",
