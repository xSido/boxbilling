...
class mailSetDomainAllowList extends openSRS_mail
{
    private $_dataObject;
    ...
    private function _validateObject()
    {
        ...
        if (
            !isset($this->_dataObject->data->list) ||
            $this->_dataObject->data->list == ""
        ) {
            trigger_error(
                "oSRS-eMail Error - list is not defined.",
                E_USER_WARNING
            );
            $allPassed = false;
        } else {
            $compile .= " list=\"" . $this->_dataObject->data->list . "\"";
        }
    ...
    private function _processRequest($command = "")
    {
        $sequence = [
            ...
            1 =>
                ...
                $this->_dataObject->data->password .
                "\"",
            2 => "set_domain_allow_list" . $command,
            3 => "quit",
        ];
