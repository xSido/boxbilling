...
if (!extension_loaded("curl")) {
    trigger_error("CNic_Toolkit requires the cURL extension!", E_USER_ERROR);
}
...
class CNic_Toolkit
{
    ...
    function __construct()
    {
        ...
        ini_set(
            "include_path",
            ini_get("include_path") . ":" . dirname(dirname(__FILE__))
        );
    }
    ...
    function execute()
    {
        ...
        curl_setopt(
            $this->agent,
            CURLOPT_TIMEOUT,
            isset($this->params["timeout"]) && $this->params["timeout"] > 0
                ? $this->params["timeout"]
                : 10
        );
        switch ($this->command) {
    ...
    private function _search()
    {
        ...
        if (isset($this->params["suffixlist"])) {
            $params["suffixlist"] = implode(",", $this->params["suffixlist"]);
        }
    ...
    private function _create_handle()
    {
        ...
        if (!empty($this->params["handle"]["street1"])) {
            ...
            $params["handle"] = $this->_format_handle_string(
                $this->params["handle"]
            );
        }
    ...
    private function _register($idn)
    {
        ...
        if (!isset($this->params["suffix"])) {
            [$this->params["domain"], $this->params["suffix"]] = explode(
                ".",
                $this->params["domain"],
                2
            );
        ...
        if (isset($this->params["ahandle"])) {
            $params["ahandle"] = $this->params["ahandle"];
        }
        if (isset($this->params["bhandle"])) {
            $params["bhandle"] = $this->params["bhandle"];
        }
        if (isset($this->params["url"])) {
            $params["url"] = $this->params["url"];
        }
        if (isset($this->params["dns"])) {
            $params = array_merge(
                $params,
                $this->_dns_register_params($this->params["dns"])
            );
        }
        $this->_raw_data = $this->_request($url, "POST", $params);
    ...
    private function _modify()
    {
        ...
        if (isset($this->params["thandle"])) {
            $params["thandle"] = $this->_format_handle_string(
                $this->params["thandle"]
            );
        }
        if (isset($this->params["chandle"])) {
            $params["chandle"] = $this->_format_handle_string(
                $this->params["chandle"]
            );
        }
        if (isset($this->params["ahandle"])) {
            $params["url"] = $this->params["ahandle"];
        }
        if (isset($this->params["bhandle"])) {
            $params["url"] = $this->params["bhandle"];
        }
        if (isset($this->params["ttl"])) {
            $params["ttl"] = $this->_params["ttl"];
        }
        if (isset($this->params["dns"])) {
            $params = array_merge(
                $params,
                $this->_dns_mod_params($this->params["dns"])
            );
        }
        if (isset($this->params["url"])) {
            $params["url"] = $this->params["url"];
    ...
    private function _issue_renewals()
    {
        ...
        for ($i = 0; $i < count($this->params["domains"]); $i++) {
            $params["domains[$i]"] = $this->params["domains"][$i];
        }
        $this->_raw_data = $this->_request($url, "POST", $params);
    ...
    private function _list_outstanding_domains()
    {
        ...
        $this->_raw_data = $this->_request($url, "POST", $params);
        return new CNic_Toolkit_Response_ListOutstandingDomains(
            $this->_raw_data
        );
    }
    ...
    private function _request()
    {
        ...
        switch ($no_args) {
            ...
            default:
                trigger_error(
                    "invalid number of arguments to CNic_Toolkit::_get()",
                    E_USER_ERROR
                );
        }

        if ($method == "POST") {
            curl_setopt($this->agent, CURLOPT_POST, 1);
        }
        ...
        if (count($params) > 0) {
            foreach ($params as $name => $value) {
                if (
                    in_array(gettype($value), ["array", "object", "resource"])
                ) {
                    trigger_error(
                        sprintf(
                            "cannot use value of type %s as value for '%s', must be a string",
                            gettype($value),
                            $name
                        ),
                        E_USER_ERROR
    ...
    private function _format_handle_string($handle)
    {
        if (is_string($handle)) {
            ...
            trigger_error(
                "invalid variable type passed to CNic_Toolkit::_format_handle_string()",
                E_USER_ERROR
            );
        } else {
    ...
    private function _dns_register_params($dns)
    {
        if (!is_array($dns)) {
            trigger_error(
                "invalid variable type passed to CNic_Toolkit::_dns_register_params()",
                E_USER_ERROR
            );
        } else {
            $array = [];
    ...
    private function _dns_mod_params($dns)
    {
        if (!is_array($dns)) {
            trigger_error(
                "invalid variable type passed to CNic_Toolkit::_dns_mod_params()",
                E_USER_ERROR
            );
        } else {
            $array = [];
    ...
    {
        if (in_array($this->command, ["whois", "search", "suffixes"])) {
