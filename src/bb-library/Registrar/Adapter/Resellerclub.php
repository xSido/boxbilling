...
class Registrar_Adapter_Resellerclub extends Registrar_AdapterAbstract
{
    ...
    public static function getConfig()
    {
        return [
            "label" =>
                "Manages domains on ResellerClub via API. ResellerClub requires your server IP in order to work. Login to the ResellerClub control panel (the url will be in the email you received when you signed up with them) and then go to Settings > API and enter the IP address of the server where BoxBilling is installed to authorize it for API access.",
            "form" => [
                "userid" => [
                    "text",
                    [
                        "label" =>
                            "Reseller ID. You can get this at ResellerClub control panel Settings > Personal information > Primary profile > Reseller ID",
    ...
    {
        $customer = $this->_getCustomerDetails($domain);
        $contacts = $this->_getDefaultContactDetails(
            $domain,
            $customer["customerid"]
        );
    ...
    public function registerDomain(Registrar_Domain $domain)
    {
        ...
        if ($tld == ".au" || $tld == ".net.au" || $tld == ".com.au") {
            ...
            if (strlen(trim($contact->getCompanyNumber())) == 0) {
                throw new Registrar_Exception(
                    "Valid contact company number is required while registering AU domain name"
                );
            }
    ...
    public function getEpp(Registrar_Domain $domain)
    {
        ...
        if (!isset($data["domsecret"])) {
            throw new Registrar_Exception(
                "Domain EPP code can be retrieved from domain registrar"
            );
        }
    ...
    private function addSubReseller($params)
    {
        ...
        if (
            isset($result["status"]) &&
            $result["status"] == "AlreadyReseller"
        ) {
            throw new Registrar_Exception(
                "You are already registered as reseller"
            );
        }
    ...
    protected function _makeRequest(
        ...
        if ($method == "POST") {
            ...
            $opts[CURLOPT_URL] =
                $opts[CURLOPT_URL] . "?" . $this->_formatParams($params);
        ...
        $result = curl_exec($ch);
        if ($result === false) {
            $e = new Registrar_Exception(
                sprintf('CurlException: "%s"', curl_error($ch))
            );
            $this->getLog()->err($e);
            curl_close($ch);
            throw $e;
    ...
    private function _getAllContacts(
        $tld,
        $customer_id,
        \Registrar_Domain_Contact $client
    ) {
        if ($tld[0] != ".") {
        ...
        $reg_contact_id = $this->_getContact(
            $contact,
            $customer_id,
            $contact["type"]
        );
        ...
        if ($tld == ".asia") {
            if (strlen(trim($client->getDocumentNr())) == 0) {
                throw new Registrar_Exception(
                    "Valid contact Passport information is required while registering ASIA domain name"
                );
            }
        ...
        if (
            ...
            if (
                strlen(trim($client->getBirthday())) === 0 ||
                strtotime($client->getBirthday()) === false
            ) {
                throw new Registrar_Exception(
                    "Valid contact Birth Date is required while registering RU domain name"
                );
            }
            ...
            $contact["attr-value2"] = date(
                "d.m.Y",
                strtotime($client->getBirthday())
            );
        ...
        while ($param_exists) {
            if (!array_key_exists("attr-name" . $attr_number, $contact)) {
                ...
                $contact["attr-value" . $attr_number] = strtolower(
                    $client->getIdnLanguageCode()
                );
                $param_exists = false;
            }
            $attr_number++;
        ...
        if ($contact["type"] != "Contact") {
            $special_contact_id = $this->_getContact(
                $contact,
                $customer_id,
                $contact["type"]
            );
        ...
        $admin_contact_id = isset($special_contact_id)
            ? $special_contact_id
            : $reg_contact_id;
        $tech_contact_id = isset($special_contact_id)
            ? $special_contact_id
            : $reg_contact_id;
        $billing_contact_id = isset($special_contact_id)
            ? $special_contact_id
            : $reg_contact_id;
    ...
    {
        $agreement = $this->_makeRequest(
            "contacts/dotca/registrantagreement",
            [],
            "GET",
            "json"
        );
