...
class Registrar_Adapter_srsx extends Registrar_Adapter_Resellerclub
{
    public function __construct($options)
    {
        ...
        if (isset($options["resellerId"]) && !empty($options["resellerId"])) {
            ...
            throw new Registrar_Exception(
                'Domain registrar "SRSX" is not configured properly. Please update configuration parameter "SRSX Reseller ID" at "Configuration -> Domain registration".'
            );
        }
        ...
        if (isset($options["apiUsername"]) && !empty($options["apiUsername"])) {
            ...
            throw new Registrar_Exception(
                'Domain registrar "SRSX" is not configured properly. Please update configuration parameter "SRSX Username" at "Configuration -> Domain registration".'
            );
        }
        ...
        if (isset($options["apiPassword"]) && !empty($options["apiPassword"])) {
            ...
            throw new Registrar_Exception(
                'Domain registrar "SRSX" is not configured properly. Please update configuration parameter "ResellerClub API Key" at "Configuration -> Domain registration".'
            );
        }
    }
    ...
    public static function getConfig()
    {
        return [
            "label" =>
                "Manages domains on SRSX via API. ResellerClub requires your server IP in order to work. Login to the ResellerClub control panel (the url will be in the email you received when you signed up with them) and then go to Settings > API and enter the IP address of the server where BoxBilling is installed to authorize it for API access.",
            "form" => [
                "resellerId" => [
                    "text",
                    [
                        "label" => "RESELLER ID",
                        "description" =>
                            "You can get this at ResellerClub control panel, go to Settings -> API",
                    ],
                ],
                ...
                "apiUsername" => [
                    ...
                    [
                        "label" =>
                            "Username Reseller. You can get this at ResellerClub control panel Settings > Personal information > Primary profile > Reseller ID",
                ...
                "apiPassword" => [
                    ...
                    [
                        "label" =>
                            "Password Reseller. You can get this at ResellerClub control panel Settings > Personal information > Primary profile > Reseller ID",
            ...
            if (strtolower($status) == "active") {
                ...
                if (
                    ...
                    if (
                        Capsule::table("hb_domains")
                            ->where("id", $domainid)
                            ->update(["status" => "Pending Registration"])
                    ) {
                        $this->addInfo(
                            "Status has been updated to Pending Registration"
                        );
                    }
                }
            }
            if (in_array(strtolower($status), ["active", "active*"])) {
                # Update start date
    ...
    public function registerDomain(Registrar_Domain $domain)
    {
        ...
        if (sprintf($userinfoResult->result->resultCode) != 1000) {
            ...
            if (sprintf($usercreate->result->resultCode) != 1000) {
                $this->addError(
                    "User create: " . sprintf($usercreate->result->resultMsg)
                );
                return false;
            }
        }
        ...
        if (sprintf($registerResult->result->resultCode) == 1000) {
            ...
            if (
                preg_match(
                    ...
                    sprintf($registerResult->result->resultMsg)
                )
            ) {
                if (
                    Capsule::table("hb_domains")
                    ...
                    $this->addInfo(
                        "Domain has been registered, but domain name activation is waiting for documents to be uploaded"
                    );
                    return true;
                }
            } elseif (
                sprintf($registerResult->result->resultMsg) ==
                "Order with the same ID already created"
            ) {
                if (
                    Capsule::table("hb_domains")
                        ->where("id", $domainid)
                    ...
                    $this->addInfo(
                        "Domain has been registered, please Synchronize this order"
                    );
                    return true;
                }
            } else {
    ...
    public function modifyContact(Registrar_Domain $domain)
    {
        ...
        $cdetails = $this->_getDefaultContactDetails(
            $domain,
            $customer["customerid"]
        );
    ...
    public function transferDomain(Registrar_Domain $domain)
    {
        ...
        if (sprintf($geteppResult->result->resultCode) == 1000) {
            ...
            $postfields = [
                "domain" => $this->options["sld"] . "." . $this->options["tld"],
                "transfersecret" => base64_encode(
                    sprintf($geteppResult->resultData->epp)
                ),
                "periode" => $this->options["numyears"],
            ...
            $domaintransferResult = $this->_callApi(
                "domain/transfer",
                $postfields
            );
            if (sprintf($domaintransferResult->result->resultCode) == 1000) {
                ...
                $this->addError(
                    sprintf($domaintransferResult->result->resultMsg)
                );
            }
        } else {
    ...
    public function getEpp(Registrar_Domain $domain)
    {
        ...
        if (sprintf($domaingeteppResult->result->resultCode) == 1000) {
            ...
            Capsule::table("hb_domains")
                ->where("id", $domainid)
                ->update([
                    "epp_code" => sprintf($domaingeteppResult->resultData->epp),
                ]);
            $this->addInfo(
                "EPP Code: " . sprintf($domaingeteppResult->resultData->epp)
            );
            return true;
        } else {
    ...
    private function _getCustomerDetails()
    {
        ...
        if (sprintf($contactinfoResult->result->resultCode) == 1000) {
            $contact = [
                "contactid" => sprintf(
                    $contactinfoResult->resultData->contactid
                ),
                "userid" => sprintf($contactinfoResult->resultData->userid),
                "nickhandle" => sprintf(
                    $contactinfoResult->resultData->nickhandle
                ),
                ...
                "companyname" => sprintf(
                    $contactinfoResult->resultData->company
                ),
                ...
                "phonenumber" => sprintf(
                    $contactinfoResult->resultData->phonenumber
                ),
    ...
    private function _createCustomer()
    {
        ...
        if (sprintf($contactinfoResult->result->resultCode) == 1000) {
            ...
            $contactupdateResult = $this->_callApi(
                "contact/create",
                $postfields
            );
            if ($contactupdateResult) {
                ...
                $this->addError(
                    sprintf($contactupdateResult->result->resultMsg)
                );
                return false;
            }
        } else {
    ...
    private function _getDefaultContactDetails(
