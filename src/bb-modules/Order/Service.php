...
class Service implements InjectionAwareInterface
{
    ...
    public function counter()
    {
        ...
        return [
            ...
            \Model_ClientOrder::STATUS_PENDING_SETUP => $this->di["array_get"](
                $data,
                \Model_ClientOrder::STATUS_PENDING_SETUP,
                0
            ),
            \Model_ClientOrder::STATUS_FAILED_SETUP => $this->di["array_get"](
                $data,
                \Model_ClientOrder::STATUS_FAILED_SETUP,
                0
            ),
            \Model_ClientOrder::STATUS_ACTIVE => $this->di["array_get"](
                $data,
                \Model_ClientOrder::STATUS_ACTIVE,
                0
            ),
            \Model_ClientOrder::STATUS_SUSPENDED => $this->di["array_get"](
                $data,
                \Model_ClientOrder::STATUS_SUSPENDED,
                0
            ),
            \Model_ClientOrder::STATUS_CANCELED => $this->di["array_get"](
                $data,
    ...
    public static function onAfterAdminOrderActivate(\Box_Event $event)
    {
        ...
        try {
            $order = $di["db"]->getExistingModelById(
                "ClientOrder",
                $order_id,
                "Order not found"
            );
            ...
            $email["code"] = sprintf(
                "mod_service%s_activated",
                $orderArr["service_type"]
            );
            $email["service"] = $s;
    ...
    {
        $type = $this->di["tools"]->from_camel_case(
            str_replace("Model_Service", "", get_class($service))
        );
        ...
        return $this->di["db"]->findOne(
            "ClientOrder",
            "service_type  = :service_type AND service_id = :service_id",
            $bindings
        );
    }
    ...
    public function getSoonExpiringActiveOrdersQuery($data = [])
    {
        ...
        $days_until_expiration = $systemService->getParamValue(
            "invoice_issue_days_before_expire",
            14
        );
        ...
        $query .=
            " HAVING DATEDIFF(expires_at, NOW()) <= :days_until_expiration ORDER BY client_id DESC";
    ...
    public function toApiArray(
        ...
        $data["meta"] = $this->di["db"]->getAssoc(
            "SELECT name, value FROM client_order_meta WHERE client_order_id = :id",
            [":id" => $model->id]
        );
        $data[
            "active_tickets"
        ] = $supportService->getActiveTicketsCountForOrder($model);
    ...
    public function getSearchQuery($data)
    {
        ...
        $ids =
            isset($data["ids"]) && is_array($data["ids"]) ? $data["ids"] : null;
        $meta =
    ...
    public function createOrder(
        ...
        if (!$currency instanceof \Model_Currency) {
            throw new \Box_Exception(
                "Currency could not be determined for order"
            );
        }

        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminOrderCreate",
            "params" => $data,
            "subject" => $product->type,
        ]);

        $period =
            isset($data["period"]) && !empty($data["period"])
                ? $data["period"]
                : null;
        ...
        $invoiceOption = $this->di["array_get"](
            $data,
            "invoice_option",
            "no-invoice"
        );
        $skipValidation = (bool) $this->di["array_get"](
            $data,
            "skip_validation",
            false
        );
        ...
        if ($product->is_addon && empty($group_id)) {
            throw new \Box_Exception(
                "Group ID parameter is missing for addon product order",
                null,
                832
            );
        }
        ...
        if (isset($data["meta"]) && is_array($data["meta"])) {
            foreach ($data["meta"] as $k => $v) {
                $mm = $this->di["db"]->findOne(
                    "client_order_meta",
                    "client_order_id = :id AND name = :n",
                    [":id" => $order->id, ":n" => $k]
                );
                if (!$mm) {
    ...
    public function getMasterOrderForClient(\Model_Client $client, $group_id)
    {
        ...
        return $this->di["db"]->findOne(
            "ClientOrder",
            "group_id = :group_id AND group_master = 1 AND client_id = :client_id",
            $bindings
        );
    }
    ...
    public function activateOrderAddons(\Model_ClientOrder $order)
    {
        ...
        foreach ($list as $addon) {
            try {
                $this->di["events_manager"]->fire([
                    "event" => "onBeforeAdminOrderActivate",
                    "params" => ["id" => $addon->id],
                ]);
                $this->createFromOrder($addon);
                $this->di["events_manager"]->fire([
                    "event" => "onAfterAdminOrderActivate",
                    "params" => ["id" => $addon->id],
                ]);
            } catch (\Exception $e) {
                error_log($e->getMessage());
    ...
    public function activateOrder(\Model_ClientOrder $order, $data = [])
    {
        ...
        if (!in_array($order->status, $statues)) {
            if (!isset($data["force"]) || !$data["force"]) {
                throw new \Box_Exception(
                    "Only pending setup or failed orders can be activated"
                );
            }
        }
    ...
    public function createFromOrder(\Model_ClientOrder $order)
    {
        ...
        if (!is_object($service)) {
            ...
            $s = $mod->getService();
            if (
                method_exists($s, "create") ||
                method_exists($s, "action_create")
            ) {
                $service = $this->_callOnService(
                    $order,
        ...
        if (!empty($order->period)) {
            ...
            $order->expires_at = date(
                "Y-m-d H:i:s",
                $period->getExpirationTime($from_time)
            );
        }
        ...
        if ($productModel instanceof \Model_Product) {
            ...
            error_log(
                sprintf(
                    "Order without product ID detected Order #%s",
                    $order->id
                )
            );
        }
    ...
    {
        return $this->di["db"]->find(
            "ClientOrder",
            "group_id = :group_id AND client_id = :client_id and (group_master = 0 OR group_master IS NULL)",
            [":group_id" => $order->group_id, ":client_id" => $order->client_id]
        );
    }
    ...
    protected function _callOnService(\Model_ClientOrder $order, $action)
    {
        ...
        if (in_array($order->service_type, $core_services)) {
            ...
            if (!method_exists($repo, $m) || !is_callable([$repo, $m])) {
                throw new \Box_Exception(
                    "Service " . $order->service_type . " do not support " . $m
                );
            }
            ...
            $o = $this->di["db"]->findOne("client_order", "id = :id", [
                ":id" => $order->id,
            ]);
            $service = null;
            ...
            if (
                method_exists($repo, $action) &&
                is_callable([$repo, $action])
            ) {
    ...
    public function updateOrderMeta(\Model_ClientOrder $order, $meta)
    {
        ...
        if (empty($meta)) {
            $this->di["db"]->exec(
                "DELETE FROM client_order_meta WHERE client_order_id = :id;",
                [":id" => $order->id]
            );
            return 1;
        }
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminOrderUpdate",
            "params" => $data,
        ]);
        $this->updatePeriod(
            $order,
            $this->di["array_get"]($data, "period", null)
        );
        ...
        if (!empty($activated_at)) {
            $order->activated_at = date(
                "Y-m-d H:i:s",
                strtotime($activated_at)
            );
        }
        ...
        $order->invoice_option = $this->di["array_get"](
            $data,
            "invoice_option",
            $order->invoice_option
        );
        $order->title = $this->di["array_get"]($data, "title", $order->title);
        $order->price = $this->di["array_get"]($data, "price", $order->price);
        $order->status = $this->di["array_get"](
            $data,
            "status",
            $order->status
        );
        $order->notes = $this->di["array_get"]($data, "notes", $order->notes);
        $order->reason = $this->di["array_get"](
            $data,
            "reason",
            $order->reason
        );
        ...
        $this->updateOrderMeta(
            $order,
            $this->di["array_get"]($data, "meta", null)
        );
    ...
    public function renewFromOrder(\Model_ClientOrder $order)
    {
        ...
        if (!empty($order->period)) {
            $from_time =
                null === $order->expires_at
                    ? time()
                    : strtotime($order->expires_at); // from expiration date
    ...
    public function suspendFromOrder(
        ...
        if (!$skipEvent) {
            $this->di["events_manager"]->fire([
                "event" => "onBeforeAdminOrderSuspend",
                "params" => ["id" => $order->id],
            ]);
        }
        ...
        $note =
            null === $reason
                ? "Order suspeded"
                : "Order suspended for " . $reason;
        $this->saveStatusChange($order, $note);

        if (!$skipEvent) {
            $this->di["events_manager"]->fire([
                "event" => "onAfterAdminOrderSuspend",
                "params" => ["id" => $order->id],
            ]);
        }
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminOrderUnsuspend",
            "params" => ["id" => $order->id],
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminOrderUnsuspend",
            "params" => ["id" => $order->id],
        ]);
    ...
    public function cancelFromOrder(
        ...
        if (!$skipEvent) {
            $this->di["events_manager"]->fire([
                "event" => "onBeforeAdminOrderCancel",
                "params" => ["id" => $order->id],
            ]);
        }
        ...
        if (
            ...
            throw new \Box_Exception(
                "Can not cancel " . $order->status . " order"
            );
        }
        ...
        $note =
            null === $reason
                ? "Order canceled"
                : "Canceled order for " . $reason;
        $this->saveStatusChange($order, $note);

        if (!$skipEvent) {
            $this->di["events_manager"]->fire([
                "event" => "onAfterAdminOrderCancel",
                "params" => ["id" => $order->id],
            ]);
        }
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminOrderUncancel",
            "params" => ["id" => $order->id],
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminOrderUncancel",
            "params" => ["id" => $order->id],
        ]);
    ...
    public function rmInvoiceItemByOrder(\Model_ClientOrder $order)
    {
        ...
        $items = $this->di["db"]->find(
            "InvoiceItem",
            "rel_id = :rel_id AND status = :status",
            $bindings
        );
    ...
    {
        $this->di["db"]->exec(
            "Delete FROM client_order_status WHERE client_order_id = :client_order_id",
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminOrderDelete",
            "params" => ["id" => $order->id],
        ]);
    ...
    public function getExpiredOrders()
    {
        ...
        return $this->di["db"]->find(
            "ClientOrder",
            "status = :status AND expires_at IS NOT NULL AND DATEDIFF(NOW(), expires_at) >= 1 ORDER BY id",
            $bindings
        );
    }
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminBatchSuspendOrders",
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminBatchSuspendOrders",
        ]);
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminBatchCancelSuspendedOrders",
        ]);

        $mod = $this->di["mod"]("order");
        $config = $mod->getConfig();
        if (
            !isset($config["batch_cancel_suspended"]) ||
            !$config["batch_cancel_suspended"]
        ) {
            return false;
        }

        $reason = $this->di["array_get"](
            $config,
            "batch_cancel_suspended_reason",
            null
        );
        $days = isset($config["batch_cancel_suspended_after_days"])
            ? (int) $config["batch_cancel_suspended_after_days"]
            : 7;
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminBatchCancelSuspendedOrders",
        ]);
    ...
    public function orderStatusAdd(
        ...
        $this->di["logger"]->info(
            "Added order status history message to order #%s",
            $id
        );
    ...
    {
        $orderStatus = $this->di["db"]->getExistingModelById(
            "ClientOrderStatus",
            $id,
            "Order history line not found"
        );
    ...
    public function getOrderServiceData(
        \Model_ClientOrder $order,
        $identity = null
    ) {
        $orderId = $order->id;
        ...
        if (!method_exists($srepo, "toApiArray")) {
            error_log(
                sprintf(
                    "service #%s method toApiArray is missing",
                    $order->service_type
                )
            );
    ...
    public function setUnpaidInvoice(
        \Model_ClientOrder $order,
        \Model_Invoice $proforma
    ) {
    ...
    public function getRelatedOrderIdByType(\Model_ClientOrder $order, $type)
    {
        ...
        $o = $this->di["db"]->findOne(
            "ClientOrder",
            "group_id = :group_id AND service_type = :service_type",
            $bindings
        );
