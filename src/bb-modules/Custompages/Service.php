...
class Service
{
    ...
    public function deletePage($id)
    {
        if (is_array($id)) {
            ...
            $this->di["pdo"]->query(
                "DELETE from custom_pages WHERE id in (" . join(", ", $id) . ")"
            );
        } else {
            $this->di["pdo"]
                ->prepare("DELETE from custom_pages WHERE id = ?")
                ->execute([$id]);
        }
    }
    ...
    public function createPage($title, $description, $keywords, $content)
    {
        ...
        $this->di["pdo"]
            ...
            ->execute([$title, $description, $keywords, $content, $slug]);
    ...
    public function updatePage(
        ...
        if ($ex->rowCount() > 0) {
            die(
                json_encode([
                    "result" => null,
                    "error" => [
                        "message" => "You need to set unique slug.",
                        "code" => 9999,
                    ],
                ])
            );
        ...
        $this->di["pdo"]
            ...
            ->execute([$title, $description, $keywords, $content, $slug, $id]);
