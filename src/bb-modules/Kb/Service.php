...
class Service
{
    ...
    {
        return $this->di["db"]->find("KbArticle", "status=:status", [
            ":status" => \Model_KbArticle::ACTIVE,
        ]);
    }
    ...
    public function toApiArray(
        ...
        $cat = $this->di["db"]->getExistingModelById(
            "KbArticleCategory",
            $model->kb_article_category_id,
    ...
    public function createArticle(
        ...
        $this->di["logger"]->info(
            "Created new knowledge base article #%s",
            $id
        );
    ...
    public function categoryRm(\Model_KbArticleCategory $model)
    {
        ...
        $articlesCount = $this->di["db"]->getCell(
            "SELECT count(*) as cnt FROM kb_article WHERE kb_article_category_id = :kb_article_category_id",
            $bindings
        );

        if ($articlesCount > 0) {
            throw new \Box_Exception(
                "Can not remove category which has articles"
            );
        }
    ...
    public function categoryToApiArray(
        \Model_KbArticleCategory $model,
        $identity = null,
        $query = null
    ) {
    ...
    public function createCategory($title, $description = null)
    {
        ...
        $this->di["logger"]->info(
            "Created new knowledge base category #%s",
            $id
        );
    ...
    public function updateCategory(
        ...
        $this->di["logger"]->info(
            "Updated knowledge base category #%s",
            $model->id
        );
