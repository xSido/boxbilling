...
class Service implements InjectionAwareInterface
{
    ...
    public function addItem(
        ...
        $event_params = array_merge($data, [
            "cart_id" => $cart->id,
            "product_id" => $product->id,
        ]);
        $this->di["events_manager"]->fire([
            "event" => "onBeforeProductAddedToCart",
            "params" => $event_params,
        ]);
        ...
        if ($this->isRecurrentPricing($product)) {
            ...
            if (!$this->isPeriodEnabledForProduct($product, $data["period"])) {
                throw new \Box_Exception(
                    "Selected billing period is not valid"
                );
            }
        }
        ...
        if (method_exists($productService, "getDomainProductFromConfig")) {
            ...
            if (
                isset($dc["config"]) &&
                $dc["product"] &&
                $dc["product"] instanceof \Model_Product
            ) {
                $list[] = $dc;
            }
        }
            ...
            if (isset($ac["selected"]) && (bool) $ac["selected"]) {
                ...
                if ($addon instanceof \Model_Product) {
                    if ($this->isRecurrentPricing($addon)) {
                        ...
                        if (
                            !$this->isPeriodEnabledForProduct(
                                $addon,
                                $ac["period"]
                            )
                        ) {
                            throw new \Box_Exception(
                                "Selected billing period is not valid for addon"
                            );
                        }
                    }
        ...
        foreach ($list as $c) {
            ...
            if (method_exists($productServiceFromList, "prependOrderConfig")) {
                $productFromListConfig = $productServiceFromList->prependOrderConfig(
                    $productFromList,
                    $productFromListConfig
                );
            }

            if (method_exists($productServiceFromList, "attachOrderConfig")) {
                $model = $this->di["db"]->load("Product", $productFromList->id);
                $productFromListConfig = $productServiceFromList->attachOrderConfig(
                    $model,
                    $productFromListConfig
                );
            }
            if (method_exists($productServiceFromList, "validateOrderData")) {
                $productServiceFromList->validateOrderData(
                    $productFromListConfig
                );
            }
            if (method_exists($productServiceFromList, "validateCustomForm")) {
                $productServiceFromList->validateCustomForm(
                    $productFromListConfig,
                    $this->di["db"]->toArray($productFromList)
                );
            }
        ...
        $this->di["logger"]->info(
            'Added "%s" to shopping cart',
            $product->title
        );

        $this->di["events_manager"]->fire([
            "event" => "onAfterProductAddedToCart",
            "params" => $event_params,
        ]);
    ...
    public function isRecurrentPricing(\Model_Product $model)
    {
        ...
        return isset($pricing["type"]) &&
            $pricing["type"] == \Model_ProductPayment::RECURRENT;
    }
    ...
    public function removeProduct(\Model_Cart $cart, $id, $removeAddons = true)
    {
        ...
        $cartProduct = $this->di["db"]->findOne(
            "CartProduct",
            "id = :id AND cart_id = :cart_id",
            $bindings
        );
        ...
        if ($removeAddons) {
            $allCartProducts = $this->di["db"]->find(
                "CartProduct",
                "cart_id = :cart_id",
                [":cart_id" => $cart->id]
            );
            foreach ((array) $allCartProducts as $cProduct) {
                ...
                if (
                    isset($config["parent_id"]) &&
                    $config["parent_id"] == $cartProduct->product_id
                ) {
                    $this->di["db"]->trash($cProduct);
                    $this->di["logger"]->info(
                        "Removed product addon from shopping cart"
                    );
                }
            }
        }
    ...
    public function changeCartCurrency(
        ...
        $this->di["logger"]->info(
            "Changed shopping cart #%s currency to %s",
            $cart->id,
            $currency->title
        );
    ...
    public function removePromo(\Model_Cart $cart)
    {
        ...
        $this->di["logger"]->info(
            "Removed promo code from shopping cart #%s",
            $cart->id
        );
    ...
    public function applyPromo(\Model_Cart $cart, \Model_Promo $promo)
    {
        ...
        if ($this->isEmptyCart($cart)) {
            throw new \Box_Exception(
                "Add products to cart before applying promo code"
            );
        }
        ...
        $this->di["logger"]->info(
            "Applied promo code %s to shopping cart",
            $promo->code
        );
    ...
    public function toApiArray(
        ...
        $currency = $this->di["db"]->getExistingModelById(
            "Currency",
            $model->currency_id
        );
        ...
        if ($model->promo_id) {
            $promo = $this->di["db"]->getExistingModelById(
                "Promo",
                $model->promo_id,
                "Promo not found"
            );
    ...
    public function isClientAbleToUsePromo(
        \Model_Client $client,
        \Model_Promo $promo
    ) {
    ...
    protected function clientHadUsedPromo(
        \Model_Client $client,
        \Model_Promo $promo
    ) {
        $sql =
            "SELECT id FROM client_order WHERE promo_id = :promo AND client_id = :cid LIMIT 1";
        $promoId = $this->di["db"]->getCell($sql, [
            ":promo" => $promo->id,
            ":cid" => $client->id,
        ]);
    ...
    public function getCartProducts(\Model_Cart $model)
    {
        return $this->di["db"]->find(
            "CartProduct",
            "cart_id = :cart_id ORDER BY id ASC",
            [":cart_id" => $model->id]
        );
    }
    ...
    public function checkoutCart(
        ...
        if ($cart->promo_id) {
            ...
            if (!$this->isClientAbleToUsePromo($client, $promo)) {
                throw new \Box_Exception(
                    "You have already used this promo code. Please remove promo code and checkout again.",
                    null,
                    9874
                );
            }
        ...
        [$order, $invoice, $orders] = $this->createFromCart(
            $client,
            $gateway_id
        );
        ...
        if (
            $invoice instanceof \Model_Invoice &&
    ...
    public function createFromCart(\Model_Client $client, $gateway_id = null)
    {
        ...
        if ($client->currency != $currency->code) {
            throw new \Box_Exception(
                "Selected currency :selected does not match your profile currency :code. Please change cart currency to continue.",
                [":selected" => $currency->code, ":code" => $client->currency]
            );
        }
        ...
        foreach ($this->getCartProducts($cart) as $p) {
            ...
            $order->discount =
                $item["discount_price"] * $currency->conversion_rate;
            ...
            if ($item["setup_price"] > 0) {
                $setup_price =
                    $item["setup_price"] * $currency->conversion_rate -
                    $item["discount_setup"] * $currency->conversion_rate;
                $invoice_items[] = [
                    "title" => __(":product setup", [
                        ":product" => $order->title,
                    ]),
                    "price" => $setup_price,
                    "quantity" => 1,
        ...
        if ($ca["total"] > 0) {
            //crete invoice if order total > 0

            $invoiceService = $this->di["mod_service"]("Invoice");
            $invoiceModel = $invoiceService->prepareInvoice($client, [
                "client_id" => $client->id,
                "items" => $invoice_items,
                "gateway_id" => $gateway_id,
            ]);

            $clientBalanceService = $this->di["mod_service"](
                "Client",
                "Balance"
            );
            ...
            $invoiceService->approveInvoice($invoiceModel, [
                "id" => $invoiceModel->id,
        ...
        foreach ($orders as $order) {
            ...
            try {
                ...
                if (
                    $ca["total"] <= 0 &&
                    $product->setup ==
                        \Model_ProductTable::SETUP_AFTER_PAYMENT &&
                    $oa["total"] - $oa["discount"] <= 0
                ) {
                    $orderService->activateOrder($order);
                }
                ...
                $status = "error";
                $notes =
                    "Order could not be activated after checkout due to error: " .
                    $e->getMessage();
    ...
    public function findActivePromoByCode($code)
    {
        return $this->di["db"]->findOne(
            "Promo",
            "code = :code AND active = 1 ORDER BY id ASC",
            [":code" => $code]
        );
    }
    ...
    protected function getItemPromoDiscount(
        \Model_CartProduct $model,
        \Model_Promo $promo
    ) {
        $product = $this->di["db"]->load("Product", $model->product_id);
