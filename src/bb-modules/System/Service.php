...
class Service
{
    ...
    public function getCompany()
    {
        ...
        return [
            ...
            "signature" => $this->di["array_get"](
                $results,
                "company_signature",
                null
            ),
            "logo_url" => $this->di["array_get"](
                $results,
                "company_logo",
                null
            ),
            "address_1" => $this->di["array_get"](
                $results,
                "company_address_1",
                null
            ),
            "address_2" => $this->di["array_get"](
                $results,
                "company_address_2",
                null
            ),
            "address_3" => $this->di["array_get"](
                $results,
                "company_address_3",
                null
            ),
            "account_number" => $this->di["array_get"](
                $results,
                "company_account_number",
                null
            ),
            "number" => $this->di["array_get"](
                $results,
                "company_number",
                null
            ),
            "note" => $this->di["array_get"]($results, "company_note", null),
            "privacy_policy" => $this->di["array_get"](
                $results,
                "company_privacy_policy",
                null
            ),
            "tos" => $this->di["array_get"]($results, "company_tos", null),
            "vat_number" => $this->di["array_get"](
                $results,
                "company_vat_number",
                null
            ),
    ...
    public function updateParams($data)
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminSettingsUpdate",
            "params" => $data,
        ]);
    ...
    public function getMessages($type)
    {
        ...
        try {
            ...
            if ($updater->getCanUpdate()) {
                ...
                $msgs["info"][] = sprintf(
                    "BoxBilling %s is available for download.",
                    $version
                );
            }
        } catch (\Exception $e) {
        ...
        if (!$last_exec) {
            $msgs["info"][] =
                "Cron was never executed. Make sure you have setup cron job.";
        }

        $install = BB_PATH_ROOT . "/install";
        if ($this->di["tools"]->fileExists(BB_PATH_ROOT . "/install")) {
            $msgs["info"][] = sprintf(
                'Install module "%s" still exists. Please remove it for security reasons.',
                $install
            );
        }
        ...
        if ($this->getVersion() == "0.0.1") {
            $msgs["info"][] =
                'BoxBilling couldn\'t find valid version information. This is okay if you downloaded BoxBilling directly from the master branch, instead of a released version. But beware, the master branch may not be stable enough for production use.';
        }

        if (!extension_loaded("openssl")) {
            $msgs["info"][] = sprintf(
                "BoxBilling requires %s extension to be enabled on this server for security reasons.",
                "php openssl"
            );
        }
        ...
        if (isset($vars["_client_id"])) {
            ...
            if ($identity instanceof \Model_Client) {
                try {
                    ...
                    error_log(
                        "api_client could not be added to template: " .
                            $e->getMessage()
                    );
                }
            }
    ...
    public function getPublicParamValue($param)
    {
        ...
        $results = $stmt->fetchColumn();
        if ($results === false) {
            throw new \Box_Exception("Parameter :param does not exist", [
                ":param" => $param,
            ]);
        }
        return $results;
        ...
        if (isset($data["country"])) {
            if (array_key_exists($data["country"], $codes)) {
                ...
                throw new \Box_Exception(
                    "Country :code phone code is not registered",
                    [":code" => $data["country"]]
                );
            }
        }
    ...
    {
        $query =
            "SELECT param, value FROM setting WHERE param IN ('nameserver_1', 'nameserver_2', 'nameserver_3', 'nameserver_4')";
