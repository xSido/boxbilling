...
class Service implements InjectionAwareInterface
{
    ...
    public function isExtensionActive($type, $id)
    {
        ...
        $id_or_null = $this->di["db"]->getCell($query, [
            "type" => $type,
            "id" => $id,
        ]);
    ...
    public function getExtensionsList($filter)
    {
        ...
        $search = isset($filter["search"])
            ? strtolower($filter["search"])
            : null;
        $result = [];
            ...
            if (ctype_alnum($name)) {
                ...
                if (!$mod->hasManifest()) {
                    error_log(
                        "Module " .
                            $m .
                            " manifest file is missing or is not readable."
                    );
                    continue;
                }
    ...
    public function getAdminNavigation($admin, $url = null)
    {
        ...
        foreach ($subpages as $page) {
            ...
            if (!isset($nav[$page["location"]])) {
                error_log(
                    "Submenu item belongs to not existing location: " .
                        $page["location"]
                );
                continue;
            }
        ...
        foreach ($nav as &$group) {
            $group["subpages"] = $this->di["tools"]->sortByOneKey(
                $group["subpages"],
                "index"
            );
        }
        ...
        if ($model->type == "mod") {
            ...
            if (empty($latest)) {
                throw new \Box_Exception(
                    "Could not retrieve version information for extension :ext",
                    [":ext" => $model->name],
                    745
                );
            }
            ...
            if (!$haveUpdate) {
                throw new \Box_Exception(
                    "Latest :mod version installed. No need to update",
                    [":mod" => $model->name],
                    785
                );
            }
            ...
            if (empty($extension)) {
                throw new \Box_Exception(
                    "Could not retrieve :ext information",
                    [":ext" => $model->name],
                    744
                );
            }
            ...
            throw new \Box_Exception(
                "Visit extension site for update information.",
                null,
                252
            );
        ...
        switch ($ext->type) {
            ...
            case \Box_Extension::TYPE_MOD:
                ...
                if ($this->isCoreModule($mod)) {
                    throw new \Box_Exception(
                        "BoxBilling core modules can not be managed"
                    );
                }
        ...
        switch ($type) {
            case \Box_Extension::TYPE_MOD:
                ...
                if (!$this->di["tools"]->rename($extracted, $destination)) {
                    throw new \Box_Exception(
                        "Extension can not be moved. Make sure your server write permissions to bb-modules folder.",
                        null,
                        437
                    );
                }
            ...
            case \Box_Extension::TYPE_THEME:
                ...
                if (!$this->di["tools"]->fileExists($destination)) {
                    if (!$this->di["tools"]->rename($extracted, $destination)) {
                        throw new \Box_Exception(
                            "Extension can not be moved. Make sure your server write permissions to bb-themes folder.",
                            null,
                            439
                        );
                    }
            ...
            case \Box_Extension::TYPE_TRANSLATION:
                ...
                if (!$this->di["tools"]->rename($extracted, $destination)) {
                    throw new \Box_Exception(
                        "Extension can not be moved. Make sure your server write permissions to bb-locale folder.",
                        null,
                        440
                    );
                }
            ...
            default:
                throw new \Box_Exception(
                    "Extension does not support auto-install feature. Extension must be installed manually"
                );
        }
    ...
    private function installModule(\Model_Extension $ext)
    {
        ...
        if ($mod->isCore()) {
            throw new \Box_Exception(
                "BoxBilling core modules can not be installed or removed"
            );
        }

        $info = $mod->getManifest();
        if (
            isset($info["minimum_boxbilling_version"]) &&
            Box_Version::compareVersion($info["minimum_boxbilling_version"]) > 0
        ) {
            throw new \Box_Exception(
                "Module can not be installed. It requires at least :min version of BoxBilling. You are using :v",
                [
                    ":min" => $info["minimum_boxbilling_version"],
                    ":v" => Box_Version::VERSION,
                ]
            );
        }
    ...
    public function activateExistingExtension($data)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminActivateExtension",
            "params" => ["id" => $ext_id],
        ]);
        try {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminActivateExtension",
            "params" => ["id" => $ext_id],
        ]);
    ...
    public function setConfig($data)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminExtensionConfigSave",
            "params" => $data,
        ]);
        $sql = "
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminExtensionConfigSave",
            "params" => $data,
        ]);
        $this->di["logger"]->info(
            'Updated extension "%s" configuration',
            $data["ext"]
        );
