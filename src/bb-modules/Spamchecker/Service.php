...
class Service implements InjectionAwareInterface
{
    ...
    public function isBlockedIp($event)
    {
        ...
        if (
            isset($config["block_ips"]) &&
            $config["block_ips"] &&
            isset($config["blocked_ips"])
        ) {
    ...
    public function isSpam(\Box_Event $event)
    {
        ...
        $data = [
            ...
            "recaptcha_challenge_field" => $this->di["array_get"](
                $params,
                "recaptcha_challenge_field",
                null
            ),
            "recaptcha_response_field" => $this->di["array_get"](
                $params,
        ...
        if (isset($config["captcha_enabled"]) && $config["captcha_enabled"]) {
            if (
                isset($config["captcha_version"]) &&
                $config["captcha_version"] == 2
            ) {
                if (
                    !isset($config["captcha_recaptcha_privatekey"]) ||
                    $config["captcha_recaptcha_privatekey"] == ""
                ) {
                    throw new \Box_Exception(
