...
class Client implements InjectionAwareInterface
{
    ...
    {
        $exc = new \Box_Exception(
            "Unknown API call :call",
            [":call" => $page],
            879
        );
    ...
    private function checkRateLimit($method = null)
    {
        ...
        $requests = $service->getRequestCount(
            time() - $rate_span,
            $this->_getIp(),
            $isLoginMethod
        );
    ...
    private function checkHttpReferer()
    {
        ...
        $check_referer_header = isset(
            $this->_api_config["require_referrer_header"]
        )
            ? (bool) $this->_api_config["require_referrer_header"]
            : false;
        if ($check_referer_header) {
            ...
            if (!$referer || $url != substr($referer, 0, strlen($url))) {
                throw new \Box_Exception(
                    "Invalid request. Make sure request origin is :from",
                    [":from" => BB_URL],
                    1004
                );
            }
    ...
    private function _tryTokenLogin()
    {
        ...
        switch ($username) {
            case "client":
                $model = $this->di["db"]->findOne("Client", "api_token = ?", [
                    $password,
                ]);
            ...
            case "admin":
                $model = $this->di["db"]->findOne("Admin", "api_token = ?", [
                    $password,
                ]);
