...
class Server implements \Box\InjectionAwareInterface
{
    ...
    public function handle_deprecated($data)
    {
        try {
            ...
            $this->_log->info($e->getMessage() . " " . $e->getCode());
            $this->_result["error"] =
                "Licensing server is temporary unavailable";
            $this->_result["error_code"] = $e->getCode();
        }
    ...
    public function process($data)
    {
        ...
        if (!isset($data["license"]) || empty($data["license"])) {
            throw new \LogicException(
                "License key is not present in call",
                1001
            );
        }
        ...
        if (!isset($data["version"]) || empty($data["version"])) {
            throw new \LogicException(
                "Version key is not present in call",
                1003
            );
        }
        ...
        if (!$service->isValidIp($model, $ip)) {
            throw new \LogicException(
                sprintf('Ip "%s" is not allowed for this license', $ip),
                1007
            );
        }
        ...
        if (!$service->isValidHost($model, $host)) {
            throw new \LogicException(
                sprintf('Host "%s" is not allowed for this license', $host),
                1008
            );
        }
        ...
        if (!$service->isValidVersion($model, $version)) {
            throw new \LogicException(
                sprintf('Version "%s" is not valid for this license', $version),
                1009
            );
        }
        ...
        if (!$service->isValidPath($model, $path)) {
            throw new \LogicException(
                sprintf(
                    'Software install path "%s" is not valid for this license',
                    $path
                ),
                1010
            );
