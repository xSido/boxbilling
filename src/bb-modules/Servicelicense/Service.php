...
class Service implements InjectionAwareInterface
{
    ...
    public function action_create(\Model_ClientOrder $order)
    {
        ...
        $model->validate_ip = (bool) $this->di["array_get"](
            $c,
            "validate_ip",
            false
        );
        $model->validate_host = (bool) $this->di["array_get"](
            $c,
            "validate_host",
            false
        );
        $model->validate_path = (bool) $this->di["array_get"](
            $c,
            "validate_path",
            false
        );
        $model->validate_version = (bool) $this->di["array_get"](
            $c,
            "validate_version",
            false
        );
    ...
    public function action_activate(\Model_ClientOrder $order)
    {
        ...
        if (!is_object($plugin)) {
            throw new \Box_Exception("License plugin :plugin was not found", [
                ":plugin" => $model->plugin,
            ]);
        }
        ...
        if (!method_exists($plugin, "generate")) {
            throw new \Box_Exception(
                "License plugin do not have generate method"
            );
        }
        ...
        do {
            ...
            if ($i++ >= $iterations) {
                throw new \Box_Exception(
                    "Maximum number of iterations reached while generating license key"
                );
            }
        } while (
            null !==
    ...
    public function reset(\Model_ServiceLicense $model)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeServicelicenseReset",
            "params" => $data,
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterServicelicenseReset",
            "params" => $data,
        ]);
        return true;
    }
    ...
    private function _getPlugin(\Model_ServiceLicense $model)
    {
        ...
        foreach ($plugins as $plugin) {
            if ($model->plugin == $plugin["filename"]) {
                ...
                $class_name =
                    "Box\\Mod\\Servicelicense\\Plugin\\" . $model->plugin;
                return new $class_name();
            }
        }
        error_log(
            sprintf(
                "License #%s plugin %s is not valid",
                $model->id,
                $model->plugin
            )
        );
    ...
    public function update(\Model_ServiceLicense $s, array $data)
    {
        ...
        $s->validate_ip = (bool) $this->di["array_get"](
            $data,
            "validate_ip",
            $s->validate_ip
        );
        $s->validate_host = (bool) $this->di["array_get"](
            $data,
            "validate_host",
            $s->validate_host
        );
        $s->validate_path = (bool) $this->di["array_get"](
            $data,
            "validate_path",
            $s->validate_path
        );
        $s->validate_version = (bool) $this->di["array_get"](
            $data,
            "validate_version",
            $s->validate_version
        );
