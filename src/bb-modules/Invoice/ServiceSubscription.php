...
class ServiceSubscription implements InjectionAwareInterface
{
    ...
    public function create(
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminSubscriptionCreate",
            "params" => ["id" => $newId],
        ]);
    ...
    public function update(\Model_Subscription $model, array $data)
    {
        ...
        $model->period = $this->di["array_get"](
            $data,
            "period",
            $model->period
        );
    ...
    public function toApiArray(
        ...
        if ($client instanceof \Model_Client) {
            ...
            $result["client"] = $clientService->toApiArray(
                $client,
                false,
                $identity
            );
        ...
        if ($gtw instanceof \Model_PayGateway) {
            $payGatewayService = $this->di["mod_service"](
                "Invoice",
                "PayGateway"
            );
            $result["gateway"] = $payGatewayService->toApiArray(
                $gtw,
                false,
                $identity
            );
    ...
    public function delete(\Model_Subscription $model)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminSubscriptionDelete",
            "params" => ["id" => $id],
        ]);
