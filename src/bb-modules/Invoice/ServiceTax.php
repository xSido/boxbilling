...
class ServiceTax implements InjectionAwareInterface
{
    ...
    public function getTaxRateForClient(\Model_Client $model, &$title = null)
    {
        ...
        $tax = $this->di["db"]->findOne("Tax", "state = ? and country = ?", [
            $model->state,
            $model->country,
        ]);
        ...
        $tax = $this->di["db"]->findOne("Tax", "country = ?", [
            $model->country,
        ]);
        if ($tax instanceof \Model_Tax) {
            $title = $tax->name;
        ...
        // find global rate
        $tax = $this->di["db"]->findOne(
            "Tax",
            '(state is NULL or state = "") and (country is null or country = "")'
        );
        if ($tax instanceof \Model_Tax) {
            $title = $tax->name;
            return $tax->taxrate;
