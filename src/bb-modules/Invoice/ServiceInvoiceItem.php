...
class ServiceInvoiceItem implements InjectionAwareInterface
{
        ...
        if ($item->type == \Model_InvoiceItem::TYPE_ORDER) {
            ...
            if (!$order instanceof \Model_ClientOrder) {
                throw new \Box_Exception(
                    "Could not activate proforma item. Order :id not found",
                    [":id" => $order_id]
                );
            }
            $orderService = $this->di["mod_service"]("Order");
            switch ($item->task) {
                case \Model_InvoiceItem::TASK_ACTIVATE:
                    ...
                    if (
                        $product->setup == \Model_Product::SETUP_AFTER_PAYMENT
                    ) {
                        try {
                            $orderService->activateOrder($order);
                        } catch (\Exception $e) {
                            error_log($e->getMessage());
                            $orderService->saveStatusChange(
                                $order,
                                "Order could not be activated due to error: " .
                                    $e->getMessage()
                            );
                        }
                    }
                ...
                case \Model_InvoiceItem::TASK_RENEW:
                    try {
                        ...
                        $orderService->saveStatusChange(
                            $order,
                            "Order could not renew due to error: " .
                                $e->getMessage()
                        );
                    }
                    break;
    ...
    public function addNew(\Model_Invoice $proforma, array $data)
    {
        ...
        $type = $this->di["array_get"](
            $data,
            "type",
            \Model_InvoiceItem::TYPE_CUSTOM
        );
        $rel_id = $this->di["array_get"]($data, "rel_id");
        $task = $this->di["array_get"](
            $data,
            "task",
            \Model_InvoiceItem::TASK_VOID
        );
        $status = $this->di["array_get"](
            $data,
            "status",
            \Model_InvoiceItem::STATUS_PENDING_PAYMENT
        );
    ...
    public function getTax(\Model_InvoiceItem $item)
    {
        ...
        $rate = $this->di["db"]->getCell(
            "SELECT taxrate FROM invoice WHERE id = :id",
            ["id" => $item->invoice_id]
        );
        if ($rate <= 0) {
            return 0;
    ...
    public function creditInvoiceItem(\Model_InvoiceItem $item)
    {
        ...
        $invoiceService->addNote(
            $invoice,
            sprintf(
                "Charged clients balance with %s %s for %s",
                $total,
                $invoice->currency,
                $item->title
            )
    ...
    public function generateFromOrder(
        ...
        if ($order->promo_recurring) {
            ...
            $discount_title = $this->_getTitleForPromoDiscount(
                $order->promo_id,
                $order->currency
            );
    ...
    private function _getTitleForPromoDiscount($promo_id, $currency)
    {
        ...
        switch ($promo->type) {
            case \Model_Promo::ABSOLUTE:
                $currencyAmount = $api_guest->currency_format([
                    "code" => $currency,
                    "price" => $promo->value,
                ]);
                return __("Promotional Code: :code - :value Discount", [
                    ":code" => $promo->code,
                    ":value" => $currencyAmount,
                ]);

            case \Model_Promo::PERCENTAGE:
                return __("Promotional Code: :code - :value%", [
                    ":code" => $promo->code,
                    ":value" => $promo->value,
                ]);
