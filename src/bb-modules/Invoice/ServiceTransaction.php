...
class ServiceTransaction implements InjectionAwareInterface
{
    ...
    {
        $this->di["logger"]->info(
            "Executed action to process received transactions"
        );
    ...
    public function update(\Model_Transaction $model, array $data)
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminTransactionUpdate",
            "params" => ["id" => $model->id],
        ]);

        $model->invoice_id = $this->di["array_get"](
            $data,
            "invoice_id",
            $model->invoice_id
        );
        $model->txn_id = $this->di["array_get"](
            $data,
            "txn_id",
            $model->txn_id
        );
        $model->txn_status = $this->di["array_get"](
            $data,
            "txn_status",
            $model->txn_status
        );
        ...
        $model->amount = $this->di["array_get"](
            $data,
            "amount",
            $model->amount
        );
        $model->currency = $this->di["array_get"](
            $data,
            "currency",
            $model->currency
        );
        ...
        $model->error_code = $this->di["array_get"](
            $data,
            "error_code",
            $model->error_code
        );
        $model->validate_ipn = $this->di["array_get"](
            $data,
            "validate_ipn",
            $model->validate_ipn
        );
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminTransactionUpdate",
            "params" => ["id" => $model->id],
        ]);
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminTransactionCreate",
            "params" => $data,
        ]);

        $skip_validation = isset($data["skip_validation"])
            ? (bool) $data["skip_validation"]
            : false;
        if (!$skip_validation) {
            ...
            $this->di["db"]->getExistingModelById(
                "Invoice",
                $data["bb_invoice_id"],
        ...
        $ipn = [
            "get" =>
                isset($data["get"]) && is_array($data["get"])
                    ? $data["get"]
                    : null,
            "post" =>
                isset($data["post"]) && is_array($data["post"])
                    ? $data["post"]
                    : null,
            "http_raw_post_data" => $this->di["array_get"](
                $data,
                "http_raw_post_data",
                null
            ),
        ...
        $transaction->gateway_id = $this->di["array_get"](
            $data,
            "bb_gateway_id",
            null
        );
        $transaction->invoice_id = $this->di["array_get"](
            $data,
            "bb_invoice_id",
            null
        );
        ...
        $this->di["logger"]->info(
            "Received transaction %s from payment gateway %s",
            $newId,
            $transaction->gateway_id
        );

        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminTransactionCreate",
            "params" => ["id" => $newId],
        ]);
    ...
    public function counter()
    {
        ...
        return [
            ...
            \Model_Transaction::STATUS_RECEIVED => isset(
                $data[\Model_Transaction::STATUS_RECEIVED]
            )
                ? $data[\Model_Transaction::STATUS_RECEIVED]
                : 0,
            \Model_Transaction::STATUS_APPROVED => isset(
                $data[\Model_Transaction::STATUS_APPROVED]
            )
                ? $data[\Model_Transaction::STATUS_APPROVED]
                : 0,
            \Model_Transaction::STATUS_PROCESSED => isset(
                $data[\Model_Transaction::STATUS_PROCESSED]
            )
                ? $data[\Model_Transaction::STATUS_PROCESSED]
                : 0,
            \Model_Transaction::STATUS_ERROR => isset(
                $data[\Model_Transaction::STATUS_ERROR]
            )
                ? $data[\Model_Transaction::STATUS_ERROR]
                : 0,
        ];
    ...
    public function preProcessTransaction(\Model_Transaction $model)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminTransactionProcess",
            "params" => ["id" => $model->id],
        ]);
    ...
    public function processTransaction($id)
    {
        ...
        if (empty($tx->gateway_id)) {
            throw new \Box_Exception(
                "Could not determine transaction origin. Transaction payment gateway is unknown.",
                null,
                701
            );
        }
        ...
        if (!$gtw instanceof \Model_PayGateway) {
            throw new \Box_Exception(
                "Can not handle transaction received from unknown payment gateway: :id",
                [":id" => $tx->gateway_id],
                704
            );
        }
        ...
        if (!method_exists($adapter, "processTransaction")) {
            throw new \Box_Exception(
                "Payment adapter :adapter does not support action :action",
                [":adapter" => $gtw->name, ":action" => "processTransaction"],
                705
            );
        }

        $ipn = json_decode($tx->ipn, 1);
        return $adapter->processTransaction(
            $this->di["api_system"],
            $id,
            $ipn,
            $tx->gateway_id
    ...
    public function process($tx)
    {
        ...
        try {
            ...
            switch ($transaction->type) {
                ...
                default:
                    throw new \Box_Exception(
                        "Unknown transaction #:id type: :type",
                        [
                            ":id" => $transaction->id,
    ...
    private function _isProcessed(\Model_Transaction $tx)
    {
        ...
        if ($this->hasProcessedTransaction($tx)) {
            $tx->note .=
                "Transaction was marked as processed. Transaction with same ID is already processed";
            $tx->updated_at = date("Y-m-d H:i:s");
            $this->di["db"]->store($tx);
    ...
    private function hasProcessedTransaction(\Model_Transaction $tx)
    {
        ...
        $res = $this->di["db"]->findOne(
            "Transaction",
            'status = "processed" and txn_id = ?',
            [$tx->txn_id]
        );
        return empty($res);
    }
    ...
    private function _parseIpnAndApprove(\Model_Transaction &$tx)
    {
        ...
        if (!$tx->invoice_id) {
            throw new \Box_Exception(
                "Transaction :id is not associated with an invoice.",
                [":id" => $tx->id],
                702
            );
        }
        ...
        if (APPLICATION_ENV != "testing" && $tx->validate_ipn) {
            if (!$adapter->isIpnValid($ipn, $mpi)) {
                ...
                throw new \Box_Exception(
                    "Instant payment notification (IPN) did not pass gateway :id validation",
                    [":id" => $gtw->gateway],
                    706
                );
            }
        ...
        if (!method_exists($adapter, "getTransaction")) {
            throw new \Box_Exception(
                "Payment adapter :adapter does not support action :action",
                [":adapter" => $gtw->name, ":action" => "getTransaction"],
                705
            );
        }
        ...
        if (!$response instanceof \Payment_Transaction) {
            throw new \Box_Exception(
                "Payment gateway :id method getTransaction should return Payment_Transaction object",
                [":id" => $gtw->gateway],
                705
            );
        }
    ...
    private function _subscribe(\Model_Transaction $tx)
    {
        ...
        if (empty($tx->s_id)) {
            throw new \Box_Exception(
                "Can not create subscription. Subscription id from payment gateway was not received"
            );
        }
    ...
    private function _unsubscribe(\Model_Transaction $tx)
    {
        ...
        if (!$model instanceof \Model_Subscription) {
            throw new \Box_Exception(
                "Subscription #:id was not found. Could not unsubscribe",
                [":id" => $tx->s_id]
            );
        }
    ...
    private function _validateApprovedTransaction(\Model_Transaction $tx)
    {
        if ($tx->status != \Model_Transaction::STATUS_APPROVED) {
            throw new \Box_Exception(
                "Only approved transaction can be processed"
            );
        }
        ...
        if ($invoice->currency != $tx->currency) {
            throw new \Box_Exception(
                "Transaction currency :code do not match required currency :required",
                [":code" => $tx->currency, ":required" => $invoice->currency],
                709
            );
        }
        ...
        if ($tx->txn_status == \Payment_Transaction::STATUS_PENDING) {
            throw new \Box_Exception(
                "Transaction status on payment gateway is Pending. Only Complete or Unknown transactions can be processed.",
                null,
                712
            );
        }
    ...
    public function debitTransaction(\Model_Transaction $tx)
    {
        ...
        if ($client->currency != $proforma->currency) {
            throw new \Box_Exception(
                "Client currency do not match invoice currency"
            );
        }
        ...
        if ($tx->amount < 0) {
            throw new \Box_Exception(
                "Can not add negative amount to client balance for debit transaction"
            );
        }
        ...
        $credit->description =
            "Invoice #" .
            $proforma->id .
            " payment received from transaction #" .
            $tx->id;
