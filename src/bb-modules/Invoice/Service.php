...
class Service implements InjectionAwareInterface
{
    ...
    public function toApiArray(
        ...
        $items = $this->di["db"]->find("InvoiceItem", "invoice_id = :iid", [
            "iid" => $row["id"],
        ]);
        ...
        foreach ($items as $item) {
            $order_id =
                $item->type == \Model_InvoiceItem::TYPE_ORDER
                    ? $item->rel_id
                    : null;
        ...
        $result["seller"] = [
            ...
            "phone" => !empty($row["seller_phone"])
                ? $row["seller_phone"]
                : $c["tel"],
            "email" => !empty($row["seller_email"])
                ? $row["seller_email"]
                : $c["email"],
            "account_number" => !empty($c["account_number"])
                ? $c["account_number"]
                : null,
        ];
        ...
        $result["subscribable"] = $subscriptionService->isSubscribable(
            $row["id"]
        );
        if ($deep && $result["subscribable"]) {
            $ip = $this->di["db"]->getCell(
                "SELECT period FROM invoice_item WHERE invoice_id = :id",
                ["id" => $row["id"]]
            );
    ...
    public static function onAfterAdminInvoicePaymentReceived(\Box_Event $event)
    {
        ...
        try {
            ...
            $invoice = $service->toApiArray($invoiceModel, [
                "id" => $params["id"],
            ]);
            if ($invoice["total"] > 0) {
                $email = [];
    ...
    public function markAsPaid(
        ...
        $invoiceItems = $this->di["db"]->find("InvoiceItem", "invoice_id = ?", [
            $invoice->id,
        ]);
        $invoiceItemService = $this->di["mod_service"](
            "Invoice",
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminInvoicePaymentReceived",
    ...
    public function getNextInvoiceNumber(\Model_Invoice $model)
    {
        ...
        if (empty($next_nr)) {
            ...
            $r = $this->di["db"]->findOne(
                "Invoice",
                "nr is not null order by id desc"
            );
    ...
    public function countIncome(\Model_Invoice $invoice)
    {
        ...
        $invoice->base_income = $table->toBaseCurrency(
            $invoice->currency,
            $this->getTotal($invoice)
        );
        $invoice->base_refund = $table->toBaseCurrency(
            $invoice->currency,
            $invoice->refund
        );
        $this->di["db"]->store($invoice);
    ...
    public function prepareInvoice(\Model_Client $client, array $data)
    {
        if (!$client->currency) {
            ...
            error_log(
                sprintf(
                    "Client #%s currency was not defined. Set default currency %s",
                    $client->id,
                    $currency->code
                )
            );
        ...
        $model->gateway_id = $this->di["array_get"](
            $data,
            "gateway_id",
            $model->gateway_id
        );
        $model->text_1 = $this->di["array_get"](
            $data,
            "text_1",
            $model->text_1
        );
        $model->text_2 = $this->di["array_get"](
            $data,
            "text_2",
            $model->text_2
        );
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminInvoiceApprove",
            "params" => ["id" => $invoice->id],
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminInvoiceApprove",
            "params" => ["id" => $invoice->id],
        ]);
    ...
    public function tryPayWithCredits(\Model_Invoice $invoice)
    {
        ...
        if (abs($balance - $required) < $epsilon) {
            if ($this->di["config"]["debug"]) {
                error_log(
                    sprintf(
                        "Setting invoice %s as paid with credits",
                        $invoice->id
                    )
                );
            }
        ...
        if ($this->di["config"]["debug"]) {
            error_log(
                sprintf(
                    "Invoice %s could not be paid with credits. Money in balance %s Required: %s",
                    $invoice->id,
                    $balance,
                    $required
                )
    ...
    public function getTax(\Model_Invoice $invoice)
    {
        ...
        $items = $this->di["db"]->find("InvoiceItem", "invoice_id = ? ", [
            $invoice->id,
        ]);
        $tax = 0;
        foreach ($items as $item) {
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminInvoiceRefund",
            "params" => ["id" => $invoice->id],
        ]);

        $systemService = $this->di["mod_service"]("system");
        $logic = $systemService->getParamValue(
        ...
        switch ($logic) {
            ...
            case "negative_invoice":
                ...
                if ($total <= 0) {
                    throw new \Box_Exception(
                        "Can not refund invoice with negative amount"
                    );
                }
                ...
                $this->addNote(
                    $invoice,
                    sprintf("Refund invoice #%s generated", $new->id)
                );
                $this->addNote(
                    $new,
                    sprintf("Refund for #%s invoice", $invoice->id)
                );
                if (!empty($note)) {
                ...
                if ($logic == "negative_invoice") {
                    $new->serie = $systemService->getParamValue(
                        "invoice_series_paid"
                    );
                ...
                if ($logic == "credit_note") {
                    $next_nr = $systemService->getParamValue(
                        "invoice_cn_starting_number",
                        1
                    );
                    $new->serie = $systemService->getParamValue(
                        "invoice_cn_series",
            ...
            case "manual":
                if ($this->di["config"]["debug"]) {
                    error_log(
                        "Refunds are managed manually. No actions performed"
                    );
                }
                break;
            default:
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminInvoiceRefund",
            "params" => ["id" => $invoice->id],
        ]);
    ...
    public function updateInvoice(\Model_Invoice $model, array $data)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminInvoiceUpdate",
            "params" => $data,
        ]);
        ...
        $model->seller_company = $this->di["array_get"](
            $data,
            "seller_company",
            $model->seller_company
        );
        $model->seller_company_vat = $this->di["array_get"](
            $data,
            "seller_company_vat",
            $model->seller_company_vat
        );
        $model->seller_company_number = $this->di["array_get"](
            $data,
            "seller_company_number",
            $model->seller_company_number
        );
        $model->seller_address = $this->di["array_get"](
            $data,
            "seller_address",
            $model->seller_address
        );
        $model->seller_phone = $this->di["array_get"](
            $data,
            "seller_phone",
            $model->seller_phone
        );
        $model->seller_email = $this->di["array_get"](
            $data,
            "seller_email",
            $model->seller_email
        );
        $model->buyer_first_name = $this->di["array_get"](
            $data,
            "buyer_first_name",
            $model->buyer_first_name
        );
        $model->buyer_last_name = $this->di["array_get"](
            $data,
            "buyer_last_name",
            $model->buyer_last_name
        );
        $model->buyer_company = $this->di["array_get"](
            $data,
            "buyer_company",
            $model->buyer_company
        );
        $model->buyer_company_vat = $this->di["array_get"](
            $data,
            "buyer_company_vat",
            $model->buyer_company_vat
        );
        $model->buyer_company_number = $this->di["array_get"](
            $data,
            "buyer_company_number",
            $model->buyer_company_number
        );
        $model->buyer_address = $this->di["array_get"](
            $data,
            "buyer_address",
            $model->buyer_address
        );
        $model->buyer_city = $this->di["array_get"](
            $data,
            "buyer_city",
            $model->buyer_city
        );
        $model->buyer_state = $this->di["array_get"](
            $data,
            "buyer_state",
            $model->buyer_state
        );
        $model->buyer_country = $this->di["array_get"](
            $data,
            "buyer_country",
            $model->buyer_country
        );
        $model->buyer_zip = $this->di["array_get"](
            $data,
            "buyer_zip",
            $model->buyer_zip
        );
        $model->buyer_phone = $this->di["array_get"](
            $data,
            "buyer_phone",
            $model->buyer_phone
        );
        $model->buyer_email = $this->di["array_get"](
            $data,
            "buyer_email",
            $model->buyer_email
        );
        ...
        $model->taxrate = $this->di["array_get"](
            $data,
            "taxrate",
            $model->taxrate
        );
        $model->taxname = $this->di["array_get"](
            $data,
            "taxname",
            $model->taxname
        );
        $model->approved = (int) $this->di["array_get"](
            $data,
            "approved",
            $model->approved
        );
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminInvoiceUpdate",
            "params" => ["id" => $model->id],
        ]);
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminInvoiceDelete",
            "params" => ["id" => $model->id],
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminInvoiceDelete",
            "params" => ["id" => $id],
        ]);
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeClientInvoiceDelete",
            "params" => ["id" => $model->id],
        ]);

        // check if invoice is associated with order
        $invoiceItem = $this->di["db"]->find("InvoiceItem", "invoice_id = ?", [
            $model->id,
        ]);
        foreach ($invoiceItem as $item) {
            if ($item->type == \Model_InvoiceItem::TYPE_ORDER) {
                throw new \Box_Exception(
                    "Invoice is related to order #:id. Please cancel order first.",
                    [":id" => $item->rel_id]
                );
            }
        }
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminGenerateRenewalInvoice",
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminGenerateRenewalInvoice",
            "params" => ["order_id" => $model->id, "id" => $invoice->id],
        ]);

        $this->di["logger"]->info(
            "Generated renewal invoice #%s",
            $invoice->id
        );
        return $invoice->id;
    }
    ...
    public function doBatchPayWithCredits(array $data)
    {
        ...
        $this->di["logger"]->info(
            "Executed action to try cover unpaid invoices with client credits"
        );
        return true;
    }
    ...
    public function generateForOrder(
        ...
        $invoiceItemService->generateFromOrder(
            $proforma,
            $order,
        ...
        // invoice due date
        if ($due_days > 0) {
            $proforma->due_at = date(
                "Y-m-d H:i:s",
                strtotime("+" . $due_days . " days")
            );
            $this->di["db"]->store($proforma);
    ...
    public function generateInvoicesForExpiringOrders()
    {
        ...
        $this->di["logger"]->info(
            "Executed action to generate new invoices for expiring orders"
        );
        return true;
    }
    ...
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminInvoiceSendReminders",
        ]);
        ...
        $this->di["logger"]->info(
            "Executed action to send invoice payment reminders"
        );
        return true;
    }
    ...
    {
        $once_per_day = isset($data["once_per_day"])
            ? (bool) $data["once_per_day"]
            : true;
        ...
        if (
            $once_per_day &&
            $last_time &&
            time() - strtotime($last_time) < 86400
        ) {
            //error_log('Already executed today.');
            return false;
        }

        $before_due_list = $this->di["db"]->getAll(
            "SELECT id, DATEDIFF(due_at, NOW()) as days_left FROM invoice WHERE status = 'unpaid' AND approved = 1 AND due_at > NOW()"
        );
        foreach ($before_due_list as $params) {
            $this->di["events_manager"]->fire([
                "event" => "onEventBeforeInvoiceIsDue",
                "params" => $params,
            ]);
        }

        $after_due_list = $this->di["db"]->getAll(
            "SELECT id, ABS(DATEDIFF(due_at, NOW())) as days_passed FROM invoice WHERE status = 'unpaid' AND approved = 1 AND ((due_at < NOW()) OR (ABS(DATEDIFF(due_at, NOW())) = 0 ))"
        );
        foreach ($after_due_list as $params) {
            $this->di["events_manager"]->fire([
                "event" => "onEventAfterInvoiceIsDue",
                "params" => $params,
            ]);
        }
        ...
        $this->di["logger"]->info(
            "Executed action to invoke invoice due event"
        );
        return true;
    }
    ...
    public function sendInvoiceReminder(\Model_Invoice $invoice)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminInvoiceSendReminder",
            "params" => ["id" => $invoice->id],
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminInvoiceReminderSent",
            "params" => ["id" => $invoice->id],
        ]);
    ...
    public function counter()
    {
        ...
        return [
            ...
            \Model_Invoice::STATUS_PAID => isset(
                $data[\Model_Invoice::STATUS_PAID]
            )
                ? $data[\Model_Invoice::STATUS_PAID]
                : 0,
            \Model_Invoice::STATUS_UNPAID => isset(
                $data[\Model_Invoice::STATUS_UNPAID]
            )
                ? $data[\Model_Invoice::STATUS_UNPAID]
                : 0,
            \Model_Invoice::STATUS_REFUNDED => isset(
                $data[\Model_Invoice::STATUS_REFUNDED]
            )
                ? $data[\Model_Invoice::STATUS_REFUNDED]
                : 0,
            \Model_Invoice::STATUS_CANCELED => isset(
                $data[\Model_Invoice::STATUS_CANCELED]
            )
                ? $data[\Model_Invoice::STATUS_CANCELED]
                : 0,
        ];
    ...
    public function processInvoice(array $data)
    {
        ...
        $invoice = $this->di["db"]->findOne("Invoice", "hash = ?", [
            $data["hash"],
        ]);
        ...
        if (
            $subscribeService->isSubscribable($invoice->id) &&
            $payGatewayService->canPerformRecurrentPayment($gtw)
        ) {
            $subscribe = true;
        }
        ...
        if (method_exists($adapter, "getHtml")) {
            ...
            return [
                "iframe" => isset($pgc["can_load_in_iframe"])
                    ? (bool) $pgc["can_load_in_iframe"]
                    : false,
                "type" => "html",
                "service_url" => "",
        ...
        $r = $subscribe
            ? $adapter->recurrentPayment($mpi)
            : $adapter->singlePayment($mpi);
        $this->di["logger"]->info(
            "Went to pay for invoice #%s via %s",
            $invoice->id,
            $gtw->gateway
        );
    ...
    public function generatePDF($hash, $identity)
    {
        ...
        $invoice_info =
            ...
            "\n" .
            __("Due date: ") .
            strftime($localeDateFormat, strtotime($invoice["due_at"])) .
            "\n" .
            __("Invoice status: ") .
        ...
        $company_info =
            ...
            "\n" .
            __("Company VAT: ") .
            $invoice["seller"]["company_vat"] .
            "\n" .
            __("Company number: ") .
            $invoice["seller"]["company_number"] .
            "\n" .
            __("Account: ") .
        ...
        foreach ($invoice["lines"] as $row) {
            ...
            $pdf->Cell(
                ...
                $row["quantity"] > 1
                    ? $row["quantity"] .
                        " x " .
                        $this->money($row["price"], $currencyCode)
                    : $this->money($row["price"], $currencyCode),
                "LR",
                0,
                "R"
            ...
            $pdf->Cell(
                $w[3],
                6,
                $this->money($row["total"], $currencyCode),
                "LR",
                0,
                "R"
            );
        ...
        if ($invoice["tax"] > 0) {
            ...
            $pdf->Cell(
                40,
                6,
                $this->money($invoice["tax"], $currencyCode),
                "LRTB",
                0,
                "C"
            );
        ...
        if (isset($invoice["discount"]) && $invoice["discount"] > 0) {
            ...
            $pdf->Cell(
                40,
                8,
                $this->money($invoice["discount"], $currencyCode),
                "LRTB",
                0,
                "C"
            );
        ...
        $pdf->Cell(
            40,
            10,
            $this->money($invoice["total"], $currencyCode),
            "LRTB",
            0,
            "C"
        );
    ...
    {
        return $this->di["api_guest"]->currency_format([
            "price" => $price,
            "code" => $currencyCode,
            "convert" => false,
        ]);
    ...
    public function addNote(\Model_Invoice $model, $note)
    {
        $n = $model->notes;
        $model->notes =
            $n . date("Y-m-d H:i:s") . ": " . $note . "       " . PHP_EOL;
        $model->updated_at = date("Y-m-d H:i:s");
        $this->di["db"]->store($model);
        return true;
    }
    ...
    public function findAllUnpaid(array $filter = null)
    {
        ...
        $client_id = isset($filter["client_id"])
    ...
    {
        return $this->di["db"]->find("Invoice", "status = ? order by id desc", [
            \Model_Invoice::STATUS_PAID,
        ]);
    }
    ...
    {
        $conditions =
            "status = ? and approved = 1 and reminded_at is null and DATEDIFF(NOW(), created_at) > ?";
        return $this->di["db"]->find("Invoice", $conditions, [
            \Model_Invoice::STATUS_UNPAID,
            $days_after_issue,
        ]);
    }
    ...
    private function _isAutoApproved()
    {
         ...
         */
        $systemService = $this->di["mod_service"]("system");
        return (bool) $systemService->getParamValue(
            "invoice_auto_approval",
            true
        );
    }
    ...
    {
        $invoices = $this->di["db"]->find("Invoice", "client_id = ?", [
            $client->id,
        ]);
