...
class Service implements \Box\InjectionAwareInterface
{
                ...
                if ($field["readonly"] == 1) {
                    ...
                    if ($data[$field_name] != $field["default_value"]) {
                        throw new \Box_Exception(
                            "Field " .
                                $field["label"] .
                                " is read only. You can not change its value",
                            null,
                            5468
                        );
                    }
    ...
    public function customCall(
        ...
        if (in_array($method, $forbidden_methods)) {
            throw new \Box_Exception(
                "Custom plugin method :method is forbidden",
                [":method" => $method],
                403
            );
        }
    ...
    private function callOnAdapter(
        \Model_ServiceCustom $model,
        $method,
        $params = []
    ) {
        ...
        if (
            ...
            !file_exists(BB_PATH_LIBRARY . DIRECTORY_SEPARATOR . $file)
        ) {
            $e = new \Box_Exception(
                "Plugin class file :file was not found",
                [":file" => $file],
                3124
            );
            if (BB_DEBUG) {
        ...
        if (!method_exists($adapter, $method)) {
            throw new \Box_Exception(
                "Plugin :plugin does not support action :action",
                [":plugin" => $plugin, ":action" => $method],
                3125
            );
        }
