...
class Admin extends \Api_Abstract
{
    ...
    public function server_update($data)
    {
        ...
        $bean = $this->di["db"]->findOne("extension_meta", "id = :id", [
            "id" => $server["id"],
        ]);
        $bean->meta_value = json_encode($data);
        $bean->updated_at = date("Y-m-d H:i:s");
    ...
    public function server_delete($data)
    {
        ...
        $sql =
            "SELECT id FROM service_centovacast WHERE server_id = :sid LIMIT 1";
        $exists = $this->di["db"]->getCell($sql, ["sid" => $server["id"]]);
        if ($exists) {
            throw new \Exception(
                "Server is used for active order. It can not be removed."
            );
        }
    ...
    public function update($data)
    {
        ...
        $this->di["logger"]->info(
            "Updated CentovaCast service %s details",
            $s->id
        );
        return true;
    }
    ...
    private function _getService($data)
    {
        ...
        $s = $this->di["db"]->findOne("service_centovacast", "id=:id", [
            "id" => $order->service_id,
        ]);
        if (!$s) {
            throw new \Box_Exception("Order :id is not activated", [
                ":id" => $order->id,
            ]);
        }
        return [$order, $s];
