...
class CCBaseAPIClient
{
    ...
    function build_request_packet($methodname, $payload)
    {
        return sprintf(
            ...
            ($this->debug ? ' debug="enabled"' : "") .
                ($this->debugconsole
                    ? ' debugconsole="' .
                        htmlentities($this->debugconsole) .
                        '"'
                    : ""),
            $payload
        );
    ...
    function parse_response_packet($packet)
    {
        ...
        if (
            !preg_match(
                "/<centovacast([^\>]+)>([\s\S]+)<\/centovacast>/i",
                $packet,
                $matches
            )
        ) {
            return $this->set_error(
                "Invalid response packet received from API server"
            );
        }
        $cctags = $matches[1];
        ...
        if (
            !preg_match(
                '/<response.*?type\s*=\s*"([^"]+)"[^\>]*>([\s\S]+)<\/response>/i',
                $payload,
                $matches
            )
        ) {
            return $this->set_error(
                "Empty or unrecognized response packet received from API server"
            );
        }
        ...
        switch (strtolower($type)) {
            ...
            default:
                return $this->set_error(
                    "Invalid response type received from API server"
                );
        }
    }
    ...
    function api_request($packet)
    {
        ...
        if (!$this->http->post($url, $postdata)) {
            $this->set_error(
                "Error contacting server: " . $this->http->get_error()
            );
            return;
        }
    ...
    function bb_data($packet)
    {
        ...
        if (
            isset($array["response"]["@attributes"]["type"]) &&
            $array["response"]["@attributes"]["type"] == "success"
        ) {
            return true;
        }
...
class CCServerAPIClient extends CCBaseAPIClient
{
    ...
    function build_argument_payload($functionargs)
    {
        if (count($functionargs) < 3) {
            trigger_error(
                sprintf(
                    "Function %s requires a minimum of 3 arguments, %d given",
                    $this->methodname,
                    count($functionargs)
                ),
                E_USER_WARNING
...
class CCSystemAPIClient extends CCBaseAPIClient
{
    ...
    function build_argument_payload($functionargs)
    {
        if (count($functionargs) < 2) {
            trigger_error(
                sprintf(
                    "Function %s requires a minimum of 2 arguments, %d given",
                    $this->methodname,
                    count($functionargs)
                ),
                E_USER_WARNING
        ...
        $arguments = array_merge(["password" => $adminpassword], $arguments);
...
if (!defined("CCAPI_NO_OVERLOAD")) {
    if (function_exists("overload")) {
        ...
        die(
            "The CentovaCast PHP API client requires that object overloading support is built into PHP."
        );
    }
}
?>
