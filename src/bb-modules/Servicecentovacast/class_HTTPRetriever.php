...
if (!extension_loaded("curl") && !defined("HTTPR_NO_REDECLARE_CURL")) {
    foreach (
        [dirname(__FILE__) . "/", dirname(__FILE__) . "/libcurlemu/"]
        as $k => $libcurlemupath
    ) {
...
class HTTPRetriever
{
    ...
    function array_to_query($arrayname, $arraycontents)
    {
        ...
        foreach ($arraycontents as $key => $value) {
            if (is_array($value)) {
                $output .= $this->array_to_query(
                    sprintf("%s[%s]", $arrayname, urlencode($key)),
                    $value
                );
            } else {
                $output .= sprintf(
    ...
    function set_user_agent($agenttype, $agentversion, $windowsversion)
    {
        ...
        $this->headers["User-Agent"] = str_replace(
            ["%agent%", "%os%"],
            [$agentversion, $windowsversion],
            $agent
        );
    }
    ...
    function _cache_store($token, $url)
    {
        if ($this->caching_intelligent) {
            ...
            if ($this->method == "POST") {
                ...
                $this->progress(
                    HRP_DEBUG,
                    "Request used query string; not caching"
                );
                return;
            } else {
                $this->progress(
                    HRP_DEBUG,
                    "Request appears to be static and cacheable"
                );
            }
        }
        ...
        foreach ($cookies as $name => $paths) {
            foreach ($paths as $path => $values) {
                ...
                $domain_match =
                    ...
                    substr($requesthost, -(strlen($cookiehost) + 1)) ==
                        "." . $cookiehost;
                ...
                if (
                    substr($requestpath, 0, strlen($cookiepath)) != $cookiepath
                ) {
                    continue;
                }
    ...
    function _execute_request($url, $cookies = false)
    {
        ...
        do {
            ...
            if (
                ...
                in_array($this->result_code, $redirect_codes)
            ) {
                // only redirect on a code 303 or if the method was GET/HEAD
                if (
                    $this->result_code == 303 ||
                    in_array($this->method, $redirect_methods)
                ) {
            ...
            if ($url && strlen($url)) {
                ...
                if (
                    is_numeric($this->follow_redirects) &&
    ...
    function _send_request($url, $cookies = false)
    {
        ...
        if ($this->manage_cookies) {
            $this->response_to_request_cookies(
                $this->managed_cookies[$urldata["host"]],
                $urldata
            );
        }
        ...
        if (
            ($urldata["scheme"] == "http" ||
                ($php_ssl_support && $urldata["scheme"] == "https")) &&
            !$this->force_curl
        ) {
            $curl_mode = false;

            $hostname = $this->connect_ip
                ? $this->connect_ip
                : $urldata["host"];
            ...
            if ($fp) {
                ...
                if ($this->stream_timeout) {
                    ...
                    if ($meta["timed_out"]) {
                        $this->error =
                            "Exceeded socket write timeout of " .
                            $this->stream_timeout .
                            " seconds";
                ...
                while (!feof($fp)) {
                    if ($data_length > 0) {
                        ...
                        $this->progress(
                            HRP_DEBUG,
                            "[DL] Got a line: [{$line}] " . gettype($line)
                        );
                        ...
                        $this->progress(
                            HRP_DEBUG,
                            "[NDL] Got a line: [{$line}] " . gettype($line)
                        );
                    ...
                    if ($line === false) {
                        ...
                        if ($meta["timed_out"]) {
                            if ($this->stream_timeout) {
                                $this->error =
                                    "Exceeded socket read timeout of " .
                                    $this->stream_timeout .
                                    " seconds";
                            } else {
                                $this->error =
                                    "Exceeded default socket read timeout";
                            }
                            $this->progress(HRP_ERROR, $this->error);
                            return false;
                        } else {
                    ...
                    if ($this->max_time > 0) {
                        if (
                            $this->getmicrotime() - $time_request_start >
                            $this->max_time
                        ) {
                            $this->error =
                                "Exceeded maximum transfer time of " .
                                $this->max_time .
                                " seconds";
                            $this->progress(HRP_ERROR, $this->error);
                            return false;
                    ...
                    if (!$headers_received && trim($line) == "") {
                        ...
                        $this->progress(
                            HRP_DEBUG,
                            "Got headers: {$this->response}"
                        );

                        if (
                            preg_match(
                                "/^Content-Length: ([0-9]+)/im",
                                $this->response,
                                $matches
                            )
                        ) {
                            ...
                            $this->progress(
                                HRP_DEBUG,
                                "No data length specified"
                            );
                        }
                        if (
                            preg_match(
                                "/^Transfer-Encoding: chunked/im",
                                $this->response,
                                $matches
                            )
                        ) {
                        ...
                        if (
                            ...
                            foreach ($cookielist as $k => $cookiedata) {
                                ...
                                if (
                                    ...
                                    $cookie["expires"] = strtotime(
                                        $cookie["expires"]
                                    );
                                }

                                if (
                                    !$this->validate_response_cookie(
                                        $cookie,
                                        $urldata["host"]
                                    )
                                ) {
                ...
                if (
                    $this->manage_cookies &&
                    is_array($this->managed_cookies[$urldata["host"]])
                ) {
                    // make sure paths are sorted in the order in which they should be applied
                    // when setting response cookies
                    foreach (
            ...
            !$this->disable_curl &&
            ($urldata["scheme"] == "https" || $this->force_curl)
        ) {
        ...
        if (!$curl_mode) {
            ...
            $this->stats = [
                ...
                "filetime" => isset($this->response_headers["Date"])
                    ? strtotime($this->response_headers["Date"])
                    : time(),
                "pretransfer_time" => $connect_time,
                "size_download" => $totallength,
                "speed_download" =>
                    $transfer_time > 0
                        ? round($totallength / $transfer_time)
                        : 0,
    ...
    function get_error()
    {
        return $this->error
            ? $this->error
            : "HTTP " . $this->result_code . ": " . $this->result_text;
    }
    ...
    function parse_page_form(
        $formindex = false,
        $formid = false,
        $formattrsearch = false
    ) {
        if (
            !preg_match_all(
                "|<form([\s\S]*?)>([\s\S]*?)</form>|",
    ...
    function form_fields_to_request_fields($fields)
    {
        ...
        foreach ($fields as $name => $field) {
            if ($field["type"] == "image") {
                ...
                $output[$name] = is_scalar($field["value"])
                    ? $field["value"]
                    : "";
            }
        }
