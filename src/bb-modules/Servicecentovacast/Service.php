...
class Service implements \Box\InjectionAwareInterface
{
    ...
    public function activate($order, $model)
    {
        ...
        $required = [
            "server_id" =>
                "CentovaCast product is not configured properly. Field server_id is missing",
            "maxclients" =>
                "CentovaCast product is not configured properly. Field maxclients is missing",
            "maxbitrate" =>
                "CentovaCast product is not configured properly. Field maxbitrate is missing",
            "transferlimit" =>
                "CentovaCast product is not configured properly. Field transferlimit is missing",
            "diskquota" =>
                "CentovaCast product is not configured properly. Field diskquota is missing",
            "template" =>
                "CentovaCast product is not configured properly. Field template is missing",
            "autostart" =>
                "CentovaCast product is not configured properly. Field autostart is missing",
        ];
        $this->di["validator"]->checkRequiredParamsForArray($required, $pc);

        $server = $this->getServer($pc["server_id"]);
        $username =
            isset($oc["username"]) && !empty($oc["username"])
                ? $oc["username"]
                : $this->_genUsername($client);
        $password =
            isset($oc["password"]) && !empty($oc["password"])
                ? $oc["password"]
    ...
    public function suspend($order, $model)
    {
        if (!is_object($model)) {
            throw new \Box_Exception(
                "Could not suspend order. Service was not created",
                null,
                7456
            );
        }
    ...
    public function unsuspend($order, $model)
    {
        if (!is_object($model)) {
            throw new \Box_Exception(
                "Could not unsuspend order. Service was not created",
                null,
                7456
            );
        }
    ...
    public function getsongs($model)
    {
        ...
        return isset($res["songs"]["row"]) ? $res["songs"]["row"] : [];
    }
     ...
     * @param string $method
     */
    private function _apiServerCall(
        $model,
        $method,
        $arguments = [],
        $admin = false
        ...
        $account_password = $this->di["crypt"]->decrypt(
            $model->pass,
            $this->_salt
        );
        $server = $this->getServer($model->server_id);
