...
class Service implements InjectionAwareInterface
{
    ...
    public function getProfile($client_id)
    {
        ...
        $posts = $this->di["db"]->getCell(
            "SELECT COUNT(id) FROM forum_topic_message WHERE client_id = :cid",
            [":cid" => $client_id]
        );

        $points = $this->di["db"]->getCell(
            'SELECT meta_value 
            FROM extension_meta 
    ...
    public function getMessagesQuery($data)
    {
        ...
        $search2 =
            isset($data["search"]) && !empty($data["search"])
                ...
                : null;
        $forum_topic_id =
            isset($data["forum_topic_id"]) && !empty($data["forum_topic_id"])
                ? $data["forum_topic_id"]
                : null;
        ...
        if (isset($data["orderby"])) {
            if (in_array(strtolower($data["orderby"]), ["id", "created_at"])) {
                $sortorder =
                    ...
                    in_array(strtolower($data["sortorder"]), ["asc", "desc"])
                        ? $data["sortorder"]
                        : "asc";
                $sql .=
                    " ORDER BY " .
    ...
    public static function onAfterClientRepliedInForum(\Box_Event $event)
    {
        ...
        if (
            isset($config["forum_points_enable"]) &&
            ...
            $di["db"]->exec(
                "UPDATE forum_topic_message SET points = :points WHERE id = :id",
