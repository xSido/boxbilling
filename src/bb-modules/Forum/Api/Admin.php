...
class Admin extends \Api_Abstract
{
    ...
    public function create($data)
    {
        ...
        $priority = $this->di["db"]->getCell(
            "SELECT MAX(priority) FROM forum LIMIT 1"
        );
    ...
    public function update($data)
    {
        ...
        if ($category) {
            if ($this->di["array_get"]($data, "update_categories")) {
                $this->di["db"]->exec(
                    "UPDATE forum SET category = :cat WHERE category = :old_cat",
    ...
    public function topic_get_list($data)
    {
        ...
        foreach ($pager["list"] as $key => $item) {
            $topic = $this->di["db"]->getExistingModelById(
                "ForumTopic",
                $item["id"],
    ...
    public function topic_create($data)
    {
        ...
        $topic->status = isset($data["status"])
            ? $data["status"]
            : \Model_ForumTopic::STATUS_ACTIVE;
    ...
    public function topic_update($data)
    {
        ...
        $model->forum_id = $this->di["array_get"](
            $data,
            "forum_id",
            $model->forum_id
        );
        ...
        $model->slug = $this->di["array_get"]($data, "slug", $model->slug);
        $model->sticky = $this->di["array_get"](
            $data,
            "sticky",
            $model->sticky
        );
    ...
    public function message_get_list($data)
    {
        ...
        foreach ($pager["list"] as &$entry) {
            $entry["author"] = $this->getService()->getProfile(
                $entry["client_id"]
            );
            if (!$entry["client_id"] && $entry["admin_id"]) {
                $admin = $this->di["db"]->getExistingModelById(
                    "Admin",
                    $entry["admin_id"],
                    "Admin not found"
                );
                $entry["author"]["gravatar"] =
                    "https://gravatar.com/avatar/" . md5($admin->email);
            }
        }
        return $pager;
    }
    ...
    public function message_get($data)
    {
        ...
        $msg = $this->di["db"]->getExistingModelById(
            "ForumTopicMessage",
            $data["id"],
    ...
    public function message_update($data)
    {
        ...
        $model->message = $this->di["array_get"](
            $data,
            "message",
            $model->message
        );
    ...
    public function message_delete($data)
    {
        ...
        $this->di["logger"]->info(
            "Admin removed forum topic message  #%s",
            $id
        );
        return true;
    }
    ...
    public function message_create($data)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminRepliedInForum",
            "params" => [
                "id" => $topic->id,
                "message_id" => $msg->id,
                "admin_id" => $admin->id,
    ...
    public function points_deduct($data)
    {
        ...
        $this->di["logger"]->info(
            "Deducted points from forum message #%s",
            $data["id"]
        );
        return true;
    }
    ...
    public function points_update($data)
    {
        ...
        $this->di["logger"]->info(
            "Updated client %s forum points balance to %s",
            $data["client_id"],
            $data["amount"]
        );
        return true;
