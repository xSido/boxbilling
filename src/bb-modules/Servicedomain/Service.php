...
class Service implements \Box\InjectionAwareInterface
{
    ...
    public function getCartProductTitle($product, array $data)
    {
        if (
            ...
            isset($data["register_tld"]) &&
            isset($data["register_sld"])
        ) {
            return __("Domain :domain registration", [
                ":domain" => $data["register_sld"] . $data["register_tld"],
            ]);
        }
        ...
        if (
            ...
            isset($data["transfer_tld"]) &&
            isset($data["transfer_sld"])
        ) {
            return __("Domain :domain transfer", [
                ":domain" => $data["transfer_sld"] . $data["transfer_tld"],
            ]);
        }
    ...
    public function validateOrderData(&$data)
    {
        ...
        $required = [
            "action" =>
                "Are you registering new domain or transferring existing? Action parameter missing",
        ];
        ...
        if ($action == "owndomain") {
            if (!isset($data["owndomain_sld"])) {
                throw new \Box_Exception(
                    "Order data must contain :field configuration field",
                    [":field" => "owndomain_sld"]
                );
            }

            if (!$validator->isSldValid($data["owndomain_sld"])) {
                $safe_dom = htmlspecialchars(
                    $data["owndomain_sld"],
                    ENT_QUOTES | ENT_HTML5,
                    "UTF-8"
                );
                throw new \Box_Exception("Domain name :domain is not valid", [
                    ":domain" => $safe_dom,
                ]);
            }
        ...
        if ($action == "register") {
            ...
            $required = [
                ...
                "register_years" =>
                    "Years parameter is missing for domain configuration.",
            ];
            $this->di["validator"]->checkRequiredParamsForArray(
                $required,
                $data
            ...
            if ($years < $tld->min_years) {
                throw new \Box_Exception(
                    ":tld can be registered for at least :years years",
                    [":tld" => $tld->tld, ":years" => $tld->min_years]
                );
            }

            $domain = $data["register_sld"] . $tld->tld;
            if (!$this->isDomainAvailable($tld, $data["register_sld"])) {
                throw new \Box_Exception(":domain is already registered!", [
                    ":domain" => $domain,
                ]);
            }
    ...
    public function action_create(\Model_ClientOrder $order)
    {
        ...
        if (empty($ns)) {
            throw new \Box_Exception(
                "Default domain nameservers are not configured"
            );
        }
        ...
        $model->tld = $tld;
        $model->period = $years;
        $model->transfer_code = $this->di["array_get"](
            $c,
            "transfer_code",
            null
        );
        ...
        $model->ns1 =
            isset($c["ns1"]) && !empty($c["ns1"])
                ? $c["ns1"]
                : $ns["nameserver_1"];
        $model->ns2 =
            isset($c["ns2"]) && !empty($c["ns1"])
                ? $c["ns2"]
                : $ns["nameserver_2"];
        $model->ns3 =
            isset($c["ns3"]) && !empty($c["ns1"])
                ? $c["ns3"]
                : $ns["nameserver_3"];
        $model->ns4 =
            isset($c["ns4"]) && !empty($c["ns1"])
                ? $c["ns4"]
                : $ns["nameserver_4"];
    ...
    public function action_renew(\Model_ClientOrder $order)
    {
        ...
        if (!$model instanceof \RedBeanPHP\SimpleModel) {
            throw new \Box_Exception("Order :id has no active service", [
                ":id" => $order->id,
            ]);
        }
        //@adapterAction
    ...
    protected function syncWhois(
        \Model_ServiceDomain $model,
        ...
        $model->registered_at = date(
            "Y-m-d H:i:s",
            $whois->getRegistrationTime()
        );
        $model->updated_at = date("Y-m-d H:i:s");
    ...
    public function enablePrivacyProtection(\Model_ServiceDomain $model)
    {
        ...
        $this->di["logger"]->info(
            "Enabled privacy protection of #%s domain",
            $id
        );
    ...
    public function disablePrivacyProtection(\Model_ServiceDomain $model)
    {
        ...
        $this->di["logger"]->info(
            "Disabled privacy protection of #%s domain",
            $id
        );
    ...
    public function canBeTransfered(\Model_Tld $model, $sld)
    {
        ...
        $tldRegistrar = $this->di["db"]->load(
            "TldRegistrar",
            $model->tld_registrar_id
        );
    ...
    protected function _getD(\Model_ServiceDomain $model)
    {
        ...
        if ($order instanceof \Model_ClientOrder) {
            $adapter = $this->registrarGetRegistrarAdapter(
                $tldRegistrar,
                $order
            );
        } else {
        ...
        $email = empty($model->contact_email)
            ? $client->email
            : $model->contact_email;
        $first_name = empty($model->contact_first_name)
            ? $client->first_name
            : $model->contact_first_name;
        $last_name = empty($model->contact_last_name)
            ? $client->last_name
            : $model->contact_last_name;
        $city = empty($model->contact_city)
            ? $client->city
            : $model->contact_city;
        $zip = empty($model->contact_postcode)
            ? $client->postcode
            : $model->contact_postcode;
        $country = empty($model->contact_country)
            ? $client->country
            : $model->contact_country;
        $state = empty($model->contact_state)
            ? $client->state
            : $model->contact_state;
        $phone = empty($model->contact_phone)
            ? $client->phone
            : $model->contact_phone;
        $phone_cc = empty($model->contact_phone_cc)
            ? $client->phone_cc
            : $model->contact_phone_cc;
        $company = empty($model->contact_company)
            ? $client->company
            : $model->contact_company;
        $address1 = empty($model->contact_address1)
            ? $client->address_1
            : $model->contact_address1;
        $address2 = empty($model->contact_address2)
            ? $client->address_2
            : $model->contact_address2;
        $birthday = !empty($client->birthday) ? $client->birthday : "";
        $company_number = !empty($client->company_number)
    ...
    public function batchSyncExpirationDates()
    {
        ...
        $this->di["logger"]->info(
            "Executed action to synchronize domain expiration dates with registrar"
        );
    ...
    public function tldCreate($data)
    {
        ...
        $model->min_years = isset($data["min_years"])
            ? (int) $data["min_years"]
            : 1;
        $model->allow_register = isset($data["allow_register"])
            ? (bool) $data["allow_register"]
            : true;
        $model->allow_transfer = isset($data["allow_transfer"])
            ? (bool) $data["allow_transfer"]
            : true;
        $model->active = isset($data["active"])
            ? (bool) $data["active"]
            : false;
        $model->updated_at = date("Y-m-d H:i:s");
        $model->created_at = date("Y-m-d H:i:s");
        ...
        $this->di["logger"]->info(
            "Created new top level domain %s",
            $model->tld
        );
    ...
    {
        $model->tld_registrar_id = $this->di["array_get"](
            $data,
            "tld_registrar_id",
            $model->tld_registrar_id
        );
        $model->price_registration = $this->di["array_get"](
            $data,
            "price_registration",
            $model->price_registration
        );
        $model->price_renew = $this->di["array_get"](
            $data,
            "price_renew",
            $model->price_renew
        );
        $model->price_transfer = $this->di["array_get"](
            $data,
            "price_transfer",
            $model->price_transfer
        );
        $model->min_years = $this->di["array_get"](
            $data,
            "min_years",
            $model->min_years
        );
        $model->allow_register = $this->di["array_get"](
            $data,
            "allow_register",
            $model->allow_register
        );
        $model->allow_transfer = $this->di["array_get"](
            $data,
            "allow_transfer",
            $model->allow_transfer
        );
        $model->active = $this->di["array_get"](
            $data,
            "active",
            $model->active
        );
    ...
    public function tldGetSearchQuery($data)
    {
        ...
        $hide_inactive = isset($data["hide_inactive"])
            ? (bool) $data["hide_inactive"]
            : false;
    ...
    public function tldFindOneActiveById($id)
    {
        return $this->di["db"]->findOne(
            "Tld",
            "id = :id AND active = 1 ORDER by id ASC",
            [":id" => $id]
        );
    }
    ...
    {
        return $this->di["db"]->getAssoc(
            "SELECT id, tld from tld WHERE active = 1 ORDER by id ASC"
        );
    }

    public function tldAlreadyRegistered($tld)
    {
        $tld = $this->di["db"]->findOne("Tld", "tld = :tld ORDER by id ASC", [
            ":tld" => $tld,
        ]);
    ...
    {
        return $this->di["db"]->findOne("Tld", "tld = :tld ORDER by id ASC", [
            ":tld" => $tld,
        ]);
    }
    ...
    {
        $query =
            "SELECT 'registrar', 'name' FROM tld_registrar GROUP BY registrar";
    ...
    {
        $query =
            "SELECT tr.id, tr.name FROM tld_registrar tr ORDER BY tr.id DESC";
    ...
    public function registrarGetRegistrarAdapterConfig(
        \Model_TldRegistrar $model
    ) {
    ...
    private function registrarGetRegistrarAdapterClassName(
        \Model_TldRegistrar $model
    ) {
        if (
            !file_exists(
            ...
            throw new \Box_Exception(
                "Domain registrar :adapter was not found",
                [":adapter" => $model->registrar]
            );
        }

        $class = sprintf("Registrar_Adapter_%s", $model->registrar);
        if (!class_exists($class)) {
            throw new \Box_Exception("Registrar :adapter was not found", [
                ":adapter" => $class,
            ]);
        }
    ...
    public function registrarGetRegistrarAdapter(\Model_TldRegistrar $r)
    {
        ...
        if (!$registrar instanceof \Registrar_AdapterAbstract) {
            throw new \Box_Exception(
                "Registrar adapter :adapter should extend Registrar_AdapterAbstract",
                [":adapter" => $class]
            );
        }
    ...
    public function registrarUpdate(\Model_TldRegistrar $model, $data)
    {
        ...
        $model->test_mode = $this->di["array_get"](
            $data,
            "test_mode",
            $model->test_mode
        );
        ...
        $this->di["logger"]->info(
            "Updated domain registrar %s configuration",
            $model->registrar
        );
    ...
    {
        $domains = $this->di["db"]->find(
            "ServiceDomain",
            "tld_registrar_id = :registrar_id",
            [":registrar_id" => $model->id]
        );
        if (count($domains) > 0) {
            throw new \Box_Exception(
                "Can not remove registrar which has domains"
            );
        }
    ...
    public function updateDomain(\Model_ServiceDomain $s, $data)
    {
        ...
        $s->privacy = (bool) $this->di["array_get"](
            $data,
            "privacy",
            $s->privacy
        );
        ...
        $s->locked = (bool) $this->di["array_get"]($data, "locked", $s->locked);
        $s->transfer_code = $this->di["array_get"](
            $data,
            "transfer_code",
            $s->transfer_code
        );
        ...
        $this->di["logger"]->info(
            "Updated domain #%s without sending actions to server",
            $s->id
        );
