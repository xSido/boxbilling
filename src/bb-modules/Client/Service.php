...
class Service implements InjectionAwareInterface
{
    ...
    public function approveClientEmailByHash($hash)
    {
        $db = $this->di["db"];
        $result = $db->getRow(
            'SELECT id, client_id FROM extension_meta WHERE extension = "mod_client" AND meta_key = "confirm_email" AND meta_value = :hash',
            [":hash" => $hash]
        );
        if (!$result) {
            throw new \Box_Exception("Invalid email confirmation link");
        }
        $db->exec("UPDATE client SET email_approved = 1 WHERE id = :id", [
            "id" => $result["client_id"],
        ]);
        $db->exec("DELETE FROM extension_meta WHERE id = :id", [
            "id" => $result["id"],
        ]);
        return true;
    }
    ...
    public static function onAfterClientSignUp(\Box_Event $event)
    {
        ...
        try {
            ...
            if (
                isset($config["require_email_confirmation"]) &&
                ...
                ] = $clientService->generateEmailConfirmationLink(
                    $params["id"]
                );
            }
    ...
    public function getSearchQuery($data, $selectStmt = "SELECT c.*")
    {
        $sql = $selectStmt;
        $sql .=
            " FROM client as c left join client_group as cg on c.client_group_id = cg.id";

        $search =
            isset($data["search"]) && !empty($data["search"])
                ? $data["search"]
                : null;
        $client_id =
            isset($data["client_id"]) && !empty($data["client_id"])
                ? $data["client_id"]
                : null;
        $group_id =
            isset($data["group_id"]) && !empty($data["group_id"])
                ? $data["group_id"]
                : null;
    ...
    public function getPairs($data)
    {
        ...
        [$sql, $params] = $this->getSearchQuery(
            $data,
    ...
    public function emailAreadyRegistered(
        $new_email,
        \Model_Client $model = null
    ) {
    ...
    public function canChangeCurrency(\Model_Client $model, $currency = null)
    {
        ...
        if ($invoice instanceof \Model_Invoice) {
            throw new \Box_Exception(
                "Currency can not be changed. Client already have invoices issued."
            );
        }
        ...
        if ($order instanceof \Model_ClientOrder) {
            throw new \Box_Exception(
                "Currency can not be changed. Client already have orders."
            );
        }
    ...
    public function addFunds(
        ...
        if (!$client->currency) {
            throw new \Box_Exception(
                "Define clients currency before adding funds."
            );
        }
    ...
    public function getExpiredPasswordReminders()
    {
        ...
        $expired = $this->di["db"]->find(
            "ClientPasswordReset",
            "UNIX_TIMESTAMP() - ? > UNIX_TIMESTAMP(created_at)",
            [$expire_after_hours * 60 * 60]
        );
        return $expired;
    }
    ...
    public function counter()
    {
        ...
        return [
            ...
            \Model_Client::ACTIVE => isset($data[\Model_Client::ACTIVE])
                ? $data[\Model_Client::ACTIVE]
                : 0,
            \Model_Client::SUSPENDED => isset($data[\Model_Client::SUSPENDED])
                ? $data[\Model_Client::SUSPENDED]
                : 0,
            \Model_Client::CANCELED => isset($data[\Model_Client::CANCELED])
                ? $data[\Model_Client::CANCELED]
                : 0,
        ];
    }
    ...
    {
        $client = $this->di["db"]->findOne(
            "Client",
            "email = ? and pass = ? and status = ?",
            [$email, $password, \Model_Client::ACTIVE]
        );
        return $client;
    }
    ...
    public function toApiArray(
        ...
        $clientGroup = $this->di["db"]->load(
            "ClientGroup",
            $model->client_group_id
        );
    ...
    {
        $client = $this->di["db"]->findOne("Client", "client_group_id = ?", [
            $model->id,
        ]);
        if ($client) {
    ...
    private function createClient(array $data)
    {
        ...
        $client->email = strtolower(
            trim($this->di["array_get"]($data, "email"))
        );
        $client->first_name = ucwords(
            $this->di["array_get"]($data, "first_name")
        );
        ...
        $client->company_number = $this->di["array_get"](
            $data,
            "company_number"
        );
        ...
        $client->created_at = !empty($created_at)
    ...
    public function adminCreateClient(array $data)
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminCreateClient",
            "params" => $data,
        ]);
        $client = $this->createClient($data);
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminCreateClient",
            "params" => ["id" => $client->id, "password" => $data["password"]],
        ]);
    ...
    public function remove(\Model_Client $model)
    {
        ...
        $pdo = $this->di["pdo"];
        $stmt = $pdo->prepare(
            "DELETE FROM extension_meta WHERE client_id = :id"
        );
    ...
    public function authorizeClient($email, $plainTextPassword)
    {
        ...
        if (
            isset($config["require_email_confirmation"]) &&
            (int) $config["require_email_confirmation"]
        ) {
            if (!$model->email_approved) {
                $meta = $this->di["db"]->findOne(
                    "ExtensionMeta",
                    ' extension = "mod_client" AND meta_key = "confirm_email" AND client_id = :client_id',
                    [":client_id" => $model->id]
                );
                if (!is_null($meta)) {
                    throw new \Box_Exception(
                        "Please check your mailbox and confirm email address."
                    );
                } else {
                    $this->sendEmailConfirmationForClient($model);
                    throw new \Box_Exception(
                        "Confirmation email was sent to your email address. Please click on link in it in order to verify your email."
                    );
                }
            }
        }
    ...
    private function sendEmailConfirmationForClient(\Model_Client $client)
    {
        try {
            ...
            $email[
                "email_confirmation_link"
            ] = $this->generateEmailConfirmationLink($client->id);
    ...
    public function checkExtraRequiredFields(array $checkArr)
    {
        ...
        foreach ($required as $field) {
            if (!isset($checkArr[$field]) || empty($checkArr[$field])) {
                ...
                throw new \Box_Exception(
                    'It is required that you provide details for field ":field"',
                    [":field" => $name]
                );
            }
        }
    ...
    public function checkCustomFields(array $checkArr)
    {
        ...
        foreach ($customFields as $cFieldName => $cField) {
            $active =
                isset($cField["active"]) && $cField["active"] ? true : false;
            $required =
                isset($cField["required"]) && $cField["required"]
                    ? true
                    : false;
            if ($active && $required) {
                if (
                    !isset($checkArr[$cFieldName]) ||
                    empty($checkArr[$cFieldName])
                ) {
                    $name =
                        isset($cField["title"]) && !empty($cField["title"])
                            ? $cField["title"]
                            : ucwords(str_replace("_", " ", $cFieldName));
                    throw new \Box_Exception(
                        'It is required that you provide details for field ":field"',
                        [":field" => $name]
                    );
