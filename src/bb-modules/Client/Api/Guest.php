...
class Guest extends \Api_Abstract
{
    ...
    public function create($data = [])
    {
        ...
        if (isset($config["allow_signup"]) && !$config["allow_signup"]) {
            throw new \Box_Exception(
                "New registrations are temporary disabled"
            );
        }
        ...
        $this->di["validator"]->isPasswordStrong(
        ...
        if ($service->clientAlreadyExists($email)) {
            throw new \Box_Exception(
                "Email is already registered. You may want to login instead of registering."
            );
        }
        ...
        if ($this->di["array_get"]($data, "auto_login", 0)) {
            try {
                $this->login([
                    "email" => $client->email,
                    "password" => $this->di["array_get"]($data, "password"),
                ]);
            } catch (\Exception $e) {
    ...
    public function login($data)
    {
        ...
        if (!$client instanceof \Model_Client) {
            $this->di["events_manager"]->fire([
                "event" => "onEventClientLoginFailed",
                "params" => $event_params,
            ]);
    ...
    public function reset_password($data)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeGuestPasswordResetRequest",
            "params" => $data,
        ]);
        ...
        $this->di["logger"]->info(
            "Client requested password reset. Sent to email %s",
            $c->email
        );
        return true;
    }
    ...
    public function confirm_reset($data)
    {
        ...
        $reset = $this->di["db"]->findOne("ClientPasswordReset", "hash = ?", [
            $data["hash"],
        ]);
        if (!$reset instanceof \Model_ClientPasswordReset) {
            throw new \Box_Exception(
                "The link have expired or you have already confirmed password reset."
            );
        }
    ...
    {
        $config = $this->di["mod_config"]("client");
        return isset($config["custom_fields"]) ? $config["custom_fields"] : [];
    }
}
