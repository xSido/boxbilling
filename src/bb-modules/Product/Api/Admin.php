...
class Admin extends \Api_Abstract
{
    ...
    public function prepare($data)
    {
        ...
        if (!array_key_exists($data["type"], $types)) {
            throw new \Box_Exception(
                "Product type :type is not registered",
                [":type" => $data["type"]],
                413
            );
        }

        $categoryId = $this->di["array_get"](
            $data,
            "product_category_id",
            null
        );

        return (int) $service->createProduct(
            $data["title"],
            $data["type"],
            $categoryId
        );
    ...
    public function promo_create($data)
    {
        ...
        $service = $this->getService();
        return (int) $service->createPromo(
            $data["code"],
            $data["type"],
            $data["value"],
            $products,
    ...
    public function promo_get($data)
    {
        ...
        return $this->getService()->toPromoApiArray(
            $model,
            true,
            $this->getIdentity()
        );
