...
class Service implements InjectionAwareInterface
{
    ...
    public function toApiArray(
        ...
        if ($identity instanceof \Model_Admin) {
            ...
            if ($model->product_category_id) {
                $productCategory = $this->di["db"]->load(
                    "ProductCategory",
    ...
    public function createProduct($title, $type, $categoryId = null)
    {
        ...
        try {
            ...
            $model->slug =
                $this->di["tools"]->slug($title) . "-" . rand(1, 9999);
        }
        $productId = $this->di["db"]->store($model);
    ...
    public function updateProduct(\Model_Product $model, $data)
    {
        ...
        if (isset($data["pricing"])) {
            ...
            if (
                !isset($data["pricing"]["type"]) ||
                !array_key_exists($data["pricing"]["type"], $types)
            ) {
                throw new \Box_Exception("Pricing type is required");
            }
            $productPayment = $this->di["db"]->getExistingModelById(
                "ProductPayment",
                $model->product_payment_id,
                "Product payment not found"
            );
            ...
            if ($data["pricing"]["type"] == \Model_ProductPayment::ONCE) {
                $productPayment->once_setup_price =
                    (float) $data["pricing"]["once"]["setup"];
                $productPayment->once_price =
                    (float) $data["pricing"]["once"]["price"];
            }

            if ($data["pricing"]["type"] == \Model_ProductPayment::RECURRENT) {
                if (isset($pricing["recurrent"]["1W"])) {
                    $productPayment->w_setup_price =
                        $pricing["recurrent"]["1W"]["setup"];
                    $productPayment->w_price =
                        $pricing["recurrent"]["1W"]["price"];
                    $productPayment->w_enabled =
                        $pricing["recurrent"]["1W"]["enabled"];
                }

                if (isset($pricing["recurrent"]["1M"])) {
                    $productPayment->m_setup_price =
                        $pricing["recurrent"]["1M"]["setup"];
                    $productPayment->m_price =
                        $pricing["recurrent"]["1M"]["price"];
                    $productPayment->m_enabled =
                        $pricing["recurrent"]["1M"]["enabled"];
                }

                if (isset($pricing["recurrent"]["3M"])) {
                    $productPayment->q_setup_price =
                        $pricing["recurrent"]["3M"]["setup"];
                    $productPayment->q_price =
                        $pricing["recurrent"]["3M"]["price"];
                    $productPayment->q_enabled =
                        $pricing["recurrent"]["3M"]["enabled"];
                }

                if (isset($pricing["recurrent"]["6M"])) {
                    $productPayment->b_setup_price =
                        $pricing["recurrent"]["6M"]["setup"];
                    $productPayment->b_price =
                        $pricing["recurrent"]["6M"]["price"];
                    $productPayment->b_enabled =
                        $pricing["recurrent"]["6M"]["enabled"];
                }

                if (isset($pricing["recurrent"]["1Y"])) {
                    $productPayment->a_setup_price =
                        $pricing["recurrent"]["1Y"]["setup"];
                    $productPayment->a_price =
                        $pricing["recurrent"]["1Y"]["price"];
                    $productPayment->a_enabled =
                        $pricing["recurrent"]["1Y"]["enabled"];
                }

                if (isset($pricing["recurrent"]["2Y"])) {
                    $productPayment->bia_setup_price =
                        $pricing["recurrent"]["2Y"]["setup"];
                    $productPayment->bia_price =
                        $pricing["recurrent"]["2Y"]["price"];
                    $productPayment->bia_enabled =
                        $pricing["recurrent"]["2Y"]["enabled"];
                }

                if (isset($pricing["recurrent"]["3Y"])) {
                    $productPayment->tria_setup_price =
                        $pricing["recurrent"]["3Y"]["setup"];
                    $productPayment->tria_price =
                        $pricing["recurrent"]["3Y"]["price"];
                    $productPayment->tria_enabled =
                        $pricing["recurrent"]["3Y"]["enabled"];
                }
            }
        ...
        $model->product_category_id = $this->di["array_get"](
            $data,
            "product_category_id",
            $model->product_category_id
        );
        $model->form_id = empty($form_id) ? null : $form_id;
        $model->icon_url = $this->di["array_get"](
            $data,
            "icon_url",
            $model->icon_url
        );
        ...
        $model->hidden = (int) $this->di["array_get"](
            $data,
            "hidden",
            $model->hidden
        );
        ...
        $model->stock_control = $this->di["array_get"](
            $data,
            "stock_control",
            $model->stock_control
        );
        $model->allow_quantity_select = $this->di["array_get"](
            $data,
            "allow_quantity_select",
            $model->allow_quantity_select
        );
        $model->quantity_in_stock = $this->di["array_get"](
            $data,
            "quantity_in_stock",
            $model->quantity_in_stock
        );
        $model->description = $this->di["array_get"](
            $data,
            "description",
            $model->description
        );
        $model->plugin = $this->di["array_get"](
            $data,
            "plugin",
            $model->plugin
        );
        ...
        $this->di["logger"]->info(
            "Updated product #%s configuration",
            $model->id
        );
    ...
    public function createAddon(
        ...
        $model->status = isset($status)
            ? $status
            : \Model_Product::STATUS_DISABLED;
        $model->title = $title;
        $model->slug = $this->di["tools"]->slug($title);
    ...
    public function createPromo(
        ...
        if (
            $this->di["db"]->findOne("Promo", "code = :code", [
                ":code" => $code,
            ])
        ) {
        ...
        $model->once_per_client = (bool) $this->di["array_get"](
            $data,
            "once_per_client",
            0
        );
        $model->recurring = (bool) $this->di["array_get"](
            $data,
            "recurring",
            0
        );
    ...
    public function toPromoApiArray(
        \Model_Promo $model,
        $deep = false,
        $identity = null
    ) {
        ...
        $clientGroups = $this->di["tools"]->getPairsForTableByIds(
            "client_group",
            $clientGroups
        );
    ...
    public function updatePromo(\Model_Promo $model, array $data = [])
    {
        ...
        $model->freesetup = $this->di["array_get"](
            $data,
            "freesetup",
            $model->freesetup
        );
        $model->once_per_client = $this->di["array_get"](
            $data,
            "once_per_client",
            $model->once_per_client
        );
        $model->recurring = $this->di["array_get"](
            $data,
            "recurring",
            $model->recurring
        );
    ...
    public function toProductCategoryApiArray(
        \Model_ProductCategory $model,
        $deep = true
    ) {
        $min_price = 0;
        $products = [];
        ...
        foreach ($pr as $p) {
            ...
            $products[] = $pa;
            $startingPrice = $this->di["array_get"](
                $pa,
                "price_starting_from",
                0
            );
    ...
    public function findOneActiveById($id)
    {
        return $this->di["db"]->findOne(
            "Product",
            "id = ? and active = 1 and status = 'enabled' and is_addon = 0",
            [$id]
        );
    }
    ...
    public function findOneActiveBySlug($slug)
    {
        return $this->di["db"]->findOne(
            "Product",
            "slug = ? and active = 1 and status = 'enabled' and is_addon = 0",
            [$slug]
        );
    }
    ...
    public function getStartingFromPrice(\Model_Product $model)
    {
        ...
        if ($model->product_payment_id) {
            $productPaymentModel = $this->di["db"]->load(
                "ProductPayment",
                $model->product_payment_id
            );
    ...
    public function getProductTitlesByIds($ids)
    {
        ...
        $rows = $this->di["db"]->getAll(
            "SELECT id, title FROM product WHERE id in (" . $slots . ")",
            $ids
        );
    ...
    public function getCategoryProducts(\Model_ProductCategory $model)
    {
        return $this->di["db"]->find(
            "Product",
            'is_addon = 0 and status="enabled" and hidden = 0 and product_category_id = ?',
            [$model->id]
        );
    }
    ...
    public function toProductPaymentApiArray(\Model_ProductPayment $model)
    {
        ...
        $periods["1W"] = [
            "price" => $model->w_price,
            "setup" => $model->w_setup_price,
            "enabled" => $model->w_enabled,
        ];
        $periods["1M"] = [
            "price" => $model->m_price,
            "setup" => $model->m_setup_price,
            "enabled" => $model->m_enabled,
        ];
        $periods["3M"] = [
            "price" => $model->q_price,
            "setup" => $model->q_setup_price,
            "enabled" => $model->q_enabled,
        ];
        $periods["6M"] = [
            "price" => $model->b_price,
            "setup" => $model->b_setup_price,
            "enabled" => $model->b_enabled,
        ];
        $periods["1Y"] = [
            "price" => $model->a_price,
            "setup" => $model->a_setup_price,
            "enabled" => $model->a_enabled,
        ];
        $periods["2Y"] = [
            "price" => $model->bia_price,
            "setup" => $model->bia_setup_price,
            "enabled" => $model->bia_enabled,
        ];
        $periods["3Y"] = [
            "price" => $model->tria_price,
            "setup" => $model->tria_setup_price,
            "enabled" => $model->tria_enabled,
        ];

        return [
            "type" => $model->type,
            \Model_ProductPayment::FREE => ["price" => 0, "setup" => 0],
            \Model_ProductPayment::ONCE => [
                "price" => $model->once_price,
                "setup" => $model->once_setup_price,
            ],
    ...
    public function getAddonById($id)
    {
        return $this->di["db"]->findOne(
            "Product",
            "type = 'custom' and is_addon = 1 and id = ?",
            [$id]
        );
    }
    ...
    public function getProductDiscount(
        ...
        switch ($promo->type) {
            ...
            case \Model_Promo::PERCENTAGE:
                if (
                    isset($config["quantity"]) &&
                    is_numeric($config["quantity"])
                ) {
                    $quantity = $config["quantity"];
                }

                $discount += round(
                    ($price * $quantity * $promo->value) / 100,
                    2
                );
                break;
    ...
    public function isPromoLinkedToProduct(
        \Model_Promo $promo,
        \Model_Product $product
    ) {
        if ($product->is_addon) {
    ...
    public function getProductAddons(\Model_Product $model)
    {
        ...
        return $this->di["db"]->find(
            "Product",
            'type = "custom" and is_addon= 1 and id != ? and id IN (' .
                $slots .
                ")",
            $ids
        );
