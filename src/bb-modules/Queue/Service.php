...
class Service implements \Box\InjectionAwareInterface
{
    ...
    public function install()
    {
        ...
        if (!array_key_exists("module", $columns)) {
            $this->di["db"]->exec(
                "ALTER TABLE  `queue` ADD  `module` VARCHAR( 255 ) NULL AFTER  `name`;"
            );
        }

        if (!array_key_exists("iteration", $columns)) {
            $this->di["db"]->exec(
                "ALTER TABLE  `queue` ADD  `iteration` VARCHAR( 255 ) NULL AFTER  `timeout`;"
            );
        }
        ...
        if (!array_key_exists("handler", $columns)) {
            $this->di["db"]->exec(
                "ALTER TABLE  `queue_message` ADD  `handler` VARCHAR( 255 ) NULL AFTER  `handle`;"
            );
        }

        if (!array_key_exists("execute_at", $columns)) {
            $this->di["db"]->exec(
                "ALTER TABLE  `queue_message` ADD  `execute_at` VARCHAR( 255 ) NULL AFTER  `log`;"
            );
        }
    }
    ...
    public function toApiArray($row)
    {
        ...
        $sql =
            "SELECT COUNT(id) FROM queue_message WHERE queue_id = :id GROUP BY queue_id";
