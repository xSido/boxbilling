...
class Admin extends \Api_Abstract
{
    ...
    public function message_add($data)
    {
        ...
        if (!method_exists($service, $handler)) {
            throw new \Box_Exception(
                "Message handler function :method does not exists",
                [
                    ":ext" => $data["mod"],
                    ":method" => get_class($service) . ":" . $handler,
                ]
            );
        }
        ...
        $q = $this->di["db"]->findOne("queue", "name = :name", [
            "name" => $data["queue"],
        ]);
        if (!$q) {
    ...
    public function execute($data)
    {
        ...
        if (!flock($file_handle, LOCK_EX | LOCK_NB)) {
            $this->di["logger"]->info(
                sprintf("Queue %s is being executed by other process.", $q->id)
            );
            throw new \Exception(
                "This queue is being executed by other process."
            );
        }
        ...
        $interval = isset($data["interval"])
            ? (int) $data["interval"]
            : $q->timeout;
        $until_complete = isset($data["until_complete"])
            ? (bool) $data["until_complete"]
            : true;

        $this->di["logger"]->info(
            sprintf(
                "Started executing %s queue by selecting %s messages every %s seconds",
                $q->name,
                $max,
                $interval
            )
        ...
        $this->di["logger"]->info(
            sprintf("Finished executing queue %s", $q->name)
        );
        return true;
    }
    ...
    {
        $lsql =
            "UPDATE queue_message SET log = :log, updated_at = :u WHERE id = :id;";
        ...
        foreach ($msgs as $msg) {
            try {
                $this->di["logger"]->info(
                    sprintf(
                        "Executing %s queue message #%s with handler %s(%s)",
                        $q->name,
                        $msg["id"],
                        get_class($service) . ":" . $msg["handler"],
                        $msg["json"]
                    )
                );
                ...
                $this->di["logger"]->info(
                    sprintf(
                        "Error executing queue %s message #%s %s",
                        $q->name,
                        $msg["id"],
                        $e->getMessage()
                    )
    ...
    private function receiveQueueMessages($qid, $max, $timeout)
    {
        ...
        $sql =
            "SELECT id, handler, body
                FROM queue_message
                WHERE queue_id = $qid
                AND (handle IS NULL OR timeout+" .
            (int) $timeout .
            " < " .
            (int) $microtime .
            ")
