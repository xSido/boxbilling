...
class Admin extends \Api_Abstract
{
    ...
    public function change_plan($data)
    {
        ...
        $plan = $this->di["db"]->getExistingModelById(
            "ServiceHostingHp",
            $data["plan_id"],
    ...
    {
        $servers = $this->di["db"]->find(
            "ServiceHostingServer",
            "ORDER BY id ASC"
        );
        $serversArr = [];
        foreach ($servers as $server) {
            $serversArr[] = $this->getService()->toHostingServerApiArray(
                $server,
                false,
                $this->getIdentity()
            );
        ...
        $result = $this->di["pager"]->getSimpleResultSet(
            $sql,
            $params,
            $per_page
        );
    ...
    public function server_create($data)
    {
        ...
        $service = $this->getService();
        return (int) $service->createServer(
            $data["name"],
            $data["ip"],
            $data["manager"],
            $data
    ...
    public function server_get($data)
    {
        ...
        $service = $this->getService();
        return $service->toHostingServerApiArray(
            $model,
            true,
            $this->getIdentity()
        );
    ...
    public function hp_get_list($data)
    {
        ...
        foreach ($pager["list"] as $key => $item) {
            ...
            $pager["list"][$key] = $this->getService()->toHostingHpApiArray(
                $model,
                false,
                $this->getIdentity()
            );
    ...
    public function hp_get($data)
    {
        ...
        return $this->getService()->toHostingHpApiArray(
            $model,
            true,
            $this->getIdentity()
        );
