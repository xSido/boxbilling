...
class Service implements InjectionAwareInterface
{
    ...
    public function action_create(\Model_ClientOrder $order)
    {
        ...
        $server = $this->di["db"]->getExistingModelById(
            "ServiceHostingServer",
            $c["server_id"],
            "Server from order configuration was not found"
        );

        $hp = $this->di["db"]->getExistingModelById(
            "ServiceHostingHp",
            $c["hosting_plan_id"],
            "Hosting plan from order configuration was not found"
        );
    ...
    public function changeAccountPlan(
        ...
        \Model_ServiceHosting $model,
        \Model_ServiceHostingHp $hp
    ) {
        $model->service_hosting_hp_id = $hp->id;
        ...
        $this->di["db"]->store($model);
        $this->di["logger"]->info(
            "Changed hosting plan of account #%s",
            $model->id
        );
        return true;
    }
    ...
    public function changeAccountUsername(
        ...
        $this->di["logger"]->info(
            "Changed hosting account %s username",
            $model->id
        );
        return true;
    }
    ...
    public function changeAccountDomain(
        ...
        $this->di["db"]->store($model);
        $this->di["logger"]->info(
            "Changed hosting account %s domain",
            $model->id
        );
        return true;
    }
    ...
    public function changeAccountPassword(
        ...
        $this->di["db"]->store($model);
        $this->di["logger"]->info(
            "Changed hosting account %s password",
            $model->id
        );
        return true;
    }
    ...
    public function sync(
        ...
        $this->di["db"]->store($model);
        $this->di["logger"]->info(
            "Synchronizing hosting account %s with server",
            $model->id
        );
        return true;
    }
    ...
    public function _getAM(
        \Model_ServiceHosting $model,
        \Model_ServiceHostingHp $hp = null
    ) {
        if (null === $hp) {
            $hp = $this->di["db"]->getExistingModelById(
                "ServiceHostingHp",
                $model->service_hosting_hp_id,
        ...
        $server = $this->di["db"]->getExistingModelById(
            "ServiceHostingServer",
            $model->service_hosting_server_id,
            "Server not found"
        );
        $c = $this->di["db"]->getExistingModelById(
            "Client",
            $model->client_id,
            "Client not found"
        );
    ...
    public function toApiArray(
        ...
        $serviceHostingServerModel = $this->di["db"]->load(
            "ServiceHostingServer",
            $model->service_hosting_server_id
        );
        $serviceHostingHpModel = $this->di["db"]->load(
            "ServiceHostingHp",
            $model->service_hosting_hp_id
        );
        $server = $this->toHostingServerApiArray(
            $serviceHostingServerModel,
            $deep,
            $identity
        );
        $hp = $this->toHostingHpApiArray(
            $serviceHostingHpModel,
            $deep,
            $identity
        );
    ...
    public function toHostingServerApiArray(
        \Model_ServiceHostingServer $model,
        $deep = false,
        $identity = null
    ) {
    ...
    private function _getDomainTuple($data)
    {
        ...
        if ($data["domain"]["action"] == "register") {
            $required = [
                "register_sld" =>
                    "Hosting product must have defined register_sld parameter",
                "register_tld" =>
                    "Hosting product must have defined register_tld parameter",
            ];
            $this->di["validator"]->checkRequiredParamsForArray(
                $required,
                $data["domain"]
        ...
        if ($data["domain"]["action"] == "transfer") {
            $required = [
                "transfer_sld" =>
                    "Hosting product must have defined transfer_sld parameter",
                "transfer_tld" =>
                    "Hosting product must have defined transfer_tld parameter",
            ];
            $this->di["validator"]->checkRequiredParamsForArray(
                $required,
                $data["domain"]
    ...
    public function update(\Model_ServiceHosting $model, array $data)
    {
        ...
        $this->di["logger"]->info(
            "Updated hosting account %s without sending actions to server",
            $model->id
        );
        return true;
    }
    ...
    public function updateServer(
        ...
        $model->hostname = $this->di["array_get"](
            $data,
            "hostname",
            $model->hostname
        );
        ...
        $model->status_url = $this->di["array_get"](
            $data,
            "status_url",
            $model->status_url
        );
        $model->max_accounts = $this->di["array_get"](
            $data,
            "max_accounts",
            $model->max_accounts
        );
        ...
        $model->manager = $this->di["array_get"](
            $data,
            "manager",
            $model->manager
        );
        $model->accesshash = $this->di["array_get"](
            $data,
            "accesshash",
            $model->accesshash
        );
        $model->port = $this->di["array_get"]($data, "port", $model->port);
        $model->secure = $this->di["array_get"](
            $data,
            "secure",
            $model->secure
        );
        $model->username = $this->di["array_get"](
            $data,
            "username",
            $model->username
        );
        $model->password = $this->di["array_get"](
            $data,
            "password",
            $model->password
        );
    ...
    public function getServerManager(\Model_ServiceHostingServer $model)
    {
        ...
        if (!$manager instanceof \Server_Manager) {
            throw new \Box_Exception("Server manager :adapter is not valid", [
                ":adapter" => $model->manager,
            ]);
        }
    ...
    {
        $id = $model->id;
        $serviceHosting = $this->di["db"]->findOne(
            "ServiceHosting",
            "service_hosting_hp_id = ?",
            [$model->id]
        );
        if ($serviceHosting) {
            throw new \Box_Exception(
                "Can not remove hosting plan which has active accounts"
            );
        }
        $this->di["db"]->trash($model);
    ...
    public function toHostingHpApiArray(
        \Model_ServiceHostingHp $model,
        $deep = false,
        $identity = null
    ) {
        $result = [
    ...
    {
        $model->name = $this->di["array_get"]($data, "name", $model->name);
        $model->bandwidth = $this->di["array_get"](
            $data,
            "bandwidth",
            $model->bandwidth
        );
        $model->quota = $this->di["array_get"]($data, "quota", $model->quota);
        $model->max_addon = $this->di["array_get"](
            $data,
            "max_addon",
            $model->max_addon
        );
        $model->max_ftp = $this->di["array_get"](
            $data,
            "max_ftp",
            $model->max_ftp
        );
        $model->max_sql = $this->di["array_get"](
            $data,
            "max_sql",
            $model->max_sql
        );
        $model->max_pop = $this->di["array_get"](
            $data,
            "max_pop",
            $model->max_pop
        );
        $model->max_sub = $this->di["array_get"](
            $data,
            "max_sub",
            $model->max_sub
        );
        $model->max_park = $this->di["array_get"](
            $data,
            "max_park",
            $model->max_park
        );
    ...
    public function getServerManagerWithLog(
        \Model_ServiceHostingServer $model,
        \Model_ClientOrder $order
    ) {
    ...
    public function getDomainProductFromConfig(
