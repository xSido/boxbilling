...
class Service implements InjectionAwareInterface
{
    ...
    {
        $data["style"]["type"] = isset($data["type"])
            ? $data["type"]
            : "horizontal";
        $data["style"]["show_title"] = isset($data["show_title"])
            ? $data["show_title"]
            : "0";
    ...
    {
        //TODO server-side required check
        ...
        $label = $this->di["array_get"](
            $field,
            "label",
            $types[$type] . " " . $field_number
        );
        $name = $this->di["array_get"](
            $field,
            "name",
            $this->slugify("new_" . $type) . "_" . $field_number
        );

        if ($type == "select" || $type == "checkbox" || $type == "radio") {
            $field["options"] =
                '{"First option":"1", "Second option": "2", "Third option":"3"}';
        }
        if ($field["type"] == "textarea" && !isset($field["options"])) {
            $field["options"] = '{"height":"100", "width": "300"}';
        }
        if (isset($field["default_value"])) {
            $field["default_value"] = is_array($field["default_value"])
                ? json_encode($field["default_value"], JSON_FORCE_OBJECT)
                : $field["default_value"];
        }
        ...
        $bean->description = $this->di["array_get"](
            $field,
            "description",
            null
        );
        $bean->type = $field["type"];
        $bean->default_value = $this->di["array_get"](
            $field,
            "default_value",
            null
        );
        ...
        $bean->show_initial = $this->di["array_get"](
            $field,
            "show_initial",
            null
        );
        $bean->show_middle = $this->di["array_get"](
            $field,
            "show_middle",
            null
        );
        $bean->show_prefix = $this->di["array_get"](
            $field,
            "show_prefix",
            null
        );
        $bean->show_suffix = $this->di["array_get"](
            $field,
            "show_suffix",
            null
        );
    ...
    private function slugify($text)
    {
        ...
        if (is_numeric(substr($text, 0, 1))) {
            throw new \Box_Exception(
                "Field name can not start with number.",
                null,
                1649
            );
        }
        ...
        if (empty($text)) {
            throw new \Box_Exception(
                "Field name can not be empty. Please make sure it is not empty and does not contain special characters.",
                null,
                3502
            );
        }
    ...
    public function updateField($field)
    {
        ...
        if (
            ...
            throw new \Box_Exception(
                "Unfortunately field with this name exists in this form already. Form must have different field names.",
                null,
                7628
            );
        }
        ...
        if (isset($field["type"])) {
            if (
                ...
                if (
                    !$this->isArrayUnique(
                        array_filter($field["values"], "strlen")
                    )
                ) {
                    throw new \Box_Exception(
                        ucfirst($field["type"]) . " values must be unique",
                        null,
                        1597
                    );
                }
                if (
                    !$this->isArrayUnique(
                        array_filter($field["labels"], "strlen")
                    )
                ) {
                    throw new \Box_Exception(
                        ucfirst($field["type"]) . " labels must be unique",
                        null,
                        1598
                    );
                }
            ...
            if ($field["type"] == "textarea") {
                if (
                    count($field["textarea_size"]) !=
                    count(array_filter($field["textarea_size"], "is_numeric"))
                ) {
                    throw new \Box_Exception(
                        "Textarea size options must be integer values",
                        null,
                        3510
                    );
                }
                $field["options"] = array_combine(
                    $field["textarea_option"],
                    $field["textarea_size"]
                );
                $field["options"] = json_encode(
                    $field["options"],
    ...
    private function fieldsJsonDecode($fields)
    {
        foreach ($fields as $key => $r) {
            ...
            $fields[$key]["default_value"] = json_decode($r["default_value"])
                ? json_decode($r["default_value"], true)
                : $r["default_value"];
        }
        return $fields;
    }
    ...
    {
        $field = $this->di["db"]->getExistingModelById(
            "FormField",
            $fieldId,
            "Field was not found"
        );
    ...
    {
        $fieldModel = $this->di["db"]->getExistingModelById(
            "FormField",
            $data["id"],
            "Field was not found"
        );
    ...
    public function formFieldNameExists($data)
    {
        ...
        $result = $this->di["db"]->findOne(
            "FormField",
            "form_id = ? and name = ? and id != ?",
            [$form_id, $field_name, $field_id]
        );
    ...
    public function duplicateForm($data)
    {
        ...
        $this->di["logger"]->info(
            "Copied form with id %s to new form %s with id %s",
            $data["form_id"],
            $data["name"],
            $new_form_id
        );
    ...
    public function updateFormSettings($data)
    {
        ...
        $sql2 = "UPDATE `form`
                SET `style` =  :type
                WHERE id = :id";
        $this->di["db"]->exec($sql2, [
            ":type" => json_encode($style, JSON_FORCE_OBJECT),
            ":id" => $data["form_id"],
        ]);
        $this->di["logger"]->info(
            "Updated form %s type to %s",
            $data["form_id"],
            $data["type"]
        );
        return true;
