...
class Service implements \Box\InjectionAwareInterface
{
    ...
    public function install()
    {
        ...
        $extensionService->activateExistingExtension([
            "id" => "queue",
            "type" => "mod",
        ]);
    ...
    public function getSearchQuery($data)
    {
        ...
        if (null !== $search) {
            $sql .=
                " AND (subject LIKE :search OR content LIKE :search OR from_email LIKE :search OR from_name LIKE :search)";
    ...
    public function getMessageReceivers($model, $data = [])
    {
        ...
        if (!empty($filter)) {
            if (
                isset($filter["client_status"]) &&
                !empty($filter["client_status"])
            ) {
                $sql .= sprintf(
                    " AND c.status IN ('%s')",
                    implode("', '", $filter["client_status"])
                );
            }

            if (
                isset($filter["client_groups"]) &&
                !empty($filter["client_groups"])
            ) {
                $sql .= sprintf(
                    " AND c.client_group_id IN ('%s')",
                    implode("', '", $filter["client_groups"])
                );
            }
            ...
            if (isset($filter["has_order"]) && !empty($filter["has_order"])) {
                $sql .= sprintf(
                    ...
                    implode("', '", $filter["has_order"])
                );
            }

            if (
                isset($filter["has_order_with_status"]) &&
                !empty($filter["has_order_with_status"])
            ) {
                $sql .= sprintf(
                    " AND co.status IN ('%s')",
                    implode("', '", $filter["has_order_with_status"])
                );
            }
        }
