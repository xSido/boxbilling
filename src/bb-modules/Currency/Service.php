...
class Service implements InjectionAwareInterface
{
    ...
    public function getBaseCurrencyRate($foreign_code)
    {
        ...
        if ($f_rate == 0) {
            throw new \Box_Exception(
                "Currency conversion rate can not be zero"
            );
        }
    ...
    {
        return $this->di["db"]->findOne("Currency", "code = :code", [
            ":code" => $code,
        ]);
    }
    ...
    {
        $sql =
            "INSERT INTO `setting` (`param`, `value`, `public`, `created_at`, `updated_at`) VALUES ('currencylayer', :key, '0', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP()) ON DUPLICATE KEY UPDATE `value`=:key, `updated_at`=CURRENT_TIMESTAMP()";
        $values = [":key" => $key];
    ...
    public function isCronEnabled()
    {
        ...
        if (
            isset($pairs["currency_cron_enabled"]) &&
            $pairs["currency_cron_enabled"] == "1"
        ) {
            return true;
        } else {
            return false;
    ...
    {
        $sql =
            "INSERT INTO `setting` (`param`, `value`, `public`, `created_at`, `updated_at`) VALUES ('currency_cron_enabled', :key, '0', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP()) ON DUPLICATE KEY UPDATE `value`=:key, `updated_at`=CURRENT_TIMESTAMP()";
    ...
    public function validateCurrencyFormat($format)
    {
        if (strpos($format, "{{price}}") === false) {
            throw new \Exception(
                "Currency format must include {{price}} tag",
                3569
            );
        }
    }
    ...
    protected function _getRate($from, $to)
    {
        ...
        if ($from_Currency == "EUR") {
            ...
            if ($array["success"] !== true) {
                throw new \Box_Exception(
                    "<b>Currencylayer threw an error:</b><br />" .
                        $array["error"]["info"]
                );
            } else {
    ...
    public function deleteCurrencyByCode($code)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminDeleteCurrency",
            "params" => ["code" => $code],
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminDeleteCurrency",
            "params" => ["code" => $code],
        ]);
