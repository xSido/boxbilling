...
class Admin extends \Api_Abstract
{
    private function _getService($data)
    {
        ...
        $s = $this->di["db"]->findOne("service_solusvm", "id=:id", [
            ":id" => $order->service_id,
        ]);
        if (!$s) {
    ...
    public function set_root_password($data)
    {
        ...
        $this->di["logger"]->info(
            "Changed VPS root password. Order ID #%s",
            $order->id
        );
        return true;
    }
    ...
    public function set_hostname($data)
    {
        ...
        $this->di["logger"]->info(
            "Changed VPS hostname. Order ID #%s",
            $order->id
        );
        return true;
    }
    ...
    public function rebuild($data)
    {
        ...
        $this->di["logger"]->info(
            "Changed VPS template. Order ID #%s",
            $order->id
        );
        return true;
    }
    ...
    public function network_disable($data)
    {
        ...
        $this->di["logger"]->info(
            "Disabled VPS network. Order ID #%s",
            $order->id
        );
        return true;
    }
    ...
    public function network_enable($data)
    {
        ...
        $this->di["logger"]->info(
            "Enabled VPS network. Order ID #%s",
            $order->id
        );
        return true;
    }
    ...
    public function client_list($data)
    {
        ...
        if ($skip) {
            ...
            foreach ($clients as $key => $client) {
                if (
                    $this->di["db"]->findOne("client", "aid = :aid", [
                        "aid" => $client["id"],
                    ])
                ) {
                    unset($clients[$key]);
    ...
    public function import_servers($data)
    {
        ...
        if (empty($nodeid)) {
            throw new \Box_Exception(
                "Node is not selected for import.",
                null,
                235
            );
        }
        ...
        $required = [
            "node" =>
                "Product is not configured completely. Please provide solusvm node in product configuration page",
            "plan" =>
                "Product is not configured completely. Please provide solusvm plan in product configuration page",
        ];
        ...
        foreach ($servers as $server) {
            ...
            try {
                ...
                $client = $this->di["db"]->findOne("client", "aid = :aid", [
                    "aid" => $server["clientid"],
                ]);
                if (!$client) {
                    throw new \Exception(
                        sprintf(
                            "Client with alternative id %s was not found",
                            $server["clientid"]
                        )
                    );
                }

                [$username] = $this->getService()->getSolusUserPassword(
                    $client
                );
                ...
                $log .=
                    sprintf(
                        'Order for server #%s was not imported due to error "%s"',
                        $server["vserverid"],
                        $e->getMessage()
                    ) . PHP_EOL;
            }
    ...
    public function import_clients($data)
    {
        ...
        if (empty($selected)) {
            throw new \Box_Exception(
                "No clients selected for import.",
                null,
                233
            );
        }
        ...
        foreach ($clients as $client) {
            ...
            try {
                ...
                $log .=
                    sprintf(
                        'Client #%s was not imported due to error "%s"',
                        $client["id"],
                        $e->getMessage()
                    ) . PHP_EOL;
            }
        ...
        $this->di["logger"]->info(
            "Imported clients from SolusVM to BoxBilling"
        );
        return $log;
    }
    ...
    public function update($data)
    {
        ...
        $vps->template = $this->di["array_get"](
            $data,
            "template",
            $vps->template
        );
        $vps->hostname = $this->di["array_get"](
            $data,
            "hostname",
            $vps->hostname
        );
        $vps->mainipaddress = $this->di["array_get"](
            $data,
            "mainipaddress",
            $vps->mainipaddress
        );
        $vps->custommemory = $this->di["array_get"](
            $data,
            "custommemory",
            $vps->custommemory
        );
        $vps->customdiskspace = $this->di["array_get"](
            $data,
            "customdiskspace",
            $vps->customdiskspace
        );
        $vps->custombandwidth = $this->di["array_get"](
            $data,
            "custombandwidth",
            $vps->custombandwidth
        );
        $vps->customcpu = $this->di["array_get"](
            $data,
            "customcpu",
            $vps->customcpu
        );
