...
class Admin extends \Api_Abstract
{
    ...
    public function get_list($data)
    {
        ...
        foreach ($pager["list"] as $key => $item) {
            ...
            $pager["list"][$key] = $this->getService()->toModel_AdminApiiArray(
                $staff
            );
        }
    ...
    public function permissions_update($data)
    {
        ...
        $this->di["logger"]->info(
            "Changed staff member %s permisions",
            $model->id
        );
        return true;
    }
    ...
    public function group_get_list($data)
    {
        ...
        foreach ($pager["list"] as $key => $item) {
            ...
            $pager["list"][$key] = $this->getService()->toAdminGroupApiArray(
                $model,
                false,
                $this->getIdentity()
            );
    ...
    public function group_get($data)
    {
        ...
        return $this->getService()->toAdminGroupApiArray(
            $model,
            true,
            $this->getIdentity()
        );
    ...
    public function login_history_get_list($data)
    {
        [
            $sql,
            $params,
        ] = $this->getService()->getActivityAdminHistorySearchQuery($data);
        $per_page = $this->di["array_get"](
            $data,
            "per_page",
            $this->di["pager"]->getPer_page()
        ...
        foreach ($pager["list"] as $key => $item) {
            $activity = $this->di["db"]->getExistingModelById(
                "ActivityAdminHistory",
                $item["id"],
                sprintf("Staff activity item #%s not found", $item["id"])
            );
            if ($activity) {
                $pager["list"][
                    $key
                ] = $this->getService()->toActivityAdminHistoryApiArray(
                    $activity
                );
            }
        }
