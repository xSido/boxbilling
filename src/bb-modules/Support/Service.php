...
class Service implements \Box\InjectionAwareInterface
{
    ...
    public static function onAfterClientOpenTicket(\Box_Event $event)
    {
        ...
        try {
            ...
            $ticketArr = $supportService->toApiArray(
                $ticketObj,
                true,
                $identity
            );
    ...
    public static function onAfterAdminPublicTicketOpen(\Box_Event $event)
    {
        ...
        try {
            $ticketObj = $supportService->getPublicTicketById($params["id"]);
            $identity = $di["loggedin_admin"];
            $ticketArr = $supportService->publicToApiArray(
                $ticketObj,
                true,
                $identity
            );
    ...
    public function getSearchQuery($data)
    {
        ...
        if ($search) {
            if (is_numeric($search)) {
                ...
                $search = "%" . $search . "%";
                $where[] =
                    "(stm.content LIKE :content OR st.subject LIKE :subject)";
    ...
    public function counter()
    {
        ...
        return [
            ...
            \Model_SupportTicket::OPENED => $this->di["array_get"](
                $data,
                \Model_SupportTicket::OPENED,
                0
            ),
            \Model_SupportTicket::CLOSED => $this->di["array_get"](
                $data,
                \Model_SupportTicket::CLOSED,
                0
            ),
            \Model_SupportTicket::ONHOLD => $this->di["array_get"](
                $data,
                \Model_SupportTicket::ONHOLD,
                0
            ),
        ];
    }
    ...
    public function checkIfTaskAlreadyExists(
        \Model_Client $client,
        $rel_id,
        $rel_type,
        $rel_task
    ...
    public function closeTicket(\Model_SupportTicket $ticket, $identity)
    {
        ...
        if ($identity instanceof \Model_Admin) {
            $this->di["events_manager"]->fire([
                "event" => "onAfterAdminCloseTicket",
                "params" => ["id" => $ticket->id],
            ]);
        } elseif ($identity instanceof \Model_Client) {
            $this->di["events_manager"]->fire([
                "event" => "onAfterClientCloseTicket",
                "params" => ["id" => $ticket->id],
            ]);
        }
    ...
    {
        $supportTicketNotes = $this->di["db"]->find(
            "SupportTicketNote",
        ...
        $supportTicketMessages = $this->di["db"]->find(
    ...
    public function toApiArray(
        ...
        $supportHelpdesk = $this->di["db"]->load(
            "SupportHelpdesk",
            $model->support_helpdesk_id
        );
    ...
    public function noteToApiArray(
        \Model_SupportTicketNote $model,
        $deep = false,
        $identity = null
    ) {
        $data = $this->di["db"]->toArray($model);
    ...
    public function helpdeskGetSearchQuery($data)
    {
        ...
        if ($search) {
            $search = "%" . $search . "%";
            $where[] =
                "(name LIKE :name OR email LIKE :email OR signature LIKE :signature)";
    ...
    public function helpdeskGetPairs()
    {
        return $this->di["db"]->getAssoc(
            "SELECT id, name FROM support_helpdesk"
        );
    }
    ...
    public function helpdeskRm(\Model_SupportHelpdesk $model)
    {
        ...
        $tickets = $this->di["db"]->find(
            "SupportTicket",
            "support_helpdesk_id = :support_helpdesk_id",
            [":support_helpdesk_id" => $model->id]
        );
        if (count($tickets) > 0) {
            throw new \Box_Exception(
                "Can not remove helpdesk which has tickets"
            );
        }
        $this->di["db"]->trash($model);
    ...
    public function messageGetTicketMessages(\Model_SupportTicket $model)
    {
        return $this->di["db"]->find(
            "supportTicketMessage",
            "support_ticket_id = :support_ticket_id ORDER BY id ASC",
            [":support_ticket_id" => $model->id]
        );
    }
    ...
    {
        $model->support_helpdesk_id = $this->di["array_get"](
            $data,
            "support_helpdesk_id",
            $model->support_helpdesk_id
        );
        ...
        $model->priority = $this->di["array_get"](
            $data,
            "priority",
            $model->priority
        );
    ...
    public function ticketMessageUpdate(
        \Model_SupportTicketMessage $model,
        $content
    ) {
    ...
    public function ticketReply(
        ...
        if ($identity instanceof \Model_Admin) {
            $this->di["events_manager"]->fire([
                "event" => "onAfterAdminReplyTicket",
                "params" => ["id" => $ticket->id],
            ]);
        } elseif ($identity instanceof \Model_Client) {
            $this->di["events_manager"]->fire([
                "event" => "onAfterClientReplyTicket",
                "params" => ["id" => $ticket->id],
            ]);
        }
    ...
    public function ticketCreateForAdmin(
        \Model_Client $client,
        \Model_SupportHelpdesk $helpdesk,
        $data,
        \Model_Admin $identity
    ) {
        $status = $this->di["array_get"](
            $data,
            "status",
            \Model_SupportTicket::ONHOLD
        );
    ...
    public function ticketCreateForGuest($data)
    {
        ...
        $altered = $this->di["events_manager"]->fire([
            "event" => "onBeforeGuestPublicTicketOpen",
            "params" => $event_params,
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterGuestPublicTicketOpen",
            "params" => ["id" => $ticketId],
        ]);

        $this->di["logger"]->info(
            '"%s" opened public ticket "%s"',
            $ticket->author_email,
            $ticketId
        );
    ...
    public function canClientSubmitNewTicket(
        \Model_Client $client,
        array $config
    ) {
        $hours = $config["wait_hours"];

        $lastTicket = $this->di["db"]->findOne(
            "SupportTicket",
        ...
        $timeSinceLast = round(
            abs(
                strtotime($lastTicket->created_at) -
        ...
        if ($timeSinceLast < $hours) {
            throw new \Box_Exception(
                sprintf(
                    "You can submit one ticket per %s hours. %s hours left",
                    $hours,
                    $hours - $timeSinceLast
                )
            );
    ...
    public function ticketCreateForClient(
        \Model_Client $client,
        \Model_SupportHelpdesk $helpdesk,
        array $data
    ) {
        ...
        $rel_status = isset($data["rel_task"])
            ? \Model_SupportTicket::REL_STATUS_PENDING
            : \Model_SupportTicket::REL_STATUS_COMPLETE;
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeClientOpenTicket",
            "params" => $event_params,
        ]);
    ...
    {
        try {
            $cannedObj = $this->di["db"]->getExistingModelById(
                "SupportPr",
                $cannedId,
                "Canned reply not found"
            );
     ...
     */
    public function messageCreateForTicket(
        \Model_SupportTicket $ticket,
        $identity,
        $content
    ) {
    ...
    public function publicFindOneByHash($hash)
    {
        ...
        $publicTicket = $this->di["db"]->findOne(
            "SupportPTicket",
            "hash = :hash",
            $bindings
        );
    ...
    public function publicGetSearchQuery($data)
    {
        ...
        if ($search) {
            if (is_numeric($search)) {
                ...
                $search = "%" . $search . "%";
                $where[] =
                    "sptm.content LIKE :p_message_content OR spt.subject LIKE :p_ticket_subject";
    ...
    public function publicCounter()
    {
        ...
        return [
            "total" => array_sum($data),
            \Model_SupportPTicket::OPENED => isset(
                $data[\Model_SupportPTicket::OPENED]
            )
                ? $data[\Model_SupportPTicket::OPENED]
                : 0,
            \Model_SupportPTicket::CLOSED => isset(
                $data[\Model_SupportPTicket::CLOSED]
            )
                ? $data[\Model_SupportPTicket::CLOSED]
                : 0,
            \Model_SupportPTicket::ONHOLD => isset(
                $data[\Model_SupportPTicket::ONHOLD]
            )
                ? $data[\Model_SupportPTicket::ONHOLD]
                : 0,
        ];
    ...
    public function publicGetExpired()
    {
        ...
        $publicTicket = $this->di["db"]->find(
            "SupportPTicket",
            "status = :status AND DATE_ADD(updated_at, INTERVAL 48 HOUR) < NOW() ORDER BY id ASC",
            $bindings
        );
    ...
    public function publicCloseTicket(\Model_SupportPTicket $model, $identity)
    {
        ...
        if ($identity instanceof \Model_Admin) {
            $this->di["events_manager"]->fire([
                "event" => "onAfterAdminPublicTicketClose",
                "params" => ["id" => $model->id],
            ]);
            $this->di["logger"]->info(
                "Public Ticket %s was closed",
                $model->id
            );
        } elseif ($identity instanceof \Model_Guest) {
            $this->di["events_manager"]->fire([
                "event" => "onAfterGuestPublicTicketClose",
                "params" => ["id" => $model->id],
            ]);
            $this->di["logger"]->info(
                '"%s" closed public ticket "%s"',
                $model->author_email,
                $model->id
            );
        }
    ...
    public function publicRm(\Model_SupportPTicket $model)
    {
        ...
        $messages = $this->di["db"]->find(
            "SupportPTicketMessage",
            "support_p_ticket_id = :support_p_ticket_id",
            $bindings
        );
    ...
    public function publicToApiArray(\Model_SupportPTicket $model, $deep = true)
    {
        $data = $this->di["db"]->toArray($model);
        $messages = [];
        $messagesArr = $this->di["db"]->find(
            "SupportPTicketMessage",
            "support_p_ticket_id = :support_p_ticket_id ORDER BY id",
            [":support_p_ticket_id" => $model->id]
        );
    ...
    public function publicMessageGetAuthorDetails(
        \Model_SupportPTicketMessage $model
    ) {
        if ($model->admin_id) {
            $author = $this->di["db"]->getExistingModelById(
                "Admin",
                $model->admin_id
            );
    ...
    public function publicMessageToApiArray(
        \Model_SupportPTicketMessage $model,
        $deep = true
    ) {
        $data = $this->di["db"]->toArray($model);
    ...
    public function publicTicketCreate($data, \Model_Admin $identity)
    {
        ...
        $this->di["events_manager"]->fire([
            "event" => "onBeforeAdminPublicTicketOpen",
            "params" => $data,
        ]);
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminPublicTicketOpen",
            "params" => ["id" => $ticketId],
        ]);

        $this->di["logger"]->info(
            'Opened public ticket for email "%s"',
            $ticket->author_email
        );
    ...
    public function publicTicketReply(
        \Model_SupportPTicket $ticket,
        \Model_Admin $identity,
        $content
    ) {
        $msg = $this->di["db"]->dispense("SupportPTicketMessage");
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterAdminPublicTicketReply",
            "params" => ["id" => $ticket->id],
        ]);
    ...
    public function publicTicketReplyForGuest(
        \Model_SupportPTicket $ticket,
        $message
    ) {
        $msg = $this->di["db"]->dispense("SupportPTicketMessage");
        ...
        $this->di["events_manager"]->fire([
            "event" => "onAfterGuestPublicTicketReply",
            "params" => ["id" => $ticket->id],
        ]);

        $this->di["logger"]->info(
            'Client "%s" replied to public ticket "%s"',
            $ticket->author_email,
            $ticket->id
        );
    ...
    public function helpdeskUpdate(\Model_SupportHelpdesk $model, $data)
    {
        ...
        $model->can_reopen = $this->di["array_get"](
            $data,
            "can_reopen",
            $model->can_reopen
        );
        $model->close_after = $this->di["array_get"](
            $data,
            "close_after",
            $model->close_after
        );
    ...
    public function cannedToApiArray(\Model_SupportPr $model)
    {
        ...
        $category = $this->di["db"]->load(
            "SupportPrCategory",
            $model->support_pr_category_id
        );
    ...
    {
        return $this->di["db"]->getAssoc(
            "SELECT id, title FROM support_pr_category"
        );
    }
    ...
    {
        $model->support_pr_category_id = $this->di["array_get"](
            $data,
            "category_id",
            $model->support_pr_category_id
        );
    ...
    public function cannedCategoryCreate($title)
    {
        ...
        $this->di["logger"]->info(
            "Created new canned response category #%s",
            $id
        );
    ...
    public function cannedCategoryUpdate(
        \Model_SupportPrCategory $model,
        $title
    ) {
        $model->title = $title;
        $model->updated_at = date("Y-m-d H:i:s");
