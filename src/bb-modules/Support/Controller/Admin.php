...
class Admin implements \Box\InjectionAwareInterface
{
    ...
    public function register(\Box_App &$app)
    {
        ...
        $app->get(
            "/support/ticket/:id/message/:messageid",
            "get_ticket",
            ["id" => "[0-9]+", "messageid" => "[0-9]+"],
            get_class($this)
        );
        $app->get(
            "/support/public-tickets",
    ...
    public function get_ticket(\Box_App $app, $id, $messageid = "")
    {
        ...
        try {
            if (
                isset($config["delay_enable"]) &&
                $config["delay_enable"] &&
                isset($config["delay_hours"]) &&
                ...
                $hours_passed =
                    round(
                        (time() - strtotime($last_message["created_at"])) / 3600
                    ) > $config["delay_hours"];
                if ($hours_passed) {
                    $delay_canned = $api->support_canned_get([
                        "id" => $config["delay_message_id"],
                    ]);
        ...
        return $app->render("mod_support_ticket", [
            "ticket" => $ticket,
            "canned_delay_message" => $cdm,
