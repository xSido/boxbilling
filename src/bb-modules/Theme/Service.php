...
class Service implements InjectionAwareInterface
{
    ...
    public function setCurrentThemePreset(
        \Box\Mod\Theme\Model\Theme $theme,
        $preset
    ) {
        ...
        foreach ($files as $filename => $f) {
            ...
            if ($f["error"] != UPLOAD_ERR_OK) {
                throw new \Box_Exception(
                    "Error uploading file :file Error code: :error",
                    [":file" => $filename, ":error" => $f["error"]]
                );
            }
    ...
    public function regenerateThemeSettingsDataFile(
        \Box\Mod\Theme\Model\Theme $theme
    ) {
        $settings = [];
        $presets = $this->getThemePresets($theme);
        foreach ($presets as $preset) {
            $settings["presets"][$preset] = $this->getThemeSettings(
                $theme,
                $preset
            );
        }
        ...
        $this->di["tools"]->file_put_contents(
    ...
    public function regenerateThemeCssAndJsFiles(
        \Box\Mod\Theme\Model\Theme $theme,
        $preset,
        $api_admin
    ) {
    ...
    public function getCurrentClientAreaThemeCode()
    {
        if (defined("BB_THEME_CLIENT")) {
            ...
            $theme = $this->di["db"]->getCell(
                "SELECT value FROM setting WHERE param = 'theme' "
            );
        }
    ...
    private function _loadTheme($theme, $client = true, $mod = null)
    {
        ...
        if (!file_exists($theme_path)) {
            throw new \Box_Exception("Theme was not found in path :path", [
                ":path" => $theme_path,
            ]);
        }
        ...
        if (!is_array($config)) {
            throw new \Box_Exception(
                "Unable to decode theme manifest file :file",
                [":file" => $manifest]
            );
        }
