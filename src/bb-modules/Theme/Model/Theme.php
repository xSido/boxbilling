...
class Theme
{
    ...
    public function __construct($name)
    {
        ...
        if (!file_exists($this->getPath())) {
            throw new \Box_Exception('Theme ":name" does not exists', [
                ":name" => $name,
            ]);
        }
    }
    ...
    public function getSnippets()
    {
        ...
        foreach ($snippets as $snippet) {
            $result[basename($snippet)] = str_replace(
                "snippet_",
                "",
                pathinfo($snippet, PATHINFO_FILENAME)
            );
        }
        return $result;
    }
    ...
    public function getSettingsPageHtml()
    {
        ...
        if (!file_exists($spp)) {
            error_log(
                "Theme " . $this->getName() . " does not have settings page"
            );
            return "";
        }

        $settings_page = file_get_contents($spp);
        $settings_page = $this->strip_tags_content(
            $settings_page,
            "<script><style>"
        );

        //remove style attributes
        $settings_page = preg_replace(
            '/(<[^>]+) style=".*?"/i',
            '$1',
            $settings_page
        );
    ...
    public function getPresetsFromSettingsDataFile()
    {
        ...
        return isset($array["presets"]) ? $array["presets"] : [];
    }
    ...
    public function getPresetFromSettingsDataFile($preset)
    {
        ...
        return is_array($array) && isset($array[$preset])
            ? $array[$preset]
            : [];
    }
