...
class Admin implements \Box\InjectionAwareInterface
{
    ...
    public function save_theme_settings(\Box_App $app, $theme)
    {
        $this->di["events_manager"]->fire([
            "event" => "onBeforeThemeSettingsSave",
            "params" => $_POST,
        ]);
        ...
        $isNewPreset = isset($_POST["save-current-setting"])
            ? (bool) $_POST["save-current-setting"]
            : false;
        $preset = $service->getCurrentThemePreset($t);
        if (
            $isNewPreset &&
            isset($_POST["save-current-setting-preset"]) &&
            !empty($_POST["save-current-setting-preset"])
        ) {
        ...
        try {
            if (!$t->isAssetsPathWritable()) {
                throw new \Box_Exception(
                    'Theme ":name" assets folder is not writable. Files can not be uploaded and settings can not be saved. Set folder permissions to 777',
                    [":name" => $t->getName()]
                );
            }
    ...
    public function get_theme(\Box_App $app, $theme)
    {
        ...
        if (!$t->isAssetsPathWritable()) {
            $info = __(
                'Theme ":name" assets folder is not writable. Set folder :folder permissions to 777',
                [":name" => $t->getName(), ":folder" => $t->getPathAssets()]
            );
        }

        if (empty($html)) {
            $info = __('Theme ":name" is not configurable', [
                ":name" => $t->getName(),
            ]);
        }
