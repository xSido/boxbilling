...
echo sprintf("Connecting to database %s@%s/%s", $user, $host, $dbname) .
    PHP_EOL;
...
while ($connected == false && $iter > 0) {
    try {
        $dbh = new PDO($type . ":host=" . $host, $user, $password, [
        ...
        $message =
            "SQLSTATE[HY000] [2002] Connection refused in /var/www/html/bin/prepare.php:23";
...
echo sprintf("Connecting to %s database with user %s", $dbname, $user) .
    PHP_EOL;
...
echo sprintf("Create SQL database structure from file %s", $structureSql) .
    PHP_EOL;
