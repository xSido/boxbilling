...
class Box_Mod_Client_ServiceTest extends ApiTestCase
{
    ...
    public function testRemove()
    {
        ...
        $invoiceModel = $this->di["db"]->findOne("Invoice", "client_id = ?", [
            $id,
        ]);
        ...
        $bool = $this->api_admin->client_balance_add_funds([
            "id" => 1,
            "amount" => 100,
        ...
        $this->assertNull($model);
        $model = $this->di["db"]->find("ForumTopicMessage", "client_id = ?", [
            $msg->client_id,
        ]);
        $this->assertEmpty($model);
        $model = $this->di["db"]->load("ClientPasswordReset", $r->id);
        $this->assertNull($model);
        $model = $this->di["db"]->find("ClientBalance", "client_id = ?", [
            $clientModel->id,
        ]);
        $this->assertEmpty($model);
