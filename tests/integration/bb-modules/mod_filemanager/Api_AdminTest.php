...
class Box_Mod_Filemanager_Api_AdminTest extends BBDbApiTestCase
{
    public function testActions()
    {
        ...
        $bool = $this->api_admin->filemanager_new_item([
            "path" => "bb-data/tmp.txt",
            "type" => "file",
        ]);
        ...
        $bool = $this->api_admin->filemanager_new_item([
            "path" => BB_PATH_DATA . "/new2",
            "type" => "dir",
        ]);
        $this->assertTrue($bool);

        $bool = $this->api_admin->filemanager_save_file([
            "path" => "bb-data/cache/tmp.txt",
            "data" => "content",
        ]);
        $this->assertTrue($bool);

        $bool = $this->api_admin->filemanager_move_file([
            "path" => "bb-data/cache/tmp.txt",
            "to" => "bb-data/log",
        ]);
        $this->assertTrue($bool);
