...
class Api_Admin_InvoiceTest extends BBDbApiTestCase
{
    ...
    public function testTransactionsSearch()
    {
        ...
        $list = $this->api_admin->invoice_transaction_get_list([
            "txn_id" => $txn_id,
        ]);
    ...
    public function testNumbering()
    {
        ...
        $next = $this->api_admin->system_param([
    ...
    public function testRefund()
    {
        ...
        $bool = $this->api_admin->system_update_params([
        ...
        $bool = $this->api_admin->system_update_params([
            "invoice_refund_logic" => "negative_invoice",
        ]);
    ...
    public function testDepositInvoice()
    {
        ...
        $invoiceItemModel = $this->di["db"]->findOne(
            "InvoiceItem",
            "invoice_id = ?",
            [$invoiceModel->id]
        );
        $this->assertEquals(
            Model_InvoiceItem::TYPE_DEPOSIT,
            $invoiceItemModel->type
        );
    ...
    public function testDepositInvoiceMarkAsPaid()
    {
        ...
        $this->assertEquals(
            $balanceAfter,
            $balanceBefore + $invoiceItemModel->price
        );

        $accountBalance = $this->di["db"]->findOne(
            "ClientBalance",
            "order by id desc"
        );
        $this->assertEquals(
            $invoiceItemModel->title,
    ...
    public function testPrepareWithItems()
    {
        ...
        $this->assertEquals(0.0, $line1["price"]);
    }
    ...
    public function testTaskHook()
    {
        ...
        $this->api_admin->extension_activate([
            "type" => "mod",
            "id" => "example",
        ]);
        $invoice = $this->api_admin->invoice_mark_as_paid([
            "id" => $invoice_id,
            "execute" => true,
        ]);
        //@todo
    ...
    public function testInvoiceBatchDeleteSubscription()
    {
        ...
        $result = $this->api_admin->invoice_batch_delete_subscription([
            "ids" => $ids,
        ]);
        $array = $this->api_admin->invoice_subscription_get_list([]);
    ...
    public function testInvoiceBatchDeleteTransaction()
    {
        ...
        $result = $this->api_admin->invoice_batch_delete_transaction([
            "ids" => $ids,
        ]);
        $array = $this->api_admin->invoice_transaction_get_list([]);
    ...
    public function testPrepareInvoiceDueDate($invoice_due_days, $diff)
    {
        if (!is_null($invoice_due_days)) {
            $this->api_admin->system_update_params([
        ...
        $array = $this->api_admin->invoice_get(["id" => $id]);
        $this->assertEquals(
            substr($array["due_at"], 0, 10),
            date("Y-m-d", strtotime("+ $diff day"))
        );
    }
