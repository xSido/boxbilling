...
class Box_Mod_Cart_Api_ClientTest extends BBDbApiTestCase
{
    ...
    public function testCheckout()
    {
        ...
        $this->assertTrue(
            $cart_before_promo["total"] != $cart_after_promo["total"],
            "Could not apply promo to cart"
        );
    ...
    public function testPromoOrderActivation()
    {
        ...
        $bool = $this->api_guest->cart_apply_promo([
            "promocode" => "TOTAL_DISCOUNT",
        ]);
        $this->assertTrue($bool);
        ...
        $this->assertEquals(
            "active",
            $order["status"],
            "Order should be activated if total amount is 0 after discount and product setup is after_payment"
        );
    }
    ...
    public function testPromoOncePerClient()
    {
        ...
        try {
            $this->api_client->cart_checkout();
            $this->fail(
                "Promo code was aplied multiple times. Promo can only be applied once."
            );
        } catch (Exception $e) {
    ...
    public function testAddonActivatedWhileOrderInvoiceUnpaid()
    {
        ...
        $masterOrder = $this->di["db"]->findOne(
            "ClientOrder",
            "group_master = 1 ORDER BY id desc"
        );
        $addonOrder = $this->di["db"]->findOne(
            "ClientOrder",
            "group_id = ? and group_master = 0",
            [$masterOrder->group_id]
        );

        $invoiceModel = $this->di["db"]->load(
            "Invoice",
            $masterOrder->unpaid_invoice_id
        );
    ...
    public function testOrderWithAddonCoverFromAccountBalance()
    {
        ...
        $invoiceItemModel = $this->di["db"]->findOne(
            "InvoiceItem",
            "type = 'order' and rel_id = ?",
            [$masterOrder->id]
        );
        $invoiceModel = $this->di["db"]->load(
            "Invoice",
            $invoiceItemModel->invoice_id
        );
