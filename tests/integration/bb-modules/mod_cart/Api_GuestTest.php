...
class Box_Mod_Cart_Api_GuestTest extends BBDbApiTestCase
{
    ...
    public function testAddHostingProductWithFreeDomainRegistration()
    {
        ...
        $this->assertEquals(
            10,
            $cart["items"][1]["discount"],
            "Could not set free domain for hosting product"
        );
    }
    ...
    public function testAddHostingProductWithFreeDomainTransfer()
    {
        ...
        $this->assertEquals(
            15,
            $cart["items"][1]["discount"],
            "Could not set free transfer for hosting product"
        );
    }
    ...
    public function testFreeSetupPromo()
    {
        ...
        $this->assertEquals(
            20,
            $cart_after_promo["items"][0]["discount_setup"]
        );
        ...
        $invoice = $this->api_client->invoice_get([
    ...
    public function testMultiple()
    {
        ...
        $this->assertEquals(
            1,
            $res["total"],
            "Generated more than one invoice"
        );
    }
     ...
     */
    public function testApplyPromoForClient(
        $discount,
        $clientGroupId,
        $ids,
        ...
        $cart_after_promo = $this->api_guest->cart_get();
        $this->assertTrue(
            $cart_before_promo["total"] -
                ($cart_before_promo["total"] * $discount) / 100 ==
                $cart_after_promo["total"],
            "Could not apply promo to cart"
        );
    }
}
