...
class PhpConsole
{
    ...
    {
        return isset($_COOKIE[self::clientProtocolCookie]) &&
            $_COOKIE[self::clientProtocolCookie] == self::serverProtocol;
    }

    protected static function setEnabledOnServer()
    {
        if (
            !isset($_COOKIE[self::serverProtocolCookie]) ||
            $_COOKIE[self::serverProtocolCookie] != self::serverProtocol
        ) {
    ...
    protected function sendEventToClient(PhpConsoleEvent $event)
    {
        ...
        $message = [];
        $message["type"] =
            strpos($event->tags, "error,") === 0 ? "error" : "debug";
        ...
        if ($event->file) {
            $message["source"] =
                ($this->sourceBasePath
                    ? preg_replace(
                        "!^" . preg_quote($this->sourceBasePath, "!") . "!",
                        "",
                        $event->file
                    )
        ...
        if ($event->trace) {
            $traceArray = $this->convertTraceToArray(
                $event->trace,
                $event->file,
                $event->line
            );
    ...
    protected function convertTraceToArray(
        $traceData,
        $eventFile = null,
        $eventLine = null
    ) {
        ...
        foreach ($traceData as $call) {
            if (
                (isset($call["class"]) && $call["class"] == __CLASS__) ||
                (!$trace &&
                    isset($call["file"]) &&
                    $call["file"] == $eventFile &&
                    $call["line"] == $eventLine)
    ...
    {
        return substr(number_format(microtime(1), 3, "", ""), -6) +
            self::$index++;
    }
        ...
        if (self::$messagesBuffer) {
            ...
            if (self::$cookiesSent == self::cookiesLimit) {
                ...
                $message = [
                    ...
                    "subject" => "PHP CONSOLE",
                    "text" =>
                        "MESSAGES LIMIT EXCEEDED BECAUSE OF COOKIES STORAGE LIMIT. TOTAL MESSAGES SENT: " .
                        self::$messagesSent,
                    "source" => __FILE__,
                    "notify" => 3,
                ];
    ...
    protected static function setCookie($name, $value)
    {
        if (headers_sent($file, $line)) {
            die(
                "PhpConsole ERROR: setcookie() failed because haders are sent (" .
                    $file .
                    ":" .
                    $line .
                    "). Try to use ob_start()"
    ...
    {
        self::setCookie(
            self::messagesCookiePrefix . self::getNextIndex(),
    ...
    public function handleError(
        ...
        $event->tags =
            "error," .
            (isset($this->codesTags[$code])
                ? $this->codesTags[$code]
                : "warning");
        $event->message = $message;
        $event->type = isset($this->codesNames[$code])
            ? $this->codesNames[$code]
            : $code;
        $event->file = $file;
        $event->line = $line;
        ...
        if (self::$callOldErrorHandler && $this->oldErrorHandler) {
            call_user_func_array($this->oldErrorHandler, [
                $code,
                $message,
                $file,
                $line,
    ...
    {
        $this->oldExceptionsHandler = set_exception_handler([
            $this,
            "handleException",
        ]);
    }
