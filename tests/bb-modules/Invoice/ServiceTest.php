...
class ServiceTest extends \BBTestCase
{
    ...
    public function testgetSearchQuery($data, $expectedStr, $expectedParams)
    {
        ...
        $this->assertTrue(
            strpos($result[0], $expectedStr) !== false,
            $result[0]
        );
    ...
    public function testtoApiArray()
    {
        ...
        $itemInvoiceServiceMock = $this->getMockBuilder(
            "\Box\Mod\Invoice\ServiceInvoiceItem"
        )->getMock();
        $itemInvoiceServiceMock
        ...
        $systemService->expects($this->atLeastOnce())->method("getCompany");

        $subscriptionServiceMock = $this->getMockBuilder(
        ...
        $periodMock = $this->getMockBuilder("\Box_Period")
            ...
            ->getMock();
        $periodMock->expects($this->atLeastOnce())->method("getUnit");
        $periodMock->expects($this->atLeastOnce())->method("getQty");
    ...
    public function testonAfterAdminInvoicePaymentReceived()
    {
        ...
        $emailService = $this->getMockBuilder(
            "\Box\Mod\Email\Service"
        )->getMock();
        $emailService->expects($this->atLeastOnce())->method("sendTemplate");
        ...
        $result = $this->service->onAfterAdminInvoicePaymentReceived(
            $eventMock
        );
    ...
    {
        $serviceMock = $this->getMockBuilder("\Box\Mod\Invoice\Service")
            ->setMethods(["countIncome", "getNextInvoiceNumber"])
            ->getMock();

        $serviceMock->expects($this->atLeastOnce())->method("countIncome");
        $serviceMock
            ->expects($this->atLeastOnce())
        ...
        $currencyService = $this->getMockBuilder(
            "\Box\Mod\Currency\Service"
        )->getMock();
        $currencyService
            ->expects($this->atLeastOnce())
        ...
        $eventManagerMock->expects($this->atLeastOnce())->method("fire");
        ...
        $di["mod_service"] = $di->protect(function (
            $serviceName,
            $sub = ""
        ) use ($systemService, $itemInvoiceServiceMock, $currencyService) {
            if ($serviceName == "system") {
    ...
    public function testgetNextInvoiceNumber()
    {
        ...
        $systemService->expects($this->atLeastOnce())->method("getParamValue");
        $systemService->expects($this->atLeastOnce())->method("setParamValue");
    ...
    public function testprepareInvoiceCurrencyWasNotDefined()
    {
        ...
        $di["mod_service"] = $di->protect(function (
            $serviceName,
            $sub = ""
        ) use ($currencyService, $itemInvoiceServiceMock) {
            if ($serviceName == "Currency") {
                return $currencyService;
            }
            if ($sub == "InvoiceItem") {
    ...
    public function testsetInvoiceDefaults()
    {
        ...
        $serviceTaxMock = $this->getMockBuilder(
        ...
        $di["mod_service"] = $di->protect(function (
            $serviceName,
            $sub = ""
        ) use ($clientService, $systemService, $serviceTaxMock) {
    ...
    public function testrefundInvoiceWithNegativeInvoiceLogic()
    {
        ...
        $serviceMock
            ...
            ->method("getTax")
            ->will($this->returnValue($tax));
        $serviceMock->expects($this->once())->method("countIncome");
        $serviceMock->expects($this->exactly(3))->method("addNote");
        $serviceMock->expects($this->once())->method("getNextInvoiceNumber");
    ...
    {
        $serviceMock = $this->getMockBuilder("\Box\Mod\Invoice\Service")
            ->setMethods(["rmInvoice"])
            ->getMock();
        $serviceMock->expects($this->once())->method("rmInvoice");
    ...
    public function testdeleteInvoiceByClientInvoiceIsRelatedToOrder()
    {
        ...
        $this->expectExceptionMessage(
            sprintf(
                "Invoice is related to order #%d. Please cancel order first.",
                $rel_id
            )
        );
    ...
    public function testrenewInvoice()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Invoice\Service")
            ->setMethods(["generateForOrder", "approveInvoice"])
            ->getMock();
    ...
    public function testdoBatchPayWithCredits()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Invoice\Service")
            ->setMethods(["findAllUnpaid", "tryPayWithCredits"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
    ...
    public function testgenerateForOrder()
    {
        ...
        $invoiceItemServiceMock = $this->getMockBuilder(
            "\Box\Mod\Invoice\ServiceInvoiceItem"
        )->getMock();
    ...
    {
        $orderService = $this->getMockBuilder(
    ...
    public function testdoBatchRemindersSend()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Invoice\Service")
            ->setMethods(["getUnpaidInvoicesLateFor", "sendInvoiceReminder"])
            ->getMock();
        $serviceMock->expects($this->once())->method("sendInvoiceReminder");
        $serviceMock
            ->expects($this->once())
    ...
    {
        $sqlResult = [
            ["status" => \Model_Invoice::STATUS_PAID, "counter" => 2],
        ];
        $dbMock = $this->getMockBuilder("\Box_Database")->getMock();
        $dbMock
            ->expects($this->atLeastOnce())
    ...
    public function testgenerateFundsInvoice()
    {
        ...
        $di["mod_service"] = $di->protect(function (
            $serviceName,
            $sub = ""
        ) use ($systemService, $itemInvoiceServiceMock) {
        ...
        $result = $serviceMock->generateFundsInvoice(
            $clientModel,
            $fundsAmount
        );
        $this->assertInstanceOf("\Model_Invoice", $result);
    ...
    public function testprocessInvoice()
    {
        ...
        $subcribeService = $this->getMockBuilder(
            "\Box\Mod\Invoice\ServiceSubscription"
        )->getMock();
        $subcribeService
            ->expects($this->atLeastOnce())
        ...
        $di["mod_service"] = $di->protect(function (
            $serviceName,
            $sub = ""
        ) use ($payGatewayService, $subcribeService) {
            if ($sub == "PayGateway") {
