...
class ServiceTest extends \BBTestCase
{
     ...
     */
    public function testToBaseCurrency(
        $defaultCode,
        $foreignCode,
        $amount,
        $rate,
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Currency\Service")
            ->setMethods(["getDefault", "getBaseCurrencyRate"])
            ->getMock();
        ...
        $serviceMock
            ->expects($this->any()) // will not be called when currencies are the same, so using any()
    ...
    public function getCurrencyByClientIdProvider()
    {
        ...
        return [
            [$model, "USD", $this->atLeastOnce(), $this->never()],
            [$model, null, $this->never(), $this->atLeastOnce()],
        ];
    }
     ...
     */
    public function testGetCurrencyByClientId(
        $row,
        $currency,
        $expectsGetByCode,
        $getDefaultCalled
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Currency\Service")
            ->setMethods(["getDefault", "getByCode"])
            ->getMock();

        $serviceMock
            ->expects($getDefaultCalled)
            ->method("getDefault")
            ->will($this->returnValue($row));

        $serviceMock
            ->expects($expectsGetByCode)
            ->method("getByCode")
            ->will($this->returnValue($row));
    ...
    {
        return [["EUR", 0.6, 0.6], ["GBP", null, 1], ["GBP", "rate", 1]];
    }
    ...
    public function setAsDefaultProvider()
    {
        ...
        return [
            [$firstModel, $this->atLeastOnce()],
            [$secondModel, $this->never()],
        ];
    }
    ...
    public function testUpdateCurrency()
    {
        ...
        $service = $this->getMockBuilder("\Box\Mod\Currency\Service")
    ...
    public function testUpdateCurrencyNotFoundException()
    {
        ...
        $service->updateCurrency(
            ...
            $price_format,
            $conversion_rate
        ); //Expecting \Box_Exception every time
    }
    ...
    public function testUpdateCurrencyRates()
    {
        ...
        $service = $this->getMockBuilder("\Box\Mod\Currency\Service")
            ->setMethods(["getDefault", "_getRate"])
            ->getMock();
        $service
            ->expects($this->atLeastOnce())
            ->method("getDefault")
