...
class ServiceTest extends \BBTestCase
{
    ...
    public function testgetCurrentThemePreset()
    {
        $serviceMock = $this->getMockBuilder("\Box\Mod\Theme\Service")
            ->setMethods(["setCurrentThemePreset"])
            ->getMock();
        $serviceMock
        ...
        $di["theme"] = $di->protect(function () use ($themeMock) {
            return $themeMock;
        });
        $di["db"] = $dbMock;
    ...
    public function testsetCurrentThemePreset()
    {
        ...
        $this->service->setDi($di);
        $result = $this->service->setCurrentThemePreset(
            $themeMock,
            "dark_blue"
        );
    ...
    {
        $serviceMock = $this->getMockBuilder("\Box\Mod\Theme\Service")
            ->setMethods(["updateSettings"])
            ->getMock();
        $serviceMock->expects($this->atLeastOnce())->method("updateSettings");

        $dbMock = $this->getMockBuilder("\Box_Database")->getMock();
        $dbMock->expects($this->atLeastOnce())->method("getAssoc");
    ...
    {
        $serviceMock = $this->getMockBuilder("\Box\Mod\Theme\Service")
            ->setMethods(["getCurrentThemePreset"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
    ...
    public function testregenerateThemeSettingsDataFile()
    {
        ...
        $toolsMock->expects($this->atLeastOnce())->method("file_put_contents");
    ...
    public function testregenerateThemeCssAndJsFiles_EmptyFiles()
    {
        ...
        $result = $this->service->regenerateThemeCssAndJsFiles(
            $themeMock,
            "default",
            new \Model_Admin()
        );
    ...
    public function testregenerateThemeCssAndJsFiles()
    {
        ...
        $toolsMock
            ->expects($this->atLeastOnce())
            ->method("glob")
            ->will($this->onConsecutiveCalls(["css.css"], ["js.js"]));
        $toolsMock->expects($this->atLeastOnce())->method("file_put_contents");
        ...
        $result = $serviceMock->regenerateThemeCssAndJsFiles(
            $themeMock,
            "default",
            new \Model_Admin()
        );
    ...
    public function testgetCurrentClientAreaTheme()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Theme\Service")
            ->setMethods(["getCurrentClientAreaThemeCode", "getTheme"])
            ->getMock();
    ...
    public function testuploadAssets()
    {
        $themeMock = $this->getMockBuilder("\Box\Mod\Theme\Model\Theme")
            ->disableOriginalConstructor()
            ->getMock();
        $themeMock->expects($this->atLeastOnce())->method("getPathAssets");
        $files = [
            "test2" => [
    ...
    public function testuploadAssets_Exception()
    {
        ...
        $this->expectExceptionMessage(
            sprintf(
                "Error uploading file %s Error code: %d",
                "test0",
