...
class ServiceTest extends \BBTestCase
{
    ...
    {
        return [
            [100, $this->atLeastOnce(), $this->never()],
            [null, $this->never(), $this->atLeastOnce()],
        ];
    }
     ...
     */
    public function testGetSessionCartDoesNotExist(
        $sessionGetWillReturn,
        $getCurrencyByClientIdExpects,
        $getDefaultExpects
    ) {
        $service = new \Box\Mod\Cart\Service();
        ...
        $currencyServiceMock = $this->getMockBuilder(
            "\Box\Mod\Currency\Service"
        )
            ->setMethods(["getCurrencyByClientId", "getDefault"])
            ->getMock();
        $currencyServiceMock
            ->expects($getCurrencyByClientIdExpects)
            ->method("getCurrencyByClientId")
            ->will($this->returnValue($curencyModel));
        $currencyServiceMock
            ->expects($getDefaultExpects)
            ->method("getDefault")
            ->will($this->returnValue($curencyModel));
    ...
    public function testIsRecurrentPricing()
    {
        ...
        $productTable
            ->expects($this->atLeastOnce())
            ->method("getPricingArray")
            ->will(
                $this->returnValue(["type" => \Model_ProductPayment::RECURRENT])
            );
    ...
    public function testIsPeriodEnabledForProduct()
    {
        ...
        $result = $this->service->isPeriodEnabledForProduct(
            $productModelMock,
            "monthly"
        );
    ...
    public function testResetCart()
    {
        ...
        $dbMock
            ...
            ->method("find")
            ->will(
                $this->returnValue([new \Model_Product(), new \Model_Product()])
            );
        $dbMock
            ->expects($this->atLeastOnce())
            ->method("trash")
    ...
    {
        $serviceMock = $this->getMockBuilder("\Box\Mod\Cart\Service")
            ->setMethods(["promoCanBeApplied", "clientHadUsedPromo"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
    ...
    public function testCheckoutCart()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Cart\Service")
            ->setMethods(["createFromCart", "isClientAbleToUsePromo", "rm"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
            ->method("createFromCart")
            ->will($this->returnValue([$order, rand(1, 100), [rand(1, 100)]]));
        $serviceMock
            ->expects($this->atLeastOnce())
            ->method("isClientAbleToUsePromo")
            ->will($this->returnValue(true));
        $serviceMock
            ->expects($this->atLeastOnce())
            ->method("rm")
    ...
    public function testCheckoutCartClientIsNotAbleToUsePromoException()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Cart\Service")
            ->setMethods(["isClientAbleToUsePromo"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
            ->method("isClientAbleToUsePromo")
    ...
    public function testaddItemm_RecurringPaymentPeriodParamMissing()
    {
        ...
        $eventMock = $this->getMockBuilder("\Box_EventManager")->getMock();
        $eventMock->expects($this->atLeastOnce())->method("fire");
        $serviceHostingServiceMock = $this->getMockBuilder(
            "\Box\Mod\Servicehosting"
        )->getMock();

        $serviceMock = $this->getMockBuilder("\Box\Mod\Cart\Service")
            ->setMethods(["isRecurrentPricing"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
        ...
        $validatorMock
            ...
            ->willThrowException(
                new \Box_Exception("Period parameter not passed")
            );
    ...
    public function testaddItemm_RecurringPaymentPeriodIsNotEnabled()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Cart\Service")
            ->setMethods(["isRecurrentPricing", "isPeriodEnabledForProduct"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
            ->method("isRecurrentPricing")
    ...
    public function testaddItemm_OutOfStock()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Cart\Service")
            ->setMethods(["isRecurrentPricing", "isStockAvailable"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
            ->method("isRecurrentPricing")
    ...
    public function testaddItemm_TypeHosting()
    {
        ...
        $serviceMock
            ...
            ->method("isStockAvailable")
            ->will($this->returnValue(true));
        $serviceMock->expects($this->atLeastOnce())->method("addProduct");
    ...
    public function testaddItemm_TypeLicense()
    {
        ...
        $serviceLicenseServiceMock = $this->getMockBuilder(
            "\Box\Mod\Servicelicense\Service"
        )->getMock();
        $serviceLicenseServiceMock
            ->expects($this->atLeastOnce())
    ...
    public function testaddItemm_TypeCustom()
    {
        ...
        $serviceCustomServiceMock = $this->getMockBuilder(
            "\Box\Mod\Servicecustom\Service"
        )->getMock();
        $serviceCustomServiceMock
            ->expects($this->atLeastOnce())
    ...
    public function testtoApiArray()
    {
        ...
        $serviceMock = $this->getMockBuilder("\Box\Mod\Cart\Service")
            ->setMethods(["getCartProducts", "cartProductToApiArray"])
            ->getMock();
        $serviceMock
            ->expects($this->atLeastOnce())
            ->method("getCartProducts")
            ->will($this->returnValue([$cartProductModel]));
        $cartProductApiArray = [
            "total" => 1,
            "discount_price" => 0,
        ];
    ...
    public function testgetProductDiscount()
    {
        ...
        $serviceMock->setDi($di);
        $setupPrice = 0;
        $result = $serviceMock->getProductDiscount(
            $cartProductModel,
            $setupPrice
        );
     ...
     */
    public function testIsPromoAvailableForClientGroup(
        \Model_Promo $promo,
        $client,
        $expectedResult
    ) {
