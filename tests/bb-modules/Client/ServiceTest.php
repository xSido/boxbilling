...
class ServiceTest extends \BBTestCase
{
    ...
    public function testapproveClientEmailByHash()
    {
        ...
        $database
            ...
            ->method("getRow")
            ->will($this->returnValue(["client_id" => 2, "id" => 1]));
    ...
    public function testgenerateEmailConfirmationLink()
    {
        ...
        $toolsMock
            ...
            ->will(
                $this->returnValue(
                    "boxbilling.com/index.php/client/confirm-email/"
                )
            );
    ...
    public function testonAfterClientSignUp()
    {
        ...
        $di["mod_config"] = $di->protect(function ($name) use ($di) {
            return ["require_email_confirmation" => false];
        });
    ...
    public function testRequireEmailConfirmonAfterClientSignUp()
    {
        ...
        $clientServiceMock = $this->getMockBuilder("\Box\Mod\Client\Service")
            ->setMethods(["generateEmailConfirmationLink"])
            ->getMock();
        $clientServiceMock
            ->expects($this->atLeastOnce())
            ->method("generateEmailConfirmationLink")
            ->will($this->returnValue("Link_string"));
        ...
        $di["mod_config"] = $di->protect(function ($name) use ($di) {
            return ["require_email_confirmation" => true];
        });
        $eventMock
            ->expects($this->atLeastOnce())
            ->method("getDi")
    ...
    public function testExceptiononAfterClientSignUp()
    {
        ...
        $service
            ...
            ->will(
                $this->throwException(
                    new \Exception("exception created in unit test")
                )
            );
    ...
    public function testgetSearchQueryChangeSelect()
    {
        ...
        $this->assertTrue(
            strpos($result[0], $selectStmt) !== false,
            $result[0]
        );
    }
        ...
        $database
            ...
            ->will(
                $this->onConsecutiveCalls(
                    $this->returnValue(null, $clientorderModel)
                )
            );
     ...
     */
    public function testgetBalanceSearchQuery(
        $data,
        $expectedStr,
        $expectedParams
    ) {
     ...
     */
    public function testgetHistorySearchQuery(
        $data,
        $expectedStr,
        $expectedParams
    ) {
    ...
    public function testgetByLoginDetails()
    {
        ...
        $result = $clientService->getByLoginDetails(
            "email@example.com",
            "password"
        );
    ...
    public function testtoApiArray()
    {
        ...
        $dbMock
            ->expects($this->atLeastOnce())
            ->method("load")
            ->will($this->returnValue($clientGroup));

        $di = new \Box_Di();
        $di["db"] = $dbMock;

        $serviceMock = $this->getMockBuilder("\Box\Mod\Client\Service")
            ->setMethods(["getClientBalance"])
            ->getMock();
        $serviceMock->expects($this->atLeastOnce())->method("getClientBalance");
     ...
     */
    public function testIsClientTaxable(
        $getParamValueReturn,
        $tax_exempt,
        $expected
    ) {
    ...
    public function testguestCreateClient()
    {
        ...
        $this->assertEquals(
            $ip,
            $result->ip,
            "IP address is not saved while creating client"
        );
    }
    ...
    public function testdeleteGroup()
    {
        ...
        $dbMock->expects($this->once())->method("trash");
    ...
    public function testauthorizeClient()
    {
        ...
        $authMock = $this->getMockBuilder("\Box_Authorization")
            ->disableOriginalConstructor()
            ->getMock();
        $authMock
            ->expects($this->atLeastOnce())
    ...
    public function testauthorizeClientEmailRequiredNotConfirmed()
    {
        ...
        $dbMock
            ...
            ->will(
                $this->onConsecutiveCalls($clientModel, $extensionMetaModel)
            );
    ...
    public function testcheckExtraRequiredFields()
    {
        ...
        $this->expectException(\Box_Exception::class);
        $this->expectExceptionMessage(
            'It is required that you provide details for field "Id"'
        );
    ...
    public function testcheckCustomFields()
    {
        ...
        $this->expectException(\Box_Exception::class);
        $this->expectExceptionMessage(
            'It is required that you provide details for field "custom_field_title"'
        );
