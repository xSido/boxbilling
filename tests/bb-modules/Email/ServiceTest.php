...
class ServiceTest extends \BBTestCase
{
     ...
     */
    public function testSendTemplateExistsStaff(
        $data,
        $clientGetExpects,
        $staffgetListExpects
    ) {
    ...
    public function testTemplate_update($data, $templateRenderExpects)
    {
        ...
        $emailServiceMock = $this->getMockBuilder("Box\Mod\Email\Service")
            ->setMethods(["template_render"])
            ->getMock();

        $db = $this->getMockBuilder("Box_Database")->getMock();
        $db->expects($this->atLeastOnce())->method("store");
        ...
        $result = $emailServiceMock->updateTemplate(
            $templateModel,
            $data["enabled"],
            $data["category"],
            $data["subject"],
    ...
    {
        return [
            [true, false, $this->never(), $this->never()],
            [false, true, $this->atLeastOnce(), $this->atLeastOnce()],
            [true, true, $this->atLeastOnce(), $this->never()],
        ];
    }
    /**
    ...
    public function testbatchSend()
    {
        ...
        $activityMock = $this->getMockBuilder("\Box\Mod\Activity\Service")
            ->setMethods(["logEmail"])
            ->getMock();
        $activityMock
            ->expects($this->atLeastOnce())
