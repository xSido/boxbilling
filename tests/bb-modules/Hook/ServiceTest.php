...
class ServiceTest extends \BBTestCase
{
    ...
    public function testgetSearchQuery()
    {
        ...
        $this->assertTrue(
            strpos(
                $sql,
                "SELECT id, rel_type, rel_id, meta_value as event, created_at, updated_at"
            ) !== false
        );
    ...
    public function testonAfterAdminActivateExtension()
    {
        ...
        $eventMock = $this->getMockBuilder("\Box_Event")
            ->setMethods(["getParameters", "getDi"])
        ...
        $hookService = $this->getMockBuilder(
    ...
    public function testbatchConnect()
    {
        ...
        $activityServiceMock = $this->getMockBuilder(
            "\Box\Mod\Activity\Service"
        )->getMock();

        $boxModMock = $this->getMockBuilder("\Box_Mod")
            ->disableOriginalConstructor()
            ->getMock();
        $boxModMock
            ->expects($this->atLeastOnce())
